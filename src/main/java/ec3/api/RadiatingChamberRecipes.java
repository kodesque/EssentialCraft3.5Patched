...
public class RadiatingChamberRecipes {
    ...
    public static List<RadiatingChamberRecipe> getRecipeByCP(ItemStack[] craftingPair) {
        ...
        for (int i = 0; i < req.length; ++i) {
            if (craftingPair[i] != null) req[i] = craftingPair[i].copy();
            else req[i] = null;
        }
        for (int i = 0; i < req.length; ++i) {
            if (req[i] != null) req[i].stackSize = 0;
        }
    ...
    public static boolean addRecipe(RadiatingChamberRecipe rec) {
        try {
            ...
            for (int i = 0; i < req.length; ++i) {
                if (rec.recipeItems[i] != null) req[i] = rec.recipeItems[i].copy();
                else req[i] = null;
            }
            for (int i = 0; i < req.length; ++i) {
                if (req[i] != null) req[i].stackSize = 0;
            ...
            if (recipes.containsKey(Arrays.toString(req))) lst = recipes.get(Arrays.toString(req));
            else lst = new ArrayList<RadiatingChamberRecipe>();
            lst.add(rec);
    ...
    public static boolean addRecipeIS(ItemStack[] craftingPair, ItemStack result, int mruRequired,
        float[] balanceBounds, int size) {
        try {
            RadiatingChamberRecipe addedRecipe = new RadiatingChamberRecipe(
                craftingPair,
                result,
                mruRequired,
                balanceBounds,
            ...
            Notifier.notifyCustomMod(
                "EssentialCraftAPI",
                "Unable to add recipe " + Arrays
                    .toString(craftingPair) + " with the result " + result + " on side " + side);
            return false;
        }
    }

    public static boolean addRecipeIS(ItemStack[] craftingPair, ItemStack result, int mruRequired,
        float[] balanceBounds, float m, int size) {
        try {
            RadiatingChamberRecipe addedRecipe = new RadiatingChamberRecipe(
                craftingPair,
                result,
