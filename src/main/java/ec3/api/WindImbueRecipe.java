...
public class WindImbueRecipe implements IRecipe {
    ...
    public static WindImbueRecipe findRecipeByComponent(ItemStack c) {
        ...
        for (int i = 0; i < recipes.size(); ++i) {
            if (c.isItemEqual(recipes.get(i).transforming)) return recipes.get(i);
        }
    ...
    public static WindImbueRecipe findRecipeByResult(ItemStack r) {
        ...
        for (int i = 0; i < recipes.size(); ++i) {
            if (r.isItemEqual(recipes.get(i).result)) return recipes.get(i);
        }
    ...
    public boolean matches(InventoryCrafting c, World w) {
        return c.getSizeInventory() > 0 && c.getSizeInventory() < 3
            && c.getStackInSlot(0) != null
            && c.getStackInSlot(0)
                .isItemEqual(transforming);
    }
