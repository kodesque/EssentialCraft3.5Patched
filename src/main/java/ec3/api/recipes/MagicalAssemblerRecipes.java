...
public class MagicalAssemblerRecipes {
    ...
    public static List<ShapedAssemblerRecipe> findRecipes(ItemStack result) {
        ...
        for (ShapedAssemblerRecipe rec : recipes) {
            if (rec.getRecipeOutput()
                .isItemEqual(result)) retLst.add(rec);
        }
        return retLst;
    }
    ...
    public static List<ShapedAssemblerRecipe> findUsageRecipes(ItemStack result) {
        ...
        for (ShapedAssemblerRecipe rec : recipes) {
            for (Object obj : rec.getInput()) {
                ...
                if (obj instanceof ItemStack) needToFind = (ItemStack) obj;
                if (obj instanceof Item) needToFind = new ItemStack((Item) obj);
                if (obj instanceof Block) needToFind = new ItemStack((Block) obj);
                if (obj instanceof ItemStack[]) {
                    f: for (ItemStack is : (ItemStack[]) obj) {
                        if (is.isItemEqual(result)) genLst.add(rec);
                        break f;
                    }
                }
                ...
                if (needToFind != null) {
                    if (needToFind.isItemEqual(result)) genLst.add(rec);
                }
            }
        }
        for (ShapedAssemblerRecipe rec : genLst) {
            if (!retLst.contains(rec)) retLst.add(rec);
        }
        return retLst;
    }
