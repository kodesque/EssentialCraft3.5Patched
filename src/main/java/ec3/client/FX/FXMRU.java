...
public class FXMRU extends EntityFX {
    ...
    private static final ResourceLocation ecparticleTextures = new ResourceLocation(
        "essentialcraft",
        "textures/special/particles.png");

    public FXMRU(World w, double x, double y, double z, double i, double j, double k) {
        super(w, x, y, z, i, j, k);
        this.motionX = i;
        this.motionY = j;
        this.motionZ = k;
    ...
    public void renderParticle(Tessellator var3, float par2, float par3, float par4, float par5, float par6,
        float par7) {
        ...
        if (!enabled) GL11.glDisable(GL11.GL_BLEND);
    ...
    public void onUpdate() {
        ...
        else this.setParticleTextureIndex((this.particleAge - this.particleMaxAge / 2) * 8 / (this.particleMaxAge / 2));
        this.prevPosX = this.posX;
        this.prevPosY = this.posY;
        this.prevPosZ = this.posZ;
        float f = (float) this.particleAge / (float) this.particleMaxAge;
        this.posX = (this.mruPosX + this.motionX * (double) f)
            + Math.sin(Math.toRadians(tickPos + this.worldObj.getWorldTime() * 10)) / 10;
        this.posY = (this.mruPosY + this.motionY * (double) f)
            + Math.cos(Math.toRadians(tickPos + this.worldObj.getWorldTime() * 10)) / 10;
