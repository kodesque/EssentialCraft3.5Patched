...
public class FXSpell extends EntityFX {

    private double mruPosX;
    private double mruPosY;
    private double mruPosZ;

    public FXSpell(World w, double x, double y, double z, double i, double j, double k) {
        super(w, x, y, z, i, j, k);
        this.motionX = i;
        this.motionY = j;
        this.motionZ = k;
    ...
    public void onUpdate() {
        ...
        if (this.particleAge++ >= this.particleMaxAge) {
            this.setDead();
            for (int t = 0; t < 10; ++t) {
                // this.worldObj.spawnParticle("smoke", posX, posY, posZ, MathUtils.randomFloat(rand)/6,
                // MathUtils.randomFloat(rand)/6, MathUtils.randomFloat(rand)/6);
                // if(this.worldObj.rand.nextFloat() < 0.01F)
                // this.worldObj.spawnParticle("explode", posX, posY, posZ, MathUtils.randomFloat(rand)/6,
                // MathUtils.randomFloat(rand)/6, MathUtils.randomFloat(rand)/6);
            }
        }
        // if(this.worldObj.rand.nextFloat() < 0.01F)
