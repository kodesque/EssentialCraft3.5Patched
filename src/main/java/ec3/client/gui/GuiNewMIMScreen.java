...
import ec3.dummycore.client.GuiElement;
...
public class GuiNewMIMScreen extends GuiContainer {
    ...
    int[] btnActions = new int[] { 1, -1, 2, -2, 4, -4, 5, -5, 8, -8, 10, -10, 16, -16, 32, -32, 50, -50, 64, -64, 100,
        -100, 128, -128, Integer.MAX_VALUE, -Integer.MAX_VALUE };
    ...
    public void updateScreen() {
        if (lastPressedTime > 0) --lastPressedTime;
        else secondPress = false;
        super.updateScreen();
    }
    ...
    public static class GuiRequestButton extends GuiButton {
        ...
        public void drawButton(Minecraft p_146112_1_, int p_146112_2_, int p_146112_3_) {
            if (this.visible) {
                ...
                boolean unicode = Minecraft.getMinecraft().gameSettings.forceUnicodeFont
                    || Minecraft.getMinecraft().fontRenderer.getUnicodeFlag();

                if (!unicode) GL11.glScalef(0.5F, 0.5F, 0.5F);
                else GL11.glScalef(0.95F, 0.95F, 0.95F);

                this.drawCenteredString(
                    fontrenderer,
                    this.displayString,
                    !unicode ? (this.xPosition + this.width / 2) * 2
                        : MathHelper.floor_float((this.xPosition + this.width / 2) / 0.95F),
                    !unicode ? (this.yPosition + (this.height - 8) / 2) * 2 + 4
                        : MathHelper.floor_float((this.yPosition + (this.height - 8) / 2) / 0.95F),
                    l);
                if (!unicode) GL11.glScalef(2, 2, 2);
                else GL11.glScalef(1F / 0.95F, 1F / 0.95F, 1F / 0.95F);
            }
        }
        ...
        for (int i = 0; i < btnActions.length; ++i) {
            ...
            if (i < 8) {
                GuiButton btn = new GuiButton(
                    i,
                    neg ? k - i / 2 * 16 + 103 - 32 : k + i / 2 * 16 + 103 + 64,
                    l + 154,
                    16,
                    16,
                    neg ? String.valueOf(btnActions[i]) : "+" + btnActions[i]);
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mX, int mY) {
        ...
        this.drawTexturedModalRect(
            k + 229,
            l + 26 + (maxScrollIndex == 0 ? 0 : MathUtils.pixelatedTextureSize(scrollIndex, maxScrollIndex, 113)),
            12,
            0,
            10,
            11);

        if (!Mouse.isButtonDown(0) && isLeftMouseButtonPressed) isLeftMouseButtonPressed = false;

        if (screen != null && screen.parent != null) {
            ArrayList<ItemStack> drawnItems;

            if (search.getText() != null && !search.getText()
                .isEmpty()) drawnItems = screen.parent.getItemsByName(search.getText());
            else drawnItems = screen.parent.getAllItems();

            ArrayList<CraftingPattern> crafts;

            if (search.getText() != null && !search.getText()
                .isEmpty()) crafts = screen.parent.getCraftsByName(search.getText());
            else crafts = screen.parent.getAllCrafts();
            ...
            if (selectedStack != null) {
                ...
                if (recipeSelected > -1) {
                    for (ItemStack is : craftsByItemStack.keySet()) {
                        if (is != null && is.isItemEqual(selectedStack)
                            && ItemStack.areItemStackTagsEqual(is, selectedStack)
                            && is.stackSize == selectedStack.stackSize) {
                            hasStack = true;
                            break;
                        }
                    }
            ...
            if (dWheel > 0) scrollIndex = Math.max(0, --scrollIndex);

            if (dWheel < 0) scrollIndex = Math.min(maxScrollIndex, ++scrollIndex);

            if (scrollIndex > maxScrollIndex) scrollIndex = 0;

            for (int i = scrollIndex * 10; i < drawnItems.size() && i < 60 + scrollIndex * 10; ++i) {
                int x = k + 32 + i % 10 * 19;
                int y = l + 28 + i / 10 * 19 - scrollIndex * 19;
                if (selectedStack != null) {
                    if (drawnItems.get(i) != null && drawnItems.get(i)
                        .isItemEqual(selectedStack)
                        && ItemStack.areItemStackTagsEqual(drawnItems.get(i), selectedStack)
                        && ((recipeSelected == -1 && i < regSize) || (recipeSelected > -1 && i == recipeSelected))) {
                        x -= 2;
                        y -= 2;
                ...
                GuiResearchBook.itemRender.renderItemAndEffectIntoGUI(
                    fontRendererObj,
                ...
                if (alterSize >= 1000000) alterSize /= 1000000;
                if (alterSize >= 1000) alterSize /= 1000;
                String s = greaterThan1M ? alterSize + "kk" : greaterThan1K ? alterSize + "k" : alterSize + "";
                if (i >= regSize) s = "Craft";
                if (i >= regSize) {
                    boolean unicode = Minecraft.getMinecraft().gameSettings.forceUnicodeFont
                        || Minecraft.getMinecraft().fontRenderer.getUnicodeFlag();
                    if (!unicode) {
                ...
                if (mX >= x && mX <= x + 16) {
                    if (mY >= y && mY <= y + 16) {
                        ...
                        if (i >= regSize) {
                            ...
                            int j2 = mX + 12;
                            int k2 = mY + drawnItems.get(i)
                                .getTooltip(player, false)
                                .size() * 10 - 8;
                            int i1 = 50;
                            int sX = 50;
                                ...
                                for (int j = 0; j < this.craftsByItemStack.get(drawnItems.get(i)).input.length; ++j) {
                                    ...
                                    if (this.craftsByItemStack.get(drawnItems.get(i)).input[j] != null) {
                                        ...
                                        GuiResearchBook.itemRender.renderItemAndEffectIntoGUI(
                                            fontRendererObj,
                                            Minecraft.getMinecraft().renderEngine,
                                            this.craftsByItemStack.get(drawnItems.get(i)).input[j],
                                            mX + j / 3 * 17 + 12,
                                            mY + j % 3 * 17 + 3);
                                        GL11.glTranslated(0, 0, -300);
                        ...
                        if (Mouse.isButtonDown(0) && !isLeftMouseButtonPressed) {
                            ...
                            if (selectedStack == null) {
                                selectedStack = drawnItems.get(i)
                                    .copy();
                                secondPress = true;
                                lastPressedTime = 20;
                                if (i >= regSize) recipeSelected = i;
                                else recipeSelected = -1;
                            } else {
                                if (isShiftKeyDown()) {
                                    secondPress = true;
                                    lastPressedTime = 20;
    ...
    public void setupMaxInt() {
        if (selectedStack == null) {
            if (!isCorrectInteger()) this.stackSize.setText("0");

            return;
        }
        if (!isCorrectInteger()) this.stackSize.setText("1");
        else {
            int i = Integer.parseInt(this.stackSize.getText());
            if (i <= 0) this.stackSize.setText("1");
