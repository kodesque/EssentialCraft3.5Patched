...
import ec3.dummycore.utils.Notifier;
...
public class GuiResearchBook extends GuiScreen {
    ...
    public static final ResourceLocation gui = new ResourceLocation(
        "essentialcraft",
        "textures/gui/research_book_generic.png");
    ...
    public void updateScreen() {
        ...
        if (!numberString.isEmpty() && pressDelay <= 0) {
            ...
            if (buttonID > -1) {
                ...
                if (buttonID >= 0) if (buttonList.size() > buttonID) {
                    GuiButton button = GuiButton.class.cast(buttonList.get(buttonID));
                    if (button.enabled) this.actionPerformed(button);
                }
            }
            numberString = "";
    ...
    public void initGui() {
        ...
        bookTag = this.mc.thePlayer.getCurrentEquippedItem()
            .getTagCompound();
        if (currentCategory == null) initCategories();
        if (currentCategory != null && currentDiscovery == null) initDiscoveries();
        if (currentCategory != null && currentDiscovery != null) initPage();
    ...
    public void drawBackground(int p_146278_1_) {
        ...
        if (currentCategory != null && currentDiscovery != null) {
            if (currentCategory == null || currentCategory.specificBookTextures == null)
                this.mc.renderEngine.bindTexture(gui);
            else this.mc.renderEngine.bindTexture(currentCategory.specificBookTextures);
        }
    ...
    public void drawScreen(int p_73863_1_, int p_73863_2_, float p_73863_3_) {
        ...
        if (isLeftMouseKeyPressed && !Mouse.isButtonDown(0)) isLeftMouseKeyPressed = false;
        if (FMLClientHandler.instance()
            .getCurrentLanguage()
            .equalsIgnoreCase("en_gb") && !firstOpened) {
            firstOpened = true;
    ...
    public void initCategories() {
        ...
        int l = (this.height - 168) / 2;
        if (ApiCore.categories != null) for (int i = 0; i < ApiCore.categories.size(); ++i) {
            CategoryEntry cat = ApiCore.categories.get(i);
            if (cat != null) {
                GuiButtonNoSound added = new GuiButtonNoSound(
                    i,
                    k + (30 * (i / 5)) + 8,
                    l + (30 * (i % 5)) + 28,
                    20,
                ...
                if (tier >= reqTier) added.enabled = true;
    ...
    public void drawPage(int mouseX, int mouseZ) {
        ...
        for (int ik = 0; ik < this.buttonList.size(); ++ik) {
            ...
            boolean hover = mouseX >= btn.xPosition && mouseZ >= btn.yPosition
                && mouseX < btn.xPosition + btn.width
                && mouseZ < btn.yPosition + btn.height;
            int id = btn.id;
            if (id == 0) {
                if (currentCategory == null || currentCategory.specificBookTextures == null)
                    this.mc.renderEngine.bindTexture(gui);
                else this.mc.renderEngine.bindTexture(currentCategory.specificBookTextures);
                if (hover) GL11.glColor3f(1, 0.8F, 1);
            ...
            if (id == 1) {
                ...
                if (hover && btn.enabled) GL11.glColor3f(1, 0.8F, 1);
                if (!btn.enabled) GL11.glColor3f(0.3F, 0.3F, 0.3F);
        ...
        if (currentPage + 1 < pagesMax) this.drawPage_1(mouseX, mouseZ);
    }
        ...
        if (currentPage == 0) {
            ...
            if (page.pageTitle == null || page.pageTitle.isEmpty()) {
                if (currentDiscovery.name == null || currentDiscovery.name.isEmpty()) {
                    added = "\u00a7l"
                        + StatCollector.translateToLocal("ec3book.discovery_" + currentDiscovery.id + ".name");
                } else added = currentDiscovery.name;
            } else {
    ...
    public int drawMagicianTableRecipe(int mouseX, int mouseZ, int k, int l, MagicianTableRecipe toDraw) {
        this.fontRendererObj
            .drawString(StatCollector.translateToLocal("ec3.txt.magicianRecipe"), k + 24, l + 12, 0x222222);
        this.fontRendererObj.drawString(
            StatCollector.translateToLocal("MRU Required: " + toDraw.mruRequired),
            k + 26,
            l + 83,
            0x222222);
        ...
        if (toDraw.requiredItems[0] != null) this.drawIS(
            toDraw.requiredItems[0].getISToDraw(Minecraft.getMinecraft().theWorld.getWorldTime()),
            k + 26 + 18,
            l + 25 + 18,
            mouseX,
            mouseZ,
            0);
        if (toDraw.requiredItems[1] != null) this.drawIS(
            toDraw.requiredItems[1].getISToDraw(Minecraft.getMinecraft().theWorld.getWorldTime()),
            k + 26,
            l + 25,
            mouseX,
            mouseZ,
            0);
        if (toDraw.requiredItems[2] != null) this.drawIS(
            toDraw.requiredItems[2].getISToDraw(Minecraft.getMinecraft().theWorld.getWorldTime()),
            k + 26 + 36,
            l + 25,
            mouseX,
            mouseZ,
            0);
        if (toDraw.requiredItems[3] != null) this.drawIS(
            toDraw.requiredItems[3].getISToDraw(Minecraft.getMinecraft().theWorld.getWorldTime()),
            k + 26,
            l + 25 + 36,
            mouseX,
            mouseZ,
            0);
        if (toDraw.requiredItems[4] != null) this.drawIS(
            toDraw.requiredItems[4].getISToDraw(Minecraft.getMinecraft().theWorld.getWorldTime()),
            k + 26 + 36,
            l + 25 + 36,
            mouseX,
            mouseZ,
    ...
    public int drawRadiatingChamberRecipe(int mouseX, int mouseZ, int k, int l, RadiatingChamberRecipe toDraw) {
        this.fontRendererObj
            .drawString(StatCollector.translateToLocal("ec3.txt.radiatingRecipe"), k + 8, l + 12, 0x222222);
        this.fontRendererObj.drawString(
            StatCollector.translateToLocal("MRU Required: " + toDraw.mruRequired),
            k + 26,
            l + 83,
        ...
        if (upperBalance > 1.0F) addeddCF = EnumChatFormatting.RED;
        else if (upperBalance < 1.0F) addeddCF = EnumChatFormatting.BLUE;
        else addeddCF = EnumChatFormatting.AQUA;
        String balanceUpper = Float.toString(upperBalance);
        if (balanceUpper.length() > 4) balanceUpper = balanceUpper.substring(0, 4);
        this.fontRendererObj.drawString(
            StatCollector.translateToLocal(
                StatCollector.translateToLocal("ec3.txt.format.upperBalance") + addeddCF + balanceUpper),
            k + 44,
            l + 32,
            0x222222);
        ...
        if (lowerBalance > 1.0F) addeddCF = EnumChatFormatting.RED;
        else if (lowerBalance < 1.0F) addeddCF = EnumChatFormatting.BLUE;
        else addeddCF = EnumChatFormatting.AQUA;
        String balanceLower = Float.toString(lowerBalance);
        if (balanceLower.length() > 4) balanceLower = balanceLower.substring(0, 4);
        this.fontRendererObj.drawString(
            StatCollector.translateToLocal(
                StatCollector.translateToLocal("ec3.txt.format.lowerBalance") + addeddCF + balanceLower),
            k + 44,
            l + 32 + 36,
            0x222222);
        ...
        MiscUtils.bindTexture("essentialcraft", "textures/gui/mruStorage.png");
        MiscUtils.drawTexturedModalRect(k + 7, l + 20, 0, 0, 18, 72, 1);
        int percentageScaled = MathUtils
            .pixelatedTextureSize((int) (toDraw.mruRequired * toDraw.costModifier), 5000, 72);
        IIcon icon = (IIcon) EssentialCraftCore.proxy.getClientIcon("mru");
        MiscUtils.drawTexture(k + 8, l - 1 + (74 - percentageScaled) + 20, icon, 16, percentageScaled - 2, 2);
        int positionY = 8;
    ...
    public int drawStructureRecipe(int mouseX, int mouseZ, int k, int l, StructureRecipe toDraw) {
        try {
            this.fontRendererObj
                .drawString(StatCollector.translateToLocal("ec3.txt.structure"), k + 8, l + 12, 0x222222);
            l += 5;
            StructureRecipe recipe = (StructureRecipe) toDraw;
            int highestStructureBlk = 0;
            for (StructureBlock blk : recipe.structure) {
                if (blk.y > highestStructureBlk) highestStructureBlk = blk.y;
            }
            for (StructureBlock blk : recipe.structure) {
                if (!Config.renderStructuresFromAbove) this.drawSB(
                    blk,
                    k + 52 + blk.x * 12 - blk.z * 12,
                    l + 32 + highestStructureBlk * 20 - blk.y * 20 + blk.z * 12 + blk.x * 12,
                    mouseX,
                    mouseZ,
                    0);
                else this.drawSB(
                    blk,
                    k + 52 + blk.x * 12,
                    l + 32 + highestStructureBlk * 20 + blk.z * 12,
                    mouseX,
                    mouseZ,
                    0);
            }
    ...
    public int drawShapedAssemblerRecipe(int mouseX, int mouseZ, int k, int l, ShapedAssemblerRecipe toDraw) {
        this.fontRendererObj
            .drawString(StatCollector.translateToLocal("ec3.txt.assemblerRecipe"), k + 8, l + 12, 0x222222);
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (drawable instanceof ItemStack) needToDraw = (ItemStack) drawable;
            if (drawable instanceof Item) needToDraw = new ItemStack((Item) drawable);
            if (drawable instanceof Block) needToDraw = new ItemStack((Block) drawable);
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (drawable instanceof Item) needToDraw = new ItemStack((Item) drawable);
            if (drawable instanceof Block) needToDraw = new ItemStack((Block) drawable);
            if (drawable instanceof ItemStack[]) needToDraw = ((ItemStack[]) drawable)[drawingID[i]];
            if (drawable instanceof String) {
                List<ItemStack> oreStacks = OreDictionary.getOres((String) drawable);
                needToDraw = oreStacks.get(drawingID[i]);
            }
    ...
    public int drawShapedOreRecipe(int mouseX, int mouseZ, int k, int l, ShapedOreRecipe toDraw) {
        this.fontRendererObj
            .drawString(StatCollector.translateToLocal("ec3.txt.shapedRecipe"), k + 8, l + 12, 0x222222);
    ...
    public int drawShapelessOreRecipe(int mouseX, int mouseZ, int k, int l, ShapelessOreRecipe toDraw) {
        this.fontRendererObj
            .drawString(StatCollector.translateToLocal("ec3.txt.shapelessRecipe"), k + 8, l + 12, 0x222222);
        l += 5;
    ...
    public List<String> parse(String s) {
        ...
        for (int i = 0; i < s.length(); ++i) {
            ...
            // System.out.println(added+"|"+added.length()+"|"+(added.length() +
            // (maxSymbols*(cycle-1)))+"|"+s.length()+"|"+i);
            if (added.length() > maxSymbols || added.length() + (maxSymbols * (cycle - 1)) == s.length()
                || i == s.length() - 1) {
    ...
    public void drawDiscoveries(int mouseX, int mouseZ) {
        ...
        this.fontRendererObj.drawStringWithShadow(
            "\u00a76 \u00a7l" + StatCollector.translateToLocal("ec3.txt.book.startup"),
            k + 6,
            l + 10,
            0xffffff);
        ...
        if (cat.name != null && !cat.name.isEmpty())
            ...
            "\u00a76 " + StatCollector.translateToLocal("ec3book.category_" + currentCategory.id + ".name"),
            k + 6 + 128,
            l + 10,
            0xffffff);
        for (int ik = 0; ik < this.buttonList.size(); ++ik) {
            ...
            if (id > 2) {
                ...
                if (disc.isNew) GL11.glColor3f(1, 1, 0);
                if (!hover) this.drawTexturedModalRect(btn.xPosition, btn.yPosition, 0, 222, 20, 20);
                else this.drawTexturedModalRect(btn.xPosition, btn.yPosition, 28, 222, 20, 20);
                GL11.glColor3f(1, 1, 1);
        ...
        for (int ik = 0; ik < this.buttonList.size(); ++ik) {
            ...
            if (id > 2) {
                if (hover) {
                    ...
                    if (disc.name == null || disc.name.isEmpty()) discStr
                        .add("\u00a7l" + StatCollector.translateToLocal("ec3book.discovery_" + disc.id + ".name"));
                    else discStr.add(disc.name);
                    if (disc.shortDescription == null || disc.shortDescription.isEmpty()) discStr
                        .add("\u00a7o" + StatCollector.translateToLocal("ec3book.discovery_" + disc.id + ".desc"));
                    else discStr.add(disc.shortDescription);
                    if (disc.isNew) discStr.add("\u00a76" + "New");
                    discStr.add(
                        StatCollector.translateToLocal("ec3.txt.contains") + disc.pages.size()
                            + StatCollector.translateToLocal("ec3.txt.pages"));
    ...
    public void drawCategories(int mouseX, int mouseZ) {
        ...
        this.fontRendererObj.drawStringWithShadow(
            ...
            0xffffff);
        this.fontRendererObj.drawStringWithShadow(
            "\u00a76 \u00a7l" + StatCollector.translateToLocal("ec3.txt.book.categories"),
            k + 134,
            l + 10,
            0xffffff);
        this.fontRendererObj.drawStringWithShadow(
            "\u00a76" + StatCollector.translateToLocal("ec3.txt.book.containedKnowledge"),
            k + 16,
            l + 25,
            0xffffff);
        int tier = bookTag.getInteger("tier");
        for (int i = 0; i <= tier; ++i) {
            this.fontRendererObj.drawStringWithShadow(
                "\u00a77-\u00a7o" + StatCollector.translateToLocal("ec3.txt.book.tier_" + i),
                k + 16,
                l + 35 + (i * 10),
                0xffffff);
        }
        this.fontRendererObj.drawStringWithShadow(
            "\u00a76" + StatCollector.translateToLocal("ec3.txt.book.edition"),
            k + 16,
            l + 90,
            0xffffff);
        this.fontRendererObj.drawString(
            "\u00a72" + FMLCommonHandler.instance()
                .findContainerFor(EssentialCraftCore.core)
                .getDisplayVersion() + "r",
            k + 16,
            l + 100,
            0xffffff);
        ...
        for (int ik = 0; ik < this.buttonList.size(); ++ik) {
            ...
            if (tier >= reqTier) {
                if (hover) {
                    ...
                    else catStr.add(cat.shortDescription);
                    catStr.add(
                        StatCollector.translateToLocal("ec3.txt.contains") + cat.discoveries.size()
                            + StatCollector.translateToLocal("ec3.txt.entries"));
                    this.func_146283_a(catStr, mouseX, mouseZ);
                }
            }
        }
    ...
    protected void actionPerformed(GuiButton b) {
        if (ticksBeforePressing > 0) return;
    ...
    protected void renderToolTip(ItemStack p_146285_1_, int p_146285_2_, int p_146285_3_) {
        ...
        FontRenderer font = p_146285_1_.getItem()
            .getFontRenderer(p_146285_1_);
