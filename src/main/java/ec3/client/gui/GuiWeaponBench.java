...
public class GuiWeaponBench extends GuiCommon {
    ...
    protected void actionPerformed(GuiButton par1GuiButton) {
        if (par1GuiButton.id == 1) {
            ...
            ++w.index;
            if (w.index > 3) w.index = 0;

            w.getWorldObj()
                .setBlockMetadataWithNotify(w.xCoord, w.yCoord, w.zCoord, w.index, 4);
    ...
    protected void drawGuiContainerBackgroundLayer(float f1, int i1, int i2) {
        ...
        if (w.index == 1) t = "item.ec3.gun.rifle.name";
        if (w.index == 2) t = "item.ec3.gun.sniper.name";
        if (w.index == 3) t = "item.ec3.gun.gatling.name";

        if (w.index == 0) this.mc.renderEngine.bindTexture(guiGenLocation_0);
        if (w.index == 1) this.mc.renderEngine.bindTexture(guiGenLocation_1);
        if (w.index == 2) this.mc.renderEngine.bindTexture(guiGenLocation_2);
        if (w.index == 3) this.mc.renderEngine.bindTexture(guiGenLocation_3);
