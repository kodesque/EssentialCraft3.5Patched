...
public class GuiPlayerPentacle extends GuiScreen {
    ...
    public static class EffectButton extends GuiButton {
        ...
        public EffectButton(int id, int x, int y, int sX, int sY, String name, int index) {
            ...
            if (effects.size() <= listIndex) this.enabled = false;
            else {
                if (effects.get(listIndex) == null) this.enabled = false;
            }
        }
        ...
        public void drawButton(Minecraft mc, int mX, int mY) {
            ...
            if (effects.size() <= listIndex) this.enabled = false;
                ...
                if (effects.get(listIndex) == null) this.enabled = false;
                    ...
                    ResourceLocation loc = effects.get(listIndex)
                        .getEffectIcon();
                    if (gui.pentacle.tier < effect.getType()
                        .ordinal()) {
                    ...
                    if (mX >= this.xPosition && mX <= this.xPosition + 20) {
                        if (mY >= this.yPosition && mY <= this.yPosition + 20) {
                            ...
                            if (desc.length() / 2 >= name.length()) length = desc.length() / 2;
                            ...
                            this.drawString(
                                renderer,
                                EnumChatFormatting.ITALIC + desc,
                                (mX + 5) * 2,
                                (mY + 15) * 2,
                                0xffffff);
                            this.drawString(
                                ...
                                EnumChatFormatting.ITALIC + ""
                                    + (enable ? EnumChatFormatting.DARK_GREEN : EnumChatFormatting.RED)
                                    + ""
                                    + effect.getStickiness()
                                    + " ESPE"
                                    + additional,
                                (mX + 5) * 2,
                                (mY + 20) * 2,
    ...
    protected void actionPerformed(GuiButton p_146284_1_) {
        ...
        if (eb.enabled) {
            ArrayList<ICorruptionEffect> effects = (ArrayList<ICorruptionEffect>) ECUtils
                .getData(Minecraft.getMinecraft().thePlayer)
                .getEffects();
            if (effects.size() > p_146284_1_.id) {
                ICorruptionEffect effect = effects.get(eb.listIndex);
                if (effect.getType()
                    .ordinal() <= this.pentacle.tier
                    || (GuiScreen.isShiftKeyDown() && Minecraft.getMinecraft().thePlayer.capabilities.isCreativeMode)) {
                    if (GuiScreen.isShiftKeyDown() && Minecraft.getMinecraft().thePlayer.capabilities.isCreativeMode)
                        MiscUtils.handleButtonPress(
                            ...
                            this.pentacle.xCoord,
                            this.pentacle.yCoord,
                            this.pentacle.zCoord,
                            "||isCreative:true");
                    else MiscUtils.handleButtonPress(
                        eb.listIndex,
                        getClass(),
                        eb.getClass(),
                        Minecraft.getMinecraft().thePlayer,
    ...
    public void drawScreen(int p_73863_1_, int p_73863_2_, float p_73863_3_) {
        ...
        drawTexturedModalRect(
            0,
            0,
            (int) (Minecraft.getMinecraft().thePlayer.ticksExisted % 24000L),
            (int) (Minecraft.getMinecraft().thePlayer.ticksExisted % 24000L),
            res.getScaledWidth(),
            res.getScaledHeight());
        ...
        if (opacityIndex == 1) {
            ...
            if (ECUtils.getData(Minecraft.getMinecraft().thePlayer) != null) {
                ...
                while (5 - displayEnergy.length() > 0) displayEnergy = " " + displayEnergy;
                displayEnergy += " ESPE";
                this.drawString(
                    fontRendererObj,
                    displayEnergy,
        ...
        if (opacityIndex == 1) super.drawScreen(p_73863_1_, p_73863_2_, p_73863_3_);
    }
