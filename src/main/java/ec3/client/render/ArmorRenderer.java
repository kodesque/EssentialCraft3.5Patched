...
public class ArmorRenderer implements IItemRenderer {
    ...
    public boolean handleRenderType(ItemStack item, ItemRenderType type) {
        if (item.getItem() instanceof ItemArmorMod) return true;
        return false;
    }
    ...
    public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
        if (item.getItem() instanceof ItemArmorMod) {
            ...
            if (index != -1) {
                ...
                float f5 = 0.0625F;
                if (atype == 0) GL11.glTranslatef(0, 1.2F, 0);
                if (atype == 1) GL11.glTranslatef(0, 2F, 0);
                if (atype == 2) GL11.glTranslatef(0, 2.7F, 0);
                if (atype == 3) GL11.glTranslatef(0, 2.7F, 0);
