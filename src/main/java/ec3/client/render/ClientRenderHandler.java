...
public class ClientRenderHandler {
    ...
    public void onClientRenderTick(RenderGameOverlayEvent.Pre event) {
        if (event.type != ElementType.ALL) {
            ...
            if (p.getCurrentEquippedItem() != null && p.getCurrentEquippedItem()
                ...
                if (event.type == ElementType.CROSSHAIRS) {
                    ...
                    tessellator.addVertexWithUV(
                        scaledresolution.getScaledWidth() / 2 - scaledresolution.getScaledWidth() / 4,
                        (double) l,
                        -90.0D,
                        0.0D,
                        1.0D);
                    tessellator.addVertexWithUV(
                        (double) k + scaledresolution.getScaledWidth() / 2 - scaledresolution.getScaledWidth() / 4,
                        (double) l,
                        -90.0D,
                        1.0D,
                        1.0D);
                    ...
                    Minecraft.getMinecraft()
                        .getTextureManager()
                        .bindTexture(RenderHandlerEC3.whitebox);
