...
public class RenderMagicalDisplay extends TileEntitySpecialRenderer {
    ...
    public void doRender(TileEntity p_76986_1_, double p_76986_2_, double p_76986_4_, double p_76986_6_,
        ...
        int metadata = display.getWorldObj()
            .getBlockMetadata(display.xCoord, display.yCoord, display.zCoord);
        ItemStack displayed = display.getStackInSlot(0);
        float rotationX = metadata == 3 ? 0 : metadata == 2 ? 180 : metadata == 4 ? 90 : 270;
        float offsetX = metadata == 1 || metadata == 0 ? 0.7F
            : metadata == 3 || metadata == 2 ? 0.5F : metadata == 4 ? 0.96F : 0.04F;
        float offsetY = metadata == 1 ? 0.04F : metadata == 0 ? 0.96F : 0.4F;
        float offsetZ = metadata == 1 || metadata == 0 ? 0.5F
            : metadata == 4 || metadata == 5 ? 0.5F : metadata == 2 ? 0.96F : 0.04F;
            ...
            if (!(displayed.getItem() instanceof ItemBlock)) {
                ...
                if (display.type == 0 || metadata == 0 || metadata == 1) {
                    MiscUtils.renderItemStack_Full(
                        ...
                        display.yCoord,
                        display.zCoord,
                        p_76986_2_ / scaleIndex,
                        p_76986_4_ / scaleIndex,
                        p_76986_6_ / scaleIndex,
                        rotationX,
                        rotationZ,
                        1,
                        1,
                ...
                if (display.type == 2 && metadata != 0 && metadata != 1) {
                    ...
                    displayed.getItem()
                        .addInformation(displayed, Minecraft.getMinecraft().thePlayer, displaySt, false);

                    int longestStr = 1;

                    for (int i = 0; i < displaySt.size(); ++i) {
                        String st = displaySt.get(i);
                        if (longestStr < st.length()) longestStr = st.length();
                    }
                    GL11.glPushMatrix();
