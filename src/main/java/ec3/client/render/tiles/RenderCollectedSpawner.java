...
public class RenderCollectedSpawner implements IItemRenderer {
    ...
    public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
        if (item == null || item.getItem() == null || !(item.getItem() instanceof ItemCollectedMonsterSpawner)) return;

        RenderBlocks rb = (RenderBlocks) data[0];

        if (type == ItemRenderType.EQUIPPED_FIRST_PERSON) GL11.glTranslated(0D, 0.75D, 0.5D);
        ...
        if (tag.hasKey("monsterSpawner")) {
            ...
            try {
                ...
                float scale = 0.4375F;
                if (e.getShadowSize() > 1.5) scale = 0.1F;
