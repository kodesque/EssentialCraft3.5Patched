...
public class RenderMagicalMirror extends TileEntitySpecialRenderer {
    ...
    public static final IModelCustom model = AdvancedModelLoader
        .loadModel(new ResourceLocation("essentialcraft:textures/special/models/assembler/Mirror.obj"));
    ...
    public void doRender(TileEntity p_76986_1_, double p_76986_2_, double p_76986_4_, double p_76986_6_,
        ...
        float yIndex = 1.0F;
        if (timeIndex <= 60) yIndex = timeIndex / 240F;
        else yIndex = 0.5F - timeIndex / 240F;
        ...
        if (tile.transferingStack != null) {
            if (tile.transferTime < 20) {
                MiscUtils.renderItemStack_Full(
                    ...
                    1,
                    0.5F,
                    -0.3F + ((float) tile.transferTime / 20F),
                    0.5F);
            } else {
                Vec3 vec = Vec3.createVectorHelper(
                    tile.inventoryPos.x + 0.5F - (tile.xCoord + 0.5F),
                    tile.inventoryPos.y - (tile.yCoord + 0.5F),
                    tile.inventoryPos.z - (tile.zCoord + 0.5F));
                ...
                MiscUtils.renderItemStack_Full(
                    ...
                    1,
                    1,
                    0.5F + ((float) vec.xCoord * (tile.transferTime - 20F) / 40),
                    0.5F + ((float) vec.yCoord * (tile.transferTime - 20F) / 40),
                    0.5F + ((float) vec.zCoord * (tile.transferTime - 20F) / 40));
            }
        }
    }
