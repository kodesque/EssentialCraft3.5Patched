...
public class RenderMonsterHolder extends TileEntitySpecialRenderer {
    ...
    public static final ResourceLocation textures = new ResourceLocation(
        "essentialcraft:textures/special/models/magicianTable.png");
    ...
    public void doRender(TileEntity p_76986_1_, double p_76986_2_, double p_76986_4_, double p_76986_6_,
        ...
        if (p_76986_1_ instanceof TileMonsterHolder) {
            ...
            List<EntityLivingBase> lst = tile.getWorldObj()
                ...
                for (int i = 0; i < lst.size(); ++i) {
                    ...
                    if (!(e instanceof EntityPlayer)) {
                        ...
                        if (dist.getDistance() < 10) {
                            GL11.glPushMatrix();
                            double[] o = new double[] { e.posX - 0.5D, e.posY + e.getEyeHeight() + 0.5D,
                                e.posZ - 0.5D };
                            ...
                            float f10 = MathHelper.sqrt_float(f4 * f4 + f5 * f5 + f6 * f6) / 32.0F
                                - (PlayerTickHandler.tickAmount + p_76986_9_) * 0.1F;
                            tessellator.startDrawing(5);
                            byte b0 = 8;

                            for (int i1 = 0; i1 <= b0; ++i1) {
                                float f11 = MathHelper.sin((float) (i1 % b0) * (float) Math.PI * 2.0F / (float) b0)
                                    * 0.75F
                                    * 0.1F;
                                float f12 = MathHelper.cos((float) (i1 % b0) * (float) Math.PI * 2.0F / (float) b0)
                                    * 0.75F
                                    * 0.1F;
                                float f13 = (float) (i1 % b0) * 1.0F / (float) b0;
                                tessellator.setColorRGBA_F(0.0F, 1.0F, 1.0F, 10F);
                                tessellator
                                    .addVertexWithUV((double) (f11), (double) (f12), 0.0D, (double) f13, (double) f10);
