...
public class BlockRedstoneDeviceNotSided extends BlockContainer {
    ...
    public static final String[] names = new String[] { "replanter", "itemShuffler", "crafter", "breeder",
        "creativeMRUStorage", "shearingStation", "childSeparator", "adultSeparator" };
    ...
    public void registerBlockIcons(IIconRegister reg) {
        for (int i = 0; i < names.length; ++i) icons[i] = reg.registerIcon("essentialcraft:" + names[i]);
    }
    ...
    public void shear(Entity en, IShearable e) {
        if (e.isShearable(new ItemStack(Items.shears), en.worldObj, (int) en.posX, (int) en.posY, (int) en.posZ)) {
            ArrayList<ItemStack> items = e
                .onSheared(new ItemStack(Items.shears), en.worldObj, (int) en.posX, (int) en.posY, (int) en.posZ, 2);
            for (ItemStack is : items) {
                if (is != null) {
                    EntityItem itm = new EntityItem(en.worldObj, en.posX, en.posY, en.posZ, is);
                    if (!en.worldObj.isRemote) en.worldObj.spawnEntityInWorld(itm);
                }
            }
        }
    }
    ...
    public void shuffle(EntityItem e) {
        if (!e.worldObj.isRemote) {
            e.setPositionAndRotation(
                e.posX + MathUtils.randomDouble(e.worldObj.rand),
                e.posY,
                e.posZ + MathUtils.randomDouble(e.worldObj.rand),
                0,
                0);
        }
    }
        ...
        if (e.getEntityItem() != null && !e.worldObj.isRemote) {
            ...
            if (stk.getItem() instanceof ItemBlock) {
                ...
                if (b instanceof IPlantable) {
                    if (e.worldObj.isAirBlock(x, y, z) && e.worldObj.getBlock(x, y - 1, z)
                        .canSustainPlant(e.worldObj, x, y, z, ForgeDirection.UP, IPlantable.class.cast(b))) {
                        FakePlayer user = new FakePlayer((WorldServer) e.worldObj, planterFakePlayerProfile);

                        stk.getItem()
                            .onItemUse(stk, user, e.worldObj, x, y - 1, z, ForgeDirection.UP.ordinal(), 0, 0, 0);
                ...
                if (stk.getItem() instanceof IPlantable) {
                    if (e.worldObj.isAirBlock(x, y, z) && e.worldObj.getBlock(x, y - 1, z)
                        .canSustainPlant(
                            ...
                            z,
                            ForgeDirection.UP,
                            IPlantable.class.cast(stk.getItem()))) {
                        FakePlayer user = new FakePlayer((WorldServer) e.worldObj, planterFakePlayerProfile);
    ...
    public void onNeighborBlockChange(World w, int x, int y, int z, Block n) {
        ...
        if (w.getBlockMetadata(x, y, z) == 6 || w.getBlockMetadata(x, y, z) == 7) {
            TileAnimalSeparator.class.cast(w.getTileEntity(x, y, z))
                .separate(w.getBlockMetadata(x, y, z) == 6);
        }
    }

    public static GameProfile planterFakePlayerProfile = new GameProfile(
        UUID.fromString("5cd89d0b-e9ba-0000-89f4-badbb05964ac"),
        "[EC3]Planter");
    public static GameProfile breederFakePlayerProfile = new GameProfile(
        UUID.fromString("5cd89d0b-e9ba-0000-89f4-badbb05964ad"),
        "[EC3]Breeder");

    @Override
    public TileEntity createNewTileEntity(World p_149915_1_, int meta) {
        return meta == 2 ? new TileCrafter()
            : meta == 4 ? new TileCreativeMRUSource() : meta == 6 || meta == 7 ? new TileAnimalSeparator() : null;
    }

    @Override
    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer,
        int par6, float par7, float par8, float par9) {
        if (par1World.getTileEntity(par2, par3, par4) == null) return false;
