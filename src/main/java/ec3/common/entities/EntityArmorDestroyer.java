...
public class EntityArmorDestroyer extends EntitySnowball {
    ...
    protected void onImpact(MovingObjectPosition p_70184_1_) {
        if (p_70184_1_.entityHit != null) {
            if (p_70184_1_.entityHit == this.getThrower()) return;

            if (!(p_70184_1_.entityHit instanceof EntityPlayer)) return;

            EntityPlayer p = (EntityPlayer) p_70184_1_.entityHit;

            if (p != null) {
                ItemStack c = p.getCurrentEquippedItem();
                if (c != null) c.damageItem(5, p);

                for (int j = 0; j < 4; ++j) {
                    ItemStack a = p.inventory.armorInventory[j];

                    if (a != null) if (a.getItem() instanceof ISpecialArmor) ISpecialArmor.class.cast(a.getItem())
                        .damageArmor(p, a, DamageSource.anvil, 5, j);
                    else a.damageItem(5, p);
                }
            }
        }
