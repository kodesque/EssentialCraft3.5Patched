...
public class EntityHologram extends EntityLiving implements IBossDisplayData {
    ...
    protected void dropFewItems(boolean playerkill, int fortune) {
        ...
        if (!this.worldObj.isRemote) {
            for (int i = 0; i < this.players.size(); ++i) {
                EntityPlayer p = MinecraftServer.getServer()
                    ...
                    .func_152612_a(this.players.get(i));
                if (p != null) p.triggerAchievement(ECAchievements.achievementList.get("hologram"));

                boolean addBig = true;

                for (int j = 0; j < 4; ++j) {
                    if (p.inventory.armorInventory[j] != null) addBig = false;
                }

                if (addBig) p.triggerAchievement(ECAchievements.achievementList.get("hologramBig"));
            }
        }
        ...
        if (prevAttackID == -1) prevAttackID = this.worldObj.rand.nextInt(4);

        if (prevAttackID == 0) meta = 70;

        if (prevAttackID == 1) meta = 73;

        if (prevAttackID == 2) meta = 72;

        if (prevAttackID == 3) meta = 71;

        this.entityDropItem(new ItemStack(ECItems.genericItem, 1, meta), 0);

        if (this.worldObj.rand.nextDouble() < 0.1D) this.entityDropItem(new ItemStack(ECItems.orbitalRemote, 1, 0), 0);

        if (this.worldObj.rand.nextDouble() < 0.1D) this.entityDropItem(new ItemStack(ECItems.dividingGun, 1, 0), 0);

        if (this.worldObj.rand.nextDouble() < 0.3D)
            this.entityDropItem(new ItemStack(ECItems.record_robocalypse, 1, 0), 0);
        ...
        if (chest != null) {
            WeightedRandomChestContent
                .generateChestContents(w.rand, WorldGenOldCatacombs.generatedItems, chest, w.rand.nextInt(26) + 18);
    ...
    public void dwWrite() {
        if (!this.worldObj.isRemote) this.getDataWatcher()
            .updateObject(12, "||aID:" + attackID + "||aTi:" + attackTimer + "||rTi:" + restingTime);
    }

    public void dwRead() {
        if (this.worldObj.isRemote) {
            String str = this.getDataWatcher()
                .getWatchableObjectString(12);
    ...
    public void onUpdate() {
        ...
        // EssentialCraftCore.proxy.stopSound("hologram");
        if (this.deathTime == 0) EssentialCraftCore.proxy.startSound("hologram", "essentialcraft:records.hologram");
        else EssentialCraftCore.proxy.stopSound("hologram");

        dwWrite();

        if (this.motionY < 0.002) this.motionY = 0.002;
        ...
        if (!this.getActivePotionEffects()
            .isEmpty()) this.clearActivePotions();
        ...
        if (!this.worldObj.isRemote) {
            if (restingTime == 0 && attackID == -1) {
                ...
                ECUtils.playSoundToAllNearby(
                    ...
                    posY,
                    posZ,
                    "essentialcraft:sound.mob.hologram.stop",
                    5,
                    2F,
                    16,
                    this.dimension);
            ...
            if (attackTimer != 0 && attackID != -1) {
                ...
                if (attackID == 0) {
                    if (attackTimer == 20) {
                        ...
                        for (int i = 0; i < players.size(); ++i) {
                            EntityPlayer p = MinecraftServer.getServer()
                                .getConfigurationManager()
                                .func_152612_a(players.get(i));
                            if (p != null) {
                                for (int j = 0; j < 1 + hMax; ++j) {
                                    EntityPlayerClone clone = new EntityPlayerClone(p.worldObj);
                                    clone.setPositionAndRotation(
                                        p.posX + MathUtils.randomDouble(rand) * 6,
                                        p.posY,
                                        p.posZ + MathUtils.randomDouble(rand) * 6,
                                        p.rotationYaw,
                                        p.rotationPitch);
                                    clone.setCurrentItemOrArmor(
                                        0,
                                        p.getCurrentEquippedItem() != null ? p.getCurrentEquippedItem()
                                            .copy() : null);

                                    clone.setCurrentItemOrArmor(
                                        1,
                                        p.inventory.armorInventory[3] != null ? p.inventory.armorInventory[3].copy()
                                            : null);
                                    clone.setCurrentItemOrArmor(
                                        2,
                                        p.inventory.armorInventory[2] != null ? p.inventory.armorInventory[2].copy()
                                            : null);
                                    clone.setCurrentItemOrArmor(
                                        3,
                                        p.inventory.armorInventory[1] != null ? p.inventory.armorInventory[1].copy()
                                            : null);
                                    clone.setCurrentItemOrArmor(
                                        4,
                                        p.inventory.armorInventory[0] != null ? p.inventory.armorInventory[0].copy()
                                            : null);
                    ...
                    if (this.attackTimer % 10 == 0) {
                        if (this.players.size() > 0) {
                            ...
                            EntityPlayer p = MinecraftServer.getServer()
                                ...
                                .func_152612_a(players.get(i));
                            if (p != null) {
                                EntityOrbitalStrike strike = new EntityOrbitalStrike(
                                    worldObj,
                                    p.posX,
                                    p.posY,
                                    p.posZ,
                                    damage,
                                    3 - (2 - this.getHealth() / this.getMaxHealth() * 2),
                                    this);
                ...
                if (attackID == 3) {
                    ...
                    if (this.attackTimer % 20 == 0) {
                        for (int i = 0; i
                            < 1 + (5 - MathHelper.floor_double(this.getHealth() / this.getMaxHealth() * 5)); ++i)
                            if (this.players.size() > 0) {
                                int i1 = this.worldObj.rand.nextInt(this.players.size());
                                EntityPlayer p = MinecraftServer.getServer()
                                    .getConfigurationManager()
                                    .func_152612_a(players.get(i1));
                                if (p != null) {
                                    EntityDivider d = new EntityDivider(
                                        worldObj,
                                        p.posX,
                                        p.posY,
                                        p.posZ,
            ...
            if (restingTime > 0) --restingTime;
        } else {
            EntityPlayer p = EssentialCraftCore.proxy.getClientPlayer();
            if (p != null && p.capabilities.isFlying
                && p.getDistanceToEntity(this) <= RANGE
        ...
        if (!this.worldObj.isRemote && this.ticksExisted % 10 == 0) {
            ...
            for (int i = 0; i < players.size(); ++i) {
                EntityPlayer p = MinecraftServer.getServer()
                    .getConfigurationManager()
                    .func_152612_a(players.get(i));
                if (p == null || p.isDead) players.remove(i);
            }
            for (int i = 0; i < manager.getCurrentPlayerCount(); ++i) {
                ...
                if (this.players.contains(player.getCommandSenderName())) {
                    ...
                    if (this.dimension != player.dimension) manager.transferPlayerToDimension(player, this.dimension);
                    ...
                    if (distance > RANGE) {
                        ...
                        ECUtils
                            .playSoundToAllNearby(posX, posY, posZ, "random.anvil_break", 1, 0.01F, 8, this.dimension);
                    }

                    if (player.capabilities.isFlying) player.capabilities.isFlying = false;

                } else {
                    if (this.dimension != player.dimension) continue;
    ...
    public void writeEntityToNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < players.size(); ++i) tag.setString("player_" + i, players.get(i));
    }
    ...
    public void readEntityFromNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < tag.getInteger("listSize"); ++i) players.add(tag.getString("player_" + i));
    }
    ...
    public boolean attackEntityFrom(DamageSource src, float f) {
        if (this.attackID != -1) return false;

        if (src == null) return false;

        if (src.getSourceOfDamage() == null) return false;

        if (!(src.getSourceOfDamage() instanceof EntityPlayer)) return false;

        if (src.getSourceOfDamage() instanceof FakePlayer) return false;

        damage += f;

        if (f > 40 || damage > 40) this.restingTime = 1;

        if (src.isProjectile()) f /= 4;

        if (src.isProjectile()) ECUtils.playSoundToAllNearby(
            posX,
            posY,
            posZ,
            "essentialcraft:sound.mob.hologram.damage.projectile",
            5,
            this.worldObj.rand.nextFloat() * 2,
            16,
            this.dimension);
        else ECUtils.playSoundToAllNearby(
            posX,
            posY,
            posZ,
            "essentialcraft:sound.mob.hologram.damage.melee",
            0.3F,
            this.worldObj.rand.nextFloat() * 2,
            16,
            this.dimension);
