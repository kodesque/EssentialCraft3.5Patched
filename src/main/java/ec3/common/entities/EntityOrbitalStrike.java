...
public class EntityOrbitalStrike extends Entity {
    ...
    public EntityOrbitalStrike(World w, double x, double y, double z, double damage, double delay,
    ...
    public void onUpdate() {
        ...
        if (!this.worldObj.isRemote) this.getDataWatcher()
            .updateObject(12, String.valueOf(delay));

        if (this.ticksExisted == 3) ECUtils
            .playSoundToAllNearby(posX, posY, posZ, "essentialcraft:sound.orbital_strike", 1, 1F, 16, this.dimension);

        if (delay <= 0 && !this.isDead) {
            List<EntityLivingBase> allEntities = this.worldObj.getEntitiesWithinAABB(
            ...
            for (int i = 0; i < 20; ++i) this.worldObj.spawnParticle(
                "hugeexplosion",
                posX + MathUtils.randomDouble(rand),
                posY + MathUtils.randomDouble(rand),
                posZ + MathUtils.randomDouble(rand),
                0,
                0,
                0);

            if (this.worldObj.getGameRules()
                .getGameRuleBooleanValue("mobGriefing")) for (int dx = -2; dx <= 2; ++dx) {
                    ...
                    for (int dy = -2; dy <= 2; ++dy) {
                        ...
                        for (int dz = -2; dz <= 2; ++dz) {
                            ...
                            if (!b.isAir(worldObj, x, y, z)) {
                                if (b.getMaterial() == Material.water || b.getMaterial() == Material.ice
                                    || b.getMaterial() == Material.snow) {
                                    if (!this.worldObj.isRemote) this.worldObj.setBlock(x, y, z, Blocks.air, 0, 2);
