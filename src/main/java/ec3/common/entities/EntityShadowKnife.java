    ...
    protected void onImpact(MovingObjectPosition mop) {
        if (mop.entityHit != null) {
            if (mop.entityHit instanceof EntityLivingBase) {
                EntityLivingBase.class.cast(mop.entityHit)
                    .attackEntityFrom(DamageSource.causeMobDamage(getThrower()), 12);
                this.teleportRandomly(mop.entityHit);
                this.setDead();
                if (mop.entityHit instanceof EntityPlayer) {
                    ShadeUtils.attackPlayerWithShade(
                        EntityPlayer.class.cast(mop.entityHit),
                        getThrower(),
                        new ItemStack(ECItems.shadeKnife, 1, 0));
                }
            }
        }
    }
    ...
    protected boolean teleport(Entity e, double x, double y, double z) {
        ...
        if (this.worldObj.blockExists(i, j, k)) {
            ...
            if (flag1) {
                ...
                if (e.worldObj.getCollidingBoundingBoxes(e, e.boundingBox)
                    .isEmpty() && !e.worldObj.isAnyLiquid(e.boundingBox)) {
                    flag = true;
                }
            }
        }
