    ...
    public EntityDemon(World w) {
        ...
        if (w.rand.nextBoolean()) {
            desiredItem = new ItemStack(
                ItemsCore.soul,
                1 + w.rand.nextInt(7),
                w.rand.nextInt(DemonTrade.allMobs.size()));
        } else {
            desiredItem = DemonTrade.trades.get(
                DemonTrade.allMobs.size()
                    + w.rand.nextInt(DemonTrade.trades.size() - DemonTrade.allMobs.size())).desiredItem;
        }
    }
