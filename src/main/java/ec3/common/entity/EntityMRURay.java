...
public class EntityMRURay extends Entity {
    ...
    public void onEntityUpdate() {
        ...
        if (!this.worldObj.isRemote) {
            this.getDataWatcher()
                .updateObject(12, "||x:" + pX + "||y:" + pY + "||z:" + pZ + "||b:" + (double) balance);
        }

        if (this.worldObj.isRemote) {
            String dataStr = this.getDataWatcher()
    ...
    public DamageSource causeMRUDamage(final EntityLivingBase attacker, EntityLivingBase attacked) {
        if (attacked instanceof EntityPlayer) {
            ...
            if (!player.worldObj.isRemote && MinecraftServer.getServer()
                .isPVPEnabled()) {
                if (attacker instanceof EntityPlayer) {
                    EntityPlayer attackerPlayer = (EntityPlayer) attacker;
                    if (attackerPlayer.getTeam() == null || player == null
                        || !(player.getTeam()
                            .isSameTeam(attackerPlayer.getTeam())))
                        if (!this.worldObj.getGameRules()
                            .getGameRuleBooleanValue("ec3:weaponMatrixDamage"))
                            ECUtils.getData(player)
                                .modifyOverhaulDamage(
                                    ECUtils.getData(player)
                                        .getOverhaulDamage() + MathHelper.floor_double(this.damage * 100));
                } else if (!this.worldObj.getGameRules()
                    .getGameRuleBooleanValue("ec3:weaponMatrixDamage"))
                    ECUtils.getData(player)
                        .modifyOverhaulDamage(
                            ECUtils.getData(player)
