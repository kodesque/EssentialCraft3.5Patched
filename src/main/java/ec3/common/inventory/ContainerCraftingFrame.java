...
public class ContainerCraftingFrame extends Container {
    ...
    public static class SlotFake extends Slot {
        ...
        public boolean canTakeStack(EntityPlayer player) {
            ...
            for (int i = 0; i < 9; ++i) {
                if (i == 0) crafting.setInventorySlotContents(i, parent.inventory.getStackInSlot(0));
                if (i == 1) crafting.setInventorySlotContents(i, parent.inventory.getStackInSlot(3));
                if (i == 2) crafting.setInventorySlotContents(i, parent.inventory.getStackInSlot(6));
                if (i == 3) crafting.setInventorySlotContents(i, parent.inventory.getStackInSlot(1));
                if (i == 4) crafting.setInventorySlotContents(i, parent.inventory.getStackInSlot(4));
                if (i == 5) crafting.setInventorySlotContents(i, parent.inventory.getStackInSlot(7));
                if (i == 6) crafting.setInventorySlotContents(i, parent.inventory.getStackInSlot(2));
                if (i == 7) crafting.setInventorySlotContents(i, parent.inventory.getStackInSlot(5));
                if (i == 8) crafting.setInventorySlotContents(i, parent.inventory.getStackInSlot(8));
            }

            ItemStack result = CraftingManager.getInstance()
                .findMatchingRecipe(crafting, player.worldObj);

            parent.inventory.setInventorySlotContents(9, result != null ? result.copy() : null);

            crafting = null;

            if (this.slotNumber == 9 && result == null) this.inventory.setInventorySlotContents(slotNumber, null);

            if (this.slotNumber != 9) {
                this.inventory.setInventorySlotContents(slotNumber, null);

                crafting = new InventoryCrafting(this.parent, 3, 3);
                for (int i = 0; i < 9; ++i) crafting.setInventorySlotContents(i, parent.inventory.getStackInSlot(i));
        ...
        public boolean isItemValid(ItemStack stk) {
            if (stk != null) {
                if (this.slotNumber != 9) {
                    ...
                    Slot.class.cast(this.parent.inventorySlots.get(9))
                        .onSlotChanged();
                } else {
                    ...
                    if (!this.getHasStack()) {
                        ...
                        if (rec == null && !parent.player.getCurrentEquippedItem()
                            .getTagCompound()
                            .getBoolean("ignoreOreDict")) rec = ECUtils.findRecipeByIS(stk, 2);
                        ...
                        if (rec != null) {
                            for (int i = 0; i < 9; ++i) {
                                ...
                                if (rec instanceof ShapelessRecipes) {
                                    ...
                                    if (srec.recipeItems.size() > i) this.parent.inventory
                                        .setInventorySlotContents(i, (ItemStack) srec.recipeItems.get(i));
                                }
                                if (rec instanceof ShapedRecipes) {
                                    ShapedRecipes srec = (ShapedRecipes) rec;
                                    if (srec.recipeItems.length > i) this.parent.inventory
                                        .setInventorySlotContents(i, (ItemStack) srec.recipeItems[i]);
                                }
                                if (rec instanceof ShapelessOreRecipe) {
                                    ...
                                    if (srec.getInput()
                                        ...
                                        if (srec.getInput()
                                            .get(i) != null) {
                                            UnformedItemStack ust = new UnformedItemStack(
                                                srec.getInput()
                                                    .get(i));
                                            this.parent.inventory.setInventorySlotContents(
                                                i,
                                                (ItemStack) ust.possibleStacks
                                                    .get(player.worldObj.rand.nextInt(ust.possibleStacks.size())));
                                        } else this.parent.inventory.setInventorySlotContents(i, null);
                                    }
                                }
                                ...
                                Slot.class.cast(this.parent.inventorySlots.get(i))
                                    .onSlotChanged();
                            }
                        }
                    }
                    ...
                    this.inventory.setInventorySlotContents(
                        slotNumber,
                        settedRec == null || settedRec.getRecipeOutput() == null ? null
                            : settedRec.getRecipeOutput()
                                .copy());
                }
            }
            return false;
