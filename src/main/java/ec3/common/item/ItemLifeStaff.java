...
public class ItemLifeStaff extends ItemStoresMRUInNBT {
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (ECUtils.tryToDecreaseMRUInStorage(player, -100) || this.setMRU(stack, -100)) {
            if (ItemDye.applyBonemeal(
                new ItemStack(stack.getItem(), stack.getItemDamage(), stack.stackSize + 1),
                world,
                x,
                y,
                z,
    ...
    public boolean onLeftClickEntity(ItemStack stack, EntityPlayer player, Entity entity) {
        if (entity instanceof EntityZombie) {
            ...
            if (e.isVillager() && (ECUtils.tryToDecreaseMRUInStorage(player, -500) || this.setMRU(stack, -500))
                && !e.worldObj.isRemote) {
        ...
        if (entity instanceof EntityAgeable) {
            ...
            if (e.isChild() && !e.worldObj.isRemote
