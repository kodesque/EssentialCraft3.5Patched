...
public class ItemMagicalDigger extends ItemPickaxe implements IItemRequiresMRU {
    ...
    public boolean onBlockDestroyed(ItemStack par1ItemStack, World par2World, Block par3, int par4, int par5, int par6,
        ...
        if (par7EntityLivingBase instanceof EntityPlayer && !par7EntityLivingBase.isSneaking()
            && this.canBreak(par1ItemStack)) {
            this.break3x3x3Blocks(
                (EntityPlayer) par7EntityLivingBase,
                new Coord3D(par4, par5, par6),
                par1ItemStack,
                par2World.getBlock(par4, par5, par6));
        }
        return true;
    }
        ...
        for (int x = -1; x <= 1; ++x) {
            for (int y = -1; y <= 1; ++y) {
                for (int z = -1; z <= 1; ++z) {
                    ...
                    for (int v = 0; v < 10; ++v) e.worldObj.spawnParticle(
                        "reddust",
                        c.x + x + e.worldObj.rand.nextFloat(),
                        c.y + y + e.worldObj.rand.nextFloat(),
                        c.z + z + e.worldObj.rand.nextFloat(),
                        1.0D,
                        0.0D,
                        1.0D);
