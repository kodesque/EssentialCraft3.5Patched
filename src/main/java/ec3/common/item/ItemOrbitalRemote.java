...
public class ItemOrbitalRemote extends ItemStoresMRUInNBT {
    ...
    public void onUpdate(ItemStack itemStack, World world, Entity entity, int indexInInventory, boolean isCurrentItem) {
        ...
        if (entity instanceof EntityPlayer && entity.ticksExisted % 20 == 0) EntityPlayer.class.cast(entity)
            .triggerAchievement(AchievementRegistry.achievementList.get("hologramRemote"));
    }
    ...
    public ItemStack onItemRightClick(ItemStack stk, World w, EntityPlayer p) {
        ...
        double d1 = p.prevPosY + (p.posY - p.prevPosY) * (double) f
            + (double) (p.worldObj.isRemote ? p.getEyeHeight() - p.getDefaultEyeHeight() : p.getEyeHeight()); // isRemote
                                                                                                              // check
                                                                                                              // to
                                                                                                              // revert
                                                                                                              // changes
        ...
        if (mop != null && mop.typeOfHit == MovingObjectType.BLOCK) {
            if ((ECUtils.tryToDecreaseMRUInStorage(p, -10000) || this.setMRU(stk, -10000))) {
                EntityOrbitalStrike eos = new EntityOrbitalStrike(
                    w,
                    mop.blockX + 0.5D,
                    mop.blockY + 1,
                    mop.blockZ + 0.5D,
                ...
                if (!w.isRemote) w.spawnEntityInWorld(eos);
            }
        }
