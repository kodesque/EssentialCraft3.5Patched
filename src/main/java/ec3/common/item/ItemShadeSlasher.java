...
public class ItemShadeSlasher extends ItemSword_Mod {
    ...
    public IIcon getIcon(ItemStack stack, int pass) {
        return MiscUtils.getStackTag(stack)
            .getBoolean("active") ? swordIcons[0] : swordIcons[1];
    }
    ...
    public void onUpdate(ItemStack sword, World w, Entity e, int slotNum, boolean held) {
        if (e instanceof EntityLivingBase && !w.isRemote && held) EntityLivingBase.class.cast(e)
            .addPotionEffect(new PotionEffect(Potion.digSlowdown.id, 3, 3, true));
        if (e instanceof IShadeCreature) toggleActivity(sword, true);

        if (e instanceof EntityPlayer) {
            EntityPlayer p = EntityPlayer.class.cast(e);
            if (ECUtils.getData(p)
                .getMatrixTypeID() == 4) toggleActivity(sword, true);
            else toggleActivity(sword, false);
        }
    }
    ...
    public Multimap getAttributeModifiers(ItemStack stack) {
        ...
        if (MiscUtils.getStackTag(stack)
            ...
            mp.put(
                ...
                new AttributeModifier(field_111210_e, "Weapon modifier", 32, 0));
        return mp;
    }
    ...
    public boolean hitEntity(ItemStack weapon, EntityLivingBase attacked, EntityLivingBase attacker) {
        ...
        if (attacker instanceof IShadeCreature
            || (attacker instanceof EntityPlayer && ECUtils.getData(EntityPlayer.class.cast(attacker))
                .getMatrixTypeID() == 4)) {
            if (!attacker.worldObj.isRemote) {
                if (attacker.worldObj.rand.nextFloat() <= 0.6F) {
                    ...
                    attacked.addVelocity(
                        (double) (-MathHelper.sin(attacker.rotationYaw * (float) Math.PI / 180.0F) * (float) i * 0.5F),
                        0.1D,
                        (double) (MathHelper.cos(attacker.rotationYaw * (float) Math.PI / 180.0F) * (float) i * 0.5F));
                }
