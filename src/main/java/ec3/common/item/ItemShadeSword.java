        ...
        if (attacker instanceof IShadeCreature
            ...
            if (!attacker.worldObj.isRemote) {
                if (attacker.worldObj.rand.nextFloat() <= 0.33F) {
                    // swap
                    Vec3 offsetVec = Vec3.createVectorHelper(
                        attacked.posX - attacker.posX,
                        attacked.posY - attacker.posY,
                        attacked.posZ - attacker.posZ);
                    offsetVec.xCoord = -offsetVec.xCoord;
                    offsetVec.zCoord = -offsetVec.zCoord;
                    float newYaw = 0;
                    if (attacker.rotationYawHead >= 180) newYaw = attacker.rotationYawHead - 180;
                    else newYaw = attacker.rotationYawHead + 180;
                    attacker.setPositionAndRotation(
                        attacked.posX - offsetVec.xCoord,
                        attacker.posY,
                        attacked.posZ - offsetVec.zCoord,
                        newYaw,
