...
public class ItemShadowKnife extends ItemSword_Mod {
    ...
    public boolean onEntitySwing(EntityLivingBase entityLiving, ItemStack stack) {
        if (stack.stackSize >= 2 && (entityLiving instanceof IShadeCreature
            ...
            if (!entityLiving.worldObj.isRemote) entityLiving.worldObj.spawnEntityInWorld(knife);
        }
        return false;
    }
    ...
    public void onUpdate(ItemStack sword, World w, Entity e, int slotNum, boolean held) {
        if (e.ticksExisted % 20 == 0 && !w.isRemote && held) {
            sword.stackSize += 1;
            if (sword.stackSize >= 32) sword.stackSize = 32;
        }
