...
public class ItemBaublesWearable extends Item implements IBauble {
    ...
    public IIcon getIcon(ItemStack stack, int pass) {
        try {
            ...
            if (bTag.hasKey("type")) {
                ...
                if (pass == 0) return baubleGenIcon[bottomInt + (type * 6)];
                else return baubleGenIcon_Top[topInt + (type * 6)];
            }
    ...
    public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
        try {
            ...
            if (bTag.hasKey("type")) {
                ...
                int bottomInt = bTag.getInteger("b");
                int topInt = bTag.getInteger("t");
                if (renderPass == 0) return baubleGenIcon[bottomInt + (type * 6)];
                else return baubleGenIcon_Top[topInt + (type * 6)];
            }
            return baubleGenIcon[0];
        } catch (Exception e) {
    ...
    public void registerIcons(IIconRegister p_94581_1_) {
        ...
        for (int i = 0; i < 6; ++i) {
            for (int j = 0; j < 3; ++j) {
                baubleGenIcon[i + (j * 6)] = p_94581_1_
                    .registerIcon("essentialcraft:baubles/bauble_" + baubleType[j] + "_" + i);
                baubleGenIcon_Top[i + (j * 6)] = p_94581_1_
                    .registerIcon("essentialcraft:baubles/bauble_" + baubleType[j] + "_" + i + "_t");
            }
        }
    }
