...
public class ItemBoundGem extends Item {
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (stack.getTagCompound() != null && MiscUtils.getStackTag(stack)
            .hasKey("pos")) return false;

        if (world.getBlock(x, y, z) == ECBlocks.rayTower && world.getBlockMetadata(x, y, z) == 1) {
            y -= 1;
        }
        TileEntity t = world.getTileEntity(x, y, z);
        if (t != null && !world.isRemote) {
            if (t instanceof ITileRequiresMRU || t instanceof ITETransfersMRU || t instanceof ITEStoresMRU) {
                ItemStack is = createTag(stack);
                MiscUtils.getStackTag(is)
                    .setIntArray("pos", new int[] { x, y, z });
                MiscUtils.getStackTag(is)
                    .setInteger("dim", player.dimension);
                if (stack.stackSize <= 0) player.inventory.setInventorySlotContents(player.inventory.currentItem, null);

                if (!player.inventory.addItemStackToInventory(is)) player.dropPlayerItemWithRandomChoice(is, false);
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        if (par1ItemStack.getTagCompound() != null) {
            int[] coord = MiscUtils.getStackTag(par1ItemStack)
                .getIntArray("pos");
            ...
            par3List.add(
                "dimension: " + MiscUtils.getStackTag(par1ItemStack)
                    .getInteger("dim"));
        }
    }
