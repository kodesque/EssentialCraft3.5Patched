...
public class ItemElementalSword extends ItemSword implements IItemRequiresMRU /* ItemStoresMRUInNBT */ {
    ...
    public boolean hitEntity(ItemStack par1ItemStack, EntityLivingBase par2EntityLivingBase,
        ...
        if (attrib.contains("Fire")) {
            if ((ECUtils.tryToDecreaseMRUInStorage((EntityPlayer) par3EntityLivingBase, -50)
                || this.setMRU(par1ItemStack, -50))) {
        ...
        if ((ECUtils.tryToDecreaseMRUInStorage((EntityPlayer) par3EntityLivingBase, -50)
            ...
            if (embers.contains("Lightning")) {
                EntityLightningBolt bold = new EntityLightningBolt(
                    par2EntityLivingBase.worldObj,
                ...
                if (!par2EntityLivingBase.worldObj.isRemote) par2EntityLivingBase.worldObj.spawnEntityInWorld(bold);
    ...
    public Multimap getAttributeModifiers(ItemStack stack) {
        ...
        for (int i = 0; i < 11; ++i) {
            if (embers.contains("Damage +" + i)) damage += i;
        }
        multimap.put(
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        if (par1ItemStack.getTagCompound() != null && par1ItemStack.getTagCompound()
            .hasKey("ember_0")) {
            List l = getEmberEffects(par1ItemStack);
            par3List.addAll(l);
            par3List.add(
                "Primal Attribute: " + par1ItemStack.getTagCompound()
                    .getString("primary"));
            par3List.add(
                "Second Attribute: " + par1ItemStack.getTagCompound()
                    .getString("secondary"));
        }
        par3List.add(
    ...
    public static void setPrimaryAttribute(ItemStack s) {
        ...
        if (tag.hasKey("primary")) {
            ...
            if (tag.hasKey("focus_0")) {
                ...
                } else tag.setString("primary", "Combined");
                List<String> secondaryAttribs = new ArrayList<String>();
                if (fire != 0) secondaryAttribs.add("Fire");
                if (water != 0) secondaryAttribs.add("Water");
                if (earth != 0) secondaryAttribs.add("Earth");
                if (air != 0) secondaryAttribs.add("Air");
                if (!secondaryAttribs.isEmpty())
                    tag.setString("secondary", secondaryAttribs.get(rand.nextInt(secondaryAttribs.size())));
                else tag.setString("secondary", getPrimaryAttribute(s));
    ...
    public static String getPrimaryAttribute(ItemStack s) {
        if (s.hasTagCompound()) return s.getTagCompound()
            .getString("primary");
    ...
    public static int getA(ItemStack s, int pass) {
        String a = "";
        if (pass == 0) a = getPrimaryAttribute(s);
        else a = getSecondaryAttribute(s);
        if (a.contains("Fire")) return 0;
        if (a.contains("Water")) return 1;
        if (a.contains("Earth")) return 2;
        if (a.contains("Air")) return 3;
        return -1;
    }
    ...
    public static List<String> getEmberEffects(ItemStack par1ItemStack) {
        ...
        String allEmbers = par1ItemStack.getTagCompound()
            .getString("ember_0") + " "
    ...
    public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
        ...
        if (attrib.contains("Fire")) {
            if ((ECUtils.tryToDecreaseMRUInStorage((EntityPlayer) par3EntityPlayer, -50)
                || this.setMRU(par1ItemStack, -50))) {
                par3EntityPlayer.addPotionEffect(new PotionEffect(Potion.fireResistance.id, 50, 0));
                List<EntityLivingBase> l = par2World.getEntitiesWithinAABB(
                    EntityLivingBase.class,
                    AxisAlignedBB.getBoundingBox(
                        par3EntityPlayer.posX - 2,
                        par3EntityPlayer.posY - 1,
                        par3EntityPlayer.posZ - 2,
                        par3EntityPlayer.posX + 2,
                        par3EntityPlayer.posY + 3,
                        par3EntityPlayer.posZ + 2));
                if (!l.isEmpty()) {
