...
public class ItemGenericEC extends Item {
    ...
    public static String[] names = new String[] { "combinedMagicalAlloys", // 0
        "elementalCore", // 1
        "enderScalePlating", // 2
        "magicalEssence", // 3
        "magicalGoldenOrb", // 4
            ...
            if (b != null) {
                for (int i = 0; i < b.getSizeInventory(); ++i) {
                    ...
                    if (is != null && is.getItem() != null
                        ...
                        && is.getItemDamage() == 8) addedEnergy = 500;
                }
            }
            int current = ApiCore.getPlayerData(base)
                .getPlayerUBMRU();
            ApiCore.getPlayerData(base)
                .modifyUBMRU(current + addedEnergy);
        } else {
    ...
    public static ItemStack getStkByName(String name) {
        ...
        if (lst.contains(name)) {
            ItemStack stk = new ItemStack(ECItems.genericItem, 1, lst.indexOf(name));
            return stk;
        }
        return null;
    }
