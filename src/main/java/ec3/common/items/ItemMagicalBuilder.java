...
public class ItemMagicalBuilder extends ItemStoresMRUInNBT {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        if (this.hasFirstPoint(par1ItemStack)) par3List.add(
            StatCollector.translateToLocal("ec3.txt.p1") + "| X:"
                + this.getFirstPoint(par1ItemStack).x
                + ", Y:"
                + this.getFirstPoint(par1ItemStack).y
                + ", Z:"
                + this.getFirstPoint(par1ItemStack).z);
        if (this.hasSecondPoint(par1ItemStack)) par3List.add(
            StatCollector.translateToLocal("ec3.txt.p2") + "| X:"
                + this.getSecondPoint(par1ItemStack).x
                + ", Y:"
                + this.getSecondPoint(par1ItemStack).y
                + ", Z:"
                + this.getSecondPoint(par1ItemStack).z);
        if (this.hasStoredBlock(par1ItemStack) && this.retrieveStackFromNBT(par1ItemStack) != null) par3List.add(
            StatCollector.translateToLocal("ec3.txt.storedStack") + ": "
                + this.retrieveStackFromNBT(par1ItemStack)
                    .getDisplayName());
    ...
    public ItemStack onItemRightClick(ItemStack is, World w, EntityPlayer p) {
        ...
        if (mop != null && mop.typeOfHit == MovingObjectType.BLOCK) {
            if (!p.isSneaking()) {
                ...
                if (!hasPos1) {
                    ...
                    if (p.worldObj.isRemote) p.addChatMessage(
                        new ChatComponentText(
                            "[Magical Builder] First position: " + mop.blockX + "," + mop.blockY + "," + mop.blockZ)
                ...
                if (!hasPos2) {
                    ...
                    if (dist.getDistance() > 48) {
                        if (p.worldObj.isRemote) p.addChatMessage(
                            new ChatComponentText(
                                "[Magical Builder]The distance between points (" + dist.getDistance()
                                    + ") is too large, max: 48!")
                                        .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.DARK_PURPLE)));
                        return is;
                    }
                    ...
                    if (p.worldObj.isRemote) p.addChatMessage(
                        new ChatComponentText(
                            "[Magical Builder] Second position: " + mop.blockX + "," + mop.blockY + "," + mop.blockZ)
                                .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.DARK_PURPLE)));
                    return is;
                }
                ...
                if (hasPos1 && hasPos2) {
                    if (p.worldObj.isRemote) p.addChatMessage(
                        new ChatComponentText(
                            "[Magical Builder]Both points already set! Shift-rightclick air to reset!")
                                .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.DARK_PURPLE)));
                }
            } else {
                if (this.setStoredStack(is, w, mop.blockX, mop.blockY, mop.blockZ)) {
                    if (this.retrieveStackFromNBT(is) != null) {
                        if (p.worldObj.isRemote) p.addChatMessage(
                            new ChatComponentText(
                                "[Magical Builder]Set the block to: " + this.retrieveStackFromNBT(is)
                                    .getDisplayName())
                                        .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.DARK_PURPLE)));
                    }
                }
            ...
            if (p.isSneaking()) {
                ...
                if (p.worldObj.isRemote) p.addChatMessage(
                    new ChatComponentText("[Magical Builder]Both points reseted!")
                        .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.DARK_PURPLE)));
            } else {
                if (this.hasFirstPoint(is) && this.hasSecondPoint(is)
                    && ((this.hasStoredBlock(is) && this.retrieveStackFromNBT(is) != null)
                        || is.getItemDamage() == 1)) {
                    int setted = this.setAreaToBlock(p, is);
                    if (p.worldObj.isRemote) p.addChatMessage(
                        new ChatComponentText(
                            "[Magical Builder]Filled selected area! " + setted + " blocks got replaced!")
                                .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.DARK_PURPLE)));
                }
    ...
    public boolean resetPoints(ItemStack is) {
        ...
        if (!tag.hasKey("p1_x") && !tag.hasKey("p2_x")) return false;
    ...
    public boolean hasFirstPoint(ItemStack is) {
        return MiscUtils.getStackTag(is)
            .hasKey("p1_x");
    }
    ...
    public int findPlayerISSlot(EntityPlayer e, ItemStack is) {
        ...
        if (e.capabilities.isCreativeMode) return Integer.MAX_VALUE;

        for (int i = 0; i < e.inventory.getSizeInventory(); ++i) {
            ItemStack stk = e.inventory.getStackInSlot(i);
            if (stk != null && stk.isItemEqual(is)) return i;
        }
        return -1;
    }
    ...
    public int setAreaToBlock(EntityPlayer e, ItemStack is) {
        ...
        int itemsSet = 0;
        if (is.getItemDamage() == 1) slotNum = Integer.MAX_VALUE;
        ...
        if (slotNum != -1) {
            for (int x = 0; x <= diffX; ++x) {
                int dx = x;
                if (start.x >= end.x) dx = MathHelper.floor_double(end.x + x);
                else dx = MathHelper.floor_double(start.x + x);

                for (int y = 0; y <= diffY; ++y) {
                    int dy = y;
                    if (start.y >= end.y) dy = MathHelper.floor_double(end.y + y);
                    else dy = MathHelper.floor_double(start.y + y);

                    for (int z = 0; z <= diffZ; ++z) {
                        int dz = z;
                        if (start.z >= end.z) dz = (int) (end.z + z);
                        else dz = (int) (start.z + z);

                        ItemStack settedTo = setTo != null ? setTo.copy() : new ItemStack(Blocks.air, 1, 0);

                        if (is.getItemDamage() == 0) {
                            if (e.worldObj.isAirBlock(dx, dy, dz)) {
                                if (!e.canPlayerEdit(dx, dy, dz, 0, settedTo)) continue;

                                if (!(ECUtils.tryToDecreaseMRUInStorage(e, -25) || this.setMRU(is, -25)))
                                    return itemsSet;

                                slotNum = this.decreasePlayerStackInSlot(e, settedTo, slotNum);
                                // e.worldObj.setBlock(dx, dy, dz, Block.getBlockFromItem(setTo.getItem()),
                                // setTo.getItemDamage(), 3);
                                ...
                                if (slotNum == -1) return itemsSet;
                            }
                        }
                        if (is.getItemDamage() == 1) {
                            if (!e.canPlayerEdit(dx, dy, dz, 0, settedTo)) continue;

                            if (!(ECUtils.tryToDecreaseMRUInStorage(e, -250) || this.setMRU(is, -250))) return itemsSet;

                            if (e.worldObj.getBlock(dx, dy, dz)
                                .getBlockHardness(e.worldObj, dx, dy, dz) >= 0 && !e.worldObj.isRemote) {
                                GameType type = GameType.SURVIVAL;
                                if (e.capabilities.isCreativeMode) type = GameType.CREATIVE;
                                if (!e.capabilities.allowEdit) type = GameType.ADVENTURE;

                                BreakEvent be = ForgeHooks
                                    .onBlockBreakEvent(e.worldObj, type, (EntityPlayerMP) e, dx, dy, dz);
                                if (!be.isCanceled()) {
                                    e.worldObj.getBlock(dx, dy, dz)
                                        .dropBlockAsItem(
                                            e.worldObj,
                                            dx,
                                            dy,
                                            dz,
                                            e.worldObj.getBlockMetadata(dx, dy, dz),
                                            0);
                        ...
                        if (is.getItemDamage() == 2) {
                            ...
                            if (e.worldObj.getBlock(dx, dy, dz)
                                .getBlockHardness(e.worldObj, dx, dy, dz) >= 0 && !e.worldObj.isRemote) {
                                ItemStack worldStack = new ItemStack(
                                    e.worldObj.getBlock(dx, dy, dz),
                                    1,
                                    e.worldObj.getBlockMetadata(dx, dy, dz));
                                ...
                                if (e.worldObj.getBlock(dx, dy, dz)
                                    ...
                                    if (!be.isCanceled()) {
                                        ...
                                        e.worldObj.setBlock(
                                            ...
                                            dy,
                                            dz,
                                            Block.getBlockFromItem(setTo.getItem()),
                                            setTo.getItemDamage(),
                                            3);
                                        ++itemsSet;
                                    }
