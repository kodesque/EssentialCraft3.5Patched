...
public class ItemMagmaticWand extends ItemStoresMRUInNBT {
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int px, int y, int pz, int side,
        ...
        ItemStack ore = new ItemStack(
            world.getBlock(px, y, pz)
                .getItem(world, px, y, pz),
            1,
            world.getBlockMetadata(px, y, pz));
        if (ore != null && !world.isRemote) {
            ...
            if (!player.worldObj.isRemote && metadata != -1
                && (ECUtils.tryToDecreaseMRUInStorage(player, -500) || this.setMRU(stack, -500))) {
                ...
                ItemStack sugStk = new ItemStack(ECItems.magicalAlloy, suggestedStackSize, metadata);

                GameType type = GameType.SURVIVAL;
                if (player.capabilities.isCreativeMode) type = GameType.CREATIVE;
                if (!player.capabilities.allowEdit) type = GameType.ADVENTURE;
