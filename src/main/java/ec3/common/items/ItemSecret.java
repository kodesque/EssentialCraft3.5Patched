...
public class ItemSecret extends Item {
    ...
    public static String[] dropNames = new String[] { "410_ticket", "d6", "ironwood_branch", "mysterious_stick",
        "smoothandsilkystone", "strange_figure", "strange_symbol" };
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        switch (metadata) {
            ...
            case 4: {
                ...
                par3List.add(
                    "Maybe some kind of " + EnumChatFormatting.DARK_GREEN
                        + "bird"
            ...
            case 0: {
                ...
                List playerLst = wrld.getEntitiesWithinAABB(
                    ...
                    AxisAlignedBB.getBoundingBox(
                        ...
                        player.posY + 10,
                        player.posZ + 10));
                BiomeGenBase biome = wrld
                    .getBiomeGenForCoords(MathHelper.floor_double(player.posX), MathHelper.floor_double(player.posZ));
                boolean canWork = (wrld.getWorldTime() % 24000 >= 14000 && wrld.getWorldTime() % 24000 <= 16000)
                ...
                if (canWork) {
                    player.inventory.setInventorySlotContents(
                        ...
                        new ItemStack(ECItems.record_everlastingSummer, 1, 0));
                    if (wrld.isRemote) {
                        player.addChatMessage(
                            new ChatComponentText(
                                EnumChatFormatting.WHITE + "You gase into the stars holding the ticket."));
                        player.addChatMessage(
                            new ChatComponentText(
                                EnumChatFormatting.WHITE + "Suddenly a gust of wind swoops upon you."));
                        player.addChatMessage(
                            new ChatComponentText(
                                EnumChatFormatting.WHITE + "You are immediately beeing attacked by lots of feels."));
                        player.addChatMessage(
                            new ChatComponentText(EnumChatFormatting.WHITE + "Strange, warm feels fall upon you."));
                        player.addChatMessage(
                            new ChatComponentText(EnumChatFormatting.WHITE + "You feel calm and relaxed."));
                        player.addChatMessage(
                            new ChatComponentText(
                                EnumChatFormatting.WHITE
                                    + "A feeling falls upon you. You feel like you've just lived a whole another life."));
                        player.addChatMessage(
                            new ChatComponentText(
                                EnumChatFormatting.WHITE
                                    + "You try to remember what happened, but memory lets you down."));
                        player.addChatMessage(
                            new ChatComponentText(
                                EnumChatFormatting.WHITE
                                    + "You suddenly realise, that you no longer keep the ticket in your hand."));
                        player.addChatMessage(
                            new ChatComponentText(EnumChatFormatting.WHITE + "Instead a music disk is in your hand."));
                        player.addChatMessage(
                            new ChatComponentText(
                                EnumChatFormatting.WHITE
                                    + "When you gase to the disk, you begin to hear the song, written on it."));
                        player.addChatMessage(
                            new ChatComponentText(
                                EnumChatFormatting.WHITE
                                    + "You start feeling really sad, like you've missed something very important to you."));
                        player.addChatMessage(
                            new ChatComponentText(
                                EnumChatFormatting.WHITE + "You feel lonely, like missing another half of you."));
                        player.addChatMessage(
                            new ChatComponentText(EnumChatFormatting.WHITE + "After some time you calm down."));
                    }
                    player.triggerAchievement(ECAchievements.achievementList.get("theEndlessSummer"));
                }
            }
        }
    ...
    public void registerIcons(IIconRegister p_94581_1_) {
        ...
        for (int i = 0; i < 7; ++i) itemIcons[i] = p_94581_1_.registerIcon("essentialcraft:secrets/" + dropNames[i]);
    }
