...
public class TileChargingChamber extends TileMRUGeneric {
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) tryChargeTools();
    }
                ...
                if (mru < maxMRU) {
                    if (mru + p < maxMRU) {
                        if (this.getMRU() > p * reqMRUModifier) {
                            ...
                            if (item.setMRU(_gen_var_0, p)) this.setMRU((int) (this.getMRU() - p * reqMRUModifier));
                        } else if (this.getMRU() > 0) {
                            if (item.setMRU(_gen_var_0, this.getMRU())) this.setMRU(0);
                        }
                    } else {
                        int k = maxMRU - mru;
                        if (this.getMRU() > k * reqMRUModifier) {
                            if (item.setMRU(_gen_var_0, k)) this.setMRU((int) (this.getMRU() - k * reqMRUModifier));
                        } else if (this.getMRU() > 0) {
                            if (item.setMRU(_gen_var_0, this.getMRU())) this.setMRU(0);
                        }
                    }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "ChargingChamberSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "Charge cost Modifier:1.0" },
