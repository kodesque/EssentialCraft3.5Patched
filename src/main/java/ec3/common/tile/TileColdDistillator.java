...
public class TileColdDistillator extends TileMRUGeneric {
    ...
    public void CdamageAround() {
        List<EntityLivingBase> l = this.worldObj.getEntitiesWithinAABB(
            ...
            for (int z = -3; z <= 3; ++z) {
                for (int y = -3; y <= 3; ++y) {
                    ...
                    if (b != null && b instanceof IColdBlock) {
                        i += ((IColdBlock) b).getColdModifier(
                            this.worldObj,
                            xCoord + x,
                            yCoord + y,
                            zCoord + z,
                            this.worldObj.getBlockMetadata(xCoord + x, yCoord + y, zCoord + z));
                    }
                }
            }
        }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "ColdDistillatorSettings",
                "tileentities",
                new String[] { "Produced Balance:0.0", "Max MRU:" + ApiCore.GENERATOR_MAX_MRU_GENERIC * 10,
