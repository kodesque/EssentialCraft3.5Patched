            ...
            if (cleared == null) {
                ...
                if (!this.worldObj.isRemote) {
                    ...
                    if (this.getMRU() - mruUsage > 0) {
                        ...
                        if (clearTime <= 0) {
                            int metadata = this.worldObj
                                .getBlockMetadata((int) cleared.x, (int) cleared.y, (int) cleared.z);
                            if (metadata == 0 || removeBlock)
                                this.worldObj.setBlockToAir((int) cleared.x, (int) cleared.y, (int) cleared.z);
                            else this.worldObj.setBlockMetadataWithNotify(
                                (int) cleared.x,
                                (int) cleared.y,
                                (int) cleared.z,
                                metadata - 1,
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "CorruptionCleanerSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage per Tick:20",
