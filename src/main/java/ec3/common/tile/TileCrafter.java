...
public class TileCrafter extends TileMRUGeneric {
    ...
    public void updateEntity() {
        if (this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)
            || this.worldObj.getBlockPowerInput(xCoord, yCoord, yCoord) > 0) {
            if (!this.hasFrame()) {
                this.makeRecipe();
            } else {
                if (this.hasSufficientForCraftWithFrame()) this.makeRecipe();
            }
        }
    }
    ...
    public boolean isFrame(ItemStack is) {
        return is != null && is.getItem() instanceof ItemCraftingFrame
            && new InventoryCraftingFrame(is).inventory[9] != null;
    }
    ...
    public boolean hasOreDict() {
        return this.getStackInSlot(10) != null && this.getStackInSlot(10)
            .getItem() instanceof ItemCraftingFrame
            && this.getStackInSlot(10).stackTagCompound != null
            && !this.getStackInSlot(10).stackTagCompound.getBoolean("ignoreOreDict");
    }

    public boolean hasFrame() {
        return this.getStackInSlot(10) != null && this.getStackInSlot(10)
            .getItem() instanceof ItemCraftingFrame
            && new InventoryCraftingFrame(this.getStackInSlot(10)).inventory[9] != null;
    }

    public boolean areStacksTheSame(ItemStack stk1, ItemStack stk2, boolean oreDict) {
        if (stk1 == null && stk2 == null) return true;

        if (stk1 == null || stk2 == null) return false;

        if (!oreDict) {
            if (!stk1.isItemEqual(stk2)) return false;

            if (!ItemStack.areItemStacksEqual(stk1, stk2)) return false;
        } else {
            if (!ECUtils.oreDictionaryCompare(stk1, stk2)) return false;
        }
    ...
    public boolean hasSufficientForCraftWithFrame() {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (!areStacksTheSame(frame[i], stk, hasOreDict())) return false;
        }
    ...
    public ItemStack[] getRecipeFromFrame() {
        if (this.getStackInSlot(10) != null && this.getStackInSlot(10)
            .getItem() instanceof ItemCraftingFrame) {
    ...
    public void makeRecipe() {
        ...
        ItemStack result = CraftingManager.getInstance()
            .findMatchingRecipe(craftingInv, this.worldObj);
        ...
        if (result != null) {
            if (this.getStackInSlot(9) == null) {
                ...
                if (this.getStackInSlot(9)
                    .isItemEqual(result) && ItemStack.areItemStackTagsEqual(result, this.getStackInSlot(9))) {
                    if (this.getStackInSlot(9).stackSize + result.stackSize <= this.getInventoryStackLimit()
                        && this.getStackInSlot(9).stackSize + result.stackSize <= result.getMaxStackSize()) {
    ...
    public void decreaseStacks() {
        for (int i = 0; i < 9; ++i) {
            ...
            if (is != null) {
                ItemStack container = is.getItem()
                    .getContainerItem(is);
