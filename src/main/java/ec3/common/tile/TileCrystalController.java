...
public class TileCrystalController extends TileMRUGeneric {
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord))
            if (!this.worldObj.isRemote && this.worldObj.rand.nextInt(chanceToUseMRU) == 0
                && this.getMRU() >= mruUsage) {
                    this.setMRU(this.getMRU() - mruUsage);
                }
        this.spawnParticles();
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) this.mutateToElement();
    }
        ...
        if (this.getMRU() > 0 && this.getCrystal() != null) {
            for (int o = 0; o < 2; ++o) {
                this.worldObj.spawnParticle(
                    "reddust",
                    xCoord + 0.3D + this.worldObj.rand.nextDouble() / 2,
                    yCoord + 0.3F + ((float) o / 2),
                    zCoord + 0.3D + this.worldObj.rand.nextDouble() / 2D,
                    -1.0D,
                    1.0D,
                    0.0D);
            }
    ...
    public void mutateToElement() {
        if (this.getStackInSlot(1) != null && this.getStackInSlot(1)
            ...
            && this.getCrystal() != null
            && this.getCrystal().size < 100) {
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "CrystalControllerSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage:100",
                    "Chance to NOT use MRU(do not set to 0!):20", "Crystal mutation chance modifier:0.001" },
                "");
            String dataString = "";
