...
public class TileDarknessObelisk extends TileMRUGeneric {
    ...
    public void updateEntity() {
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            if (this.getStackInSlot(1) == null || !(Block.getBlockFromItem(
                this.getStackInSlot(1)
                    .getItem()) instanceof BlockMobSpawner)) {
                ...
                if (l != null && !l.isEmpty()
                    ...
                    && this.worldObj.rand.nextFloat() < worldSpawnChance
                    && this.getMRU() - mruUsage > 0) {
                    ...
                    if (SpawnerAnimals.canCreatureTypeSpawnAtLocation(
                        ...
                        spawnlistentry = wrld
                            .spawnRandomCreature(EnumCreatureType.monster, rndOffsetX, rndOffsetY, rndOffsetZ);
                        if (spawnlistentry != null) {
                            ...
                            try {
                                entityliving = (EntityLiving) spawnlistentry.entityClass
                                    ...
                                    .newInstance(new Object[] { wrld });
                                entityliving.setLocationAndAngles(
                                    (double) rndOffsetX + 0.5F,
                                    (double) rndOffsetY,
                                    (double) rndOffsetZ + 0.5D,
                                    wrld.rand.nextFloat() * 360.0F,
                                    0.0F);
                                Result canSpawn = ForgeEventFactory.canEntitySpawn(
                                    entityliving,
                                    wrld,
                                    (float) rndOffsetX + 0.5F,
                                    (float) rndOffsetY,
                                    (float) rndOffsetZ + 0.5F);
                ...
                if (Block.getBlockFromItem(
                    ...
                    if (this.innerRotation >= mobSpanwerDelay && this.getMRU() - mruUsage > 0) {
                        ...
                        int metadata = this.getStackInSlot(1)
                            .getItemDamage();
                        ...
                        if (base instanceof EntityLiving) {
                            ...
                            int rndOffsetX = (int) (xCoord
                                + MathUtils.randomDouble(this.worldObj.rand) * mobSpawnerRadius);
                            int rndOffsetY = (int) (yCoord + MathUtils.randomDouble(this.worldObj.rand));
                            int rndOffsetZ = (int) (zCoord
                                + MathUtils.randomDouble(this.worldObj.rand) * mobSpawnerRadius);
                            entityliving.setLocationAndAngles(
                                rndOffsetX + 0.5D,
                                rndOffsetY,
                                rndOffsetZ + 0.5D,
                                this.worldObj.rand.nextFloat() * 360.0F,
                                0.0F);
                            if (entityliving.getCanSpawnHere()) {
                                if (!this.worldObj.isRemote) this.worldObj.spawnEntityInWorld(entityliving);
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "DarknessObeliskSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage:500",
                    ...
                    "Enable mob spawning using spawner:true", "Radius for a spawner:3", "Radius for no spawner:8" },
                "");
            String dataString = "";
