...
public class TileElementalCrystal extends TileEntity {
    ...
    public float getElementByNum(int num) {
        if (num == 0) return this.fire;
        if (num == 1) return this.water;
        if (num == 2) return this.earth;
        if (num == 3) return this.air;
        return -1;
    }

    public void setElementByNum(int num, float amount) {
        if (num == 0) this.fire += amount;
        if (num == 1) this.water += amount;
        if (num == 2) this.earth += amount;
        if (num == 3) this.air += amount;
    }
        ...
        if (metadata == 1) {
            ...
            if (!b.isBlockSolid(worldObj, xCoord, yCoord - 1, zCoord, 0)) {
                this.worldObj.getBlock(xCoord, yCoord, zCoord)
                    .dropBlockAsItem(getWorldObj(), xCoord, yCoord, zCoord, metadata, 0);
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "ElementalCrystalSettings",
                "tileentities",
                new String[] { "Chance to mutate per tick:0.001",
