        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            ...
            if (ore != null) {
                ...
                if (metadata != -1) {
                    ...
                    } else if (this.getStackInSlot(2)
                        .getItem() == ItemsCore.magicalAlloy
                        && this.getStackInSlot(2)
                            .getItemDamage() == metadata
                        && this.getStackInSlot(2).stackSize + 1 <= this.getStackInSlot(2)
                            .getMaxStackSize()
                        && this.getStackInSlot(2).stackSize + 1 <= this.getInventoryStackLimit()) {
                            if (this.getMRU() - usage >= 0) {
                                this.setMRU(this.getMRU() - usage);
            ...
            if (alloy != null && this.getStackInSlot(1)
                .getItem() == ItemsCore.magicalAlloy) {
                ...
                if (oreLst != null && !oreLst.isEmpty()) {
                    ...
                    if (this.getStackInSlot(2) == null) {
                        ...
                        && this.getStackInSlot(2).stackSize + 2 <= this.getStackInSlot(2)
                            .getMaxStackSize()
                        && this.getStackInSlot(2).stackSize + 2 <= this.getInventoryStackLimit()) {
                            if (this.getMRU() - usage >= 0) {
                                this.setMRU(this.getMRU() - usage);
                                this.worldObj.spawnParticle(
                                    "flame",
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "FurnaceMagicSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage:25",
                    "Can this device actually generate corruption:false",
                    "The amount of corruption generated each tick(do not set to 0!):3", "Ticks required to smelt:400" },
                "");
            String dataString = "";
