...
public class TileMIM extends TileMRUGeneric {
    ...
    public void updateEntity() {
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            if (!this.worldObj.isRemote) for (int i = 1; i < this.getSlots(); ++i) {
                    ...
                    if (boundGem.getItem() instanceof ItemBoundGem) {
                        ...
                        if (coords != null && coords.length == 3) {
                            ...
                            if (tile != null && this.getMRU() - 3 > 0
                                && !this.worldObj.isBlockIndirectlyGettingPowered(coords[0], coords[1], coords[2])) {
            ...
            for (int j = 64; j < 128; ++j) {
                ...
                if (is != null && this.findSlotForItemStack(is) != -1) {
                    ...
                    if (this.getStackInSlotInner(i) == null) {
                        ...
                        if (this.getStackInSlotInner(i).stackSize + 1 <= this.getStackInSlotInner(i)
                            .getMaxStackSize()) {
        ...
        if (tile instanceof TileAMINInjector) {
                        ...
                        if (is != null) {
                            if (ECUtils.canFilterAcceptItem(filterInventory, is, filter)) {
                                if (e.getStackInSlot(i + 64) == null) {
                                    ...
                                    if (e.getStackInSlot(i + 64).stackSize + 1 <= e.getStackInSlot(i + 64)
                                        .getMaxStackSize()) {
            ...
            if (tile instanceof TileMINInjector) {
                ...
                for (int j = 0; j < 256; ++j) {
                    ItemStack is = this.getStackInSlotInner(j);
                    if (is != null) {
                        if (e.getStackInSlot(0) == null || (e.getStackInSlot(0)
                            .getItem() instanceof ItemFilter
                            && ECUtils.canFilterAcceptItem(
                                new InventoryMagicFilter(e.getStackInSlot(0)),
                                is,
                                e.getStackInSlot(0)))) {
                            if (e.getStackInSlot(1) == null) {
                                ...
                                if (e.getStackInSlot(1).stackSize + 1 <= e.getStackInSlot(1)
                                    .getMaxStackSize()) {
    ...
    public int getSlotsInner() {
        int retInt = 0;
        for (int i = 0; i < 256; ++i) {
            if (this.getStackInSlotInner(i) != null) ++retInt;
            else {
                if (i < 255) {
                    if (this.getStackInSlot(i + 1) == null) return retInt;
                } else return retInt;
            }
        }
        return retInt;
    ...
    public int getSlots() {
        ...
        for (int i = 1; i < 9 * 6 + 1; ++i) {
            if (this.getStackInSlot(i) != null) ++retInt;
            else {
                return retInt;
            }
        }
    ...
    public int findSlotForItemStack(ItemStack stk) {
        for (int i = 0; i < 256; ++i) {
            ...
            if (is != null && stk.isItemEqual(is)
                && ItemStack.areItemStackTagsEqual(stk, is)
                && is.stackSize + 1 <= is.getMaxStackSize()) return i;
        }
        for (int i = 0; i < 256; ++i) {
            ItemStack is = this.getStackInSlotInner(i);
            if (is == null) return i;
