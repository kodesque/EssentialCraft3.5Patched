...
import ec3.dummycore.utils.TileStatTracker;
...
public abstract class TileMRUGeneric extends TileEntity implements ITERequiresMRU, IInventory, ISidedInventory {
    ...
    public int[] getAccessibleSlotsFromSide(int side) {
        if (this.getSizeInventory() > 0) {
            if (side == 1 && slot0IsBoundGem) return new int[] { 0 };

            if (side == 0) return getOutputSlots();
            else {
                int[] retInt;
                ...
                else retInt = new int[0];
                int cnt = 0;
                if (retInt.length > 0) for (int i = 0; i < this.getSizeInventory(); ++i) {
                    if ((i != 0 && slot0IsBoundGem) && !MathUtils.arrayContains(getOutputSlots(), i)) {
                        if (cnt < retInt.length) retInt[cnt] = i;
                        ++cnt;
                    } else {
