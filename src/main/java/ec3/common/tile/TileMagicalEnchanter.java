...
public class TileMagicalEnchanter extends TileMRUGeneric {
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) this.tryEnchant();
    ...
    public void enchant() {
        ...
        ItemStack enchanted = this.getStackInSlot(1)
            .copy();
            ...
            for (int y = 0; y <= 2; ++y) {
                for (int z = -2; z <= 2; ++z) {
                    if (x == 0 && y == 0 && z == 0) {
                        ...
                        l += ForgeHooks
                            .getEnchantPower(this.worldObj, this.xCoord + x, this.yCoord + y, this.zCoord + z);
                    }
                }
            }
        }
        if (l > maxEnchantmentLevel) l = maxEnchantmentLevel;
        return l;
    }
    ...
    public boolean canItemBeEnchanted() {
        try {
            ...
            if (s != null && this.getMaxPower() > 0 && this.getMRU() > mruUsage && this.getStackInSlot(2) == null) {
                if (s.isItemEnchantable() && this.getEnchantmentsForStack(s) != null
                    && !this.getEnchantmentsForStack(s)
                        .isEmpty()) {
                    return true;
                }
            }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MagicalEnchanterSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage:100",
                    "Max level of enchantment:60", "Can this device actually generate corruption:false",
