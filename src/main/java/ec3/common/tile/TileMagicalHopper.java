...
public class TileMagicalHopper extends TileMRUGeneric {
    ...
    public void updateEntity() {
        ...
        if (delay <= 0 && !this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            ...
            AxisAlignedBB teleportBB = AxisAlignedBB.getBoundingBox(
                xCoord + r.offsetX,
                yCoord + r.offsetY,
                zCoord + r.offsetZ,
                xCoord + 1 + r.offsetX,
            ...
            List<EntityItem> items = this.worldObj.getEntitiesWithinAABB(
                ...
                AxisAlignedBB.getBoundingBox(xCoord, yCoord, zCoord, xCoord + 1, yCoord + 1, zCoord + 1)
                    .expand(itemHopRadius, itemHopRadius, itemHopRadius));
    ...
    public boolean canTeleport(EntityItem item) {
        if (item.getEntityItem() == null) return false;

        if (this.getStackInSlot(0) == null || !(this.getStackInSlot(0)
            .getItem() instanceof ItemFilter)) return true;

        return ECUtils.canFilterAcceptItem(
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MagicalHopperSettings",
                "tileentities",
                new String[] { "Radius in which the hopper will detect items(int):3",
