...
public class TileMagicalJukebox extends TileMRUGeneric {
    ...
    String[] achievementNames = new String[] { "Top Secret!", "Secret Achievement 2014!",
        "Never give up on your secrets!", "Go,Go secret rangers!", "Such Secrets!", "The Secret is a Lie!",
        "Secret Bro!", "Secrets, Secrets everywhere!", "Secrets,Secrets,Secrets!", "Never give up!", "Unachievable!",
        "Too much Secrets!", "Very Secret!", "Top Secret of 2164!", "The Secret Paradize!" };
    String[] monsterNames = new String[] { "jeb_", "Bob", "Jordan", "Monstro", "Isaac's Fork", "Bucker", "Arthem",
        "Cube", "2XStuffed", "Rainbow!", "StringFormatException", "java", "Secret!!!", "Ronny", "Clementine", "Jack",
        "Roland", "The Tower", "Judjment", "Jorji", "EZIC", "Roshan", "Ursa", "Enderbro", "Zombie", "poodiepie",
        "Cartman", "Markiplier", "The Game Theorist!", "Direwolf20", "Pahimar", "Eloraam", "CoTH", "[CoTH]",
        "ForgeDevName", "Player000", "Herobrine", "[DATA REMOVED]", "SCP-126", "No More!", "The longest list!",
        "Much Text", "Thanks!", "Time", "Moon", "Sun", "Earth", "Mars", "Theory", "Bang!", "Poo", "Lol 69", "146%",
        "2000", "1337", "OVER 9000!", "++i + ++i", "Brain", "<--->>---<<-", "Hello World!", "Doge", "The Wurm",
        "Spice!", "Happy", ":3", "^_^" };
    ...
    public void updateEntity() {
        ...
        if (this.getStackInSlot(1) != null && this.getStackInSlot(1)
            .getItem() == ItemsCore.record_secret && this.recordCooldownTime <= 0) {
            this.recordPlayed = 1;
            this.recordCooldownTime = 257 * 20;
            if (!this.worldObj.isRemote) this.worldObj.playAuxSFXAtEntity(
                (EntityPlayer) null,
                1005,
                xCoord,
                yCoord,
        ...
        if (this.getStackInSlot(1) != null && this.getStackInSlot(1)
            ...
            for (int i = 0; i < 50; ++i) this.worldObj.spawnParticle(
                "mobSpell",
                xCoord + randomX,
                yCoord + randomY,
                zCoord + randomZ,
                ...
                randomColorB);
            for (int i = 0; i < 100; ++i) {
                if (randomBool) this.worldObj.spawnParticle(
                    "crit",
                    xCoord + randomX,
                    yCoord + randomY,
                    zCoord + randomZ,
                    MathUtils.randomDouble(this.worldObj.rand),
                    1.1D + MathUtils.randomDouble(this.worldObj.rand),
                    MathUtils.randomDouble(this.worldObj.rand));
                else this.worldObj.spawnParticle(
                    "magicCrit",
                    xCoord + randomX,
                    yCoord + randomY,
                    zCoord + randomZ,
            ...
            ChatStyle greenItalicAchievement = new ChatStyle().setColor(EnumChatFormatting.GREEN)
                .setItalic(true);
            ...
            List<EntityPlayer> playerLst = this.worldObj.getEntitiesWithinAABB(
                EntityPlayer.class,
                AxisAlignedBB.getBoundingBox(xCoord - 6, yCoord - 6, zCoord - 6, xCoord + 6, yCoord + 6, zCoord + 6));
            if (!playerLst.isEmpty() && this.worldObj.getWorldTime() % 60 == 0 && !this.worldObj.isRemote) {
                ...
                player.addChatMessage(
                    new ChatComponentText(player.getDisplayName() + " has just earned the achievement ")
                        .appendSibling(achievementText));
            }
            List<EntitySheep> sheepLst = this.worldObj.getEntitiesWithinAABB(
                EntitySheep.class,
                AxisAlignedBB
                    .getBoundingBox(xCoord - 16, yCoord - 16, zCoord - 16, xCoord + 16, yCoord + 16, zCoord + 16));
            ...
            List<EntityLiving> baseLst = this.worldObj.getEntitiesWithinAABB(
                EntityLiving.class,
                AxisAlignedBB
                    .getBoundingBox(xCoord - 16, yCoord - 16, zCoord - 16, xCoord + 16, yCoord + 16, zCoord + 16));
            ...
            if (this.worldObj.getWorldTime() % 20 == 0) {
                EntityFallingBlock flower = new EntityFallingBlock(
                    worldObj,
                    xCoord + MathUtils.randomDouble(this.worldObj.rand) * 16,
                    255,
                    zCoord + MathUtils.randomDouble(this.worldObj.rand) * 16,
                    fallingBlock);
                flower.field_145812_b = 3;
                if (!this.worldObj.isRemote) this.worldObj.spawnEntityInWorld(flower);
            }
            worldObj.spawnParticle(
                "note",
                (double) xCoord + 0.5D + MathUtils.randomDouble(this.worldObj.rand) * 3,
                (double) yCoord + 1.2D,
                (double) zCoord + 0.5D + MathUtils.randomDouble(this.worldObj.rand) * 3,
                (double) MathUtils.randomDouble(this.worldObj.rand) * 24 / 24.0D,
                0.0D,
                0.0D);
            return;
        }
        ...
        if (this.getStackInSlot(1) != null) {
            if (this.worldObj.rand.nextFloat() <= 0.33F) worldObj.spawnParticle(
                "note",
                (double) xCoord + 0.5D,
                (double) yCoord + 1.2D,
                (double) zCoord + 0.5D + MathUtils.randomDouble(this.worldObj.rand) / 2,
                (double) MathUtils.randomDouble(this.worldObj.rand) * 24 / 24.0D,
                0.0D,
                0.0D);
        }

        if (this.recordCooldownTime > 0) --this.recordCooldownTime;
        ...
        if (this.getStackInSlot(1) == null || !(this.getStackInSlot(1)
            .getItem() instanceof ItemRecord) && this.recordPlayed == 1) {
            this.recordPlayed = 0;
