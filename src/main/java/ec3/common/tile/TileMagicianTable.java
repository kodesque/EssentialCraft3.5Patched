...
public class TileMagicianTable extends TileMRUGeneric {
    ...
    public void updateEntity() {
        if (this.upgrade == -1) this.speedFactor = 1F;
        else this.speedFactor = MagicianTableUpgrades.upgradeEfficency.get(upgrade);
        if (this.speedFactor != 1) this.mruConsume = this.speedFactor * 2 * mruUsage;
        else this.mruConsume = 1 * mruUsage;
        super.updateEntity();
    ...
    public boolean canFunction(MagicianTableRecipe rec) {
        ...
        if (result != null) {
            if (this.getStackInSlot(6) == null) {
                ...
                if (this.getStackInSlot(6)
                    .isItemEqual(result)) {
                    if (this.getStackInSlot(6).stackSize + result.stackSize <= this.getInventoryStackLimit()
                        && this.getStackInSlot(6).stackSize + result.stackSize <= this.getStackInSlot(6)
                            .getMaxStackSize()) {
                        return true;
                    }
                }
    ...
    public void craft() {
        if (this.canFunction(this.currentRecipe)) {
            ...
            if (this.getStackInSlot(6) == null) {
                ...
                if (copied.stackSize == 0) copied.stackSize = 1;
                ...
                this.setInventorySlotContents(
                    ...
                    new ItemStack(
                        stk.getItem(),
                        stk.stackSize + this.getStackInSlot(6).stackSize,
                        stk.getItemDamage()));
            }
            for (int i = 1; i < 6; ++i) {
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MagicianTableSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage Modifier:1.0",
