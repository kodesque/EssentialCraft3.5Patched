        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            if (!this.worldObj.isRemote) {
                ...
                if (stk != null && stk.getItem() instanceof ItemSoulStone) {
                    if (stk.getTagCompound() != null) {
                        String username = stk.getTagCompound()
                        ...
                        if (MinecraftServer.getServer() != null && MinecraftServer.getServer()
                            ...
                            if (p != null) {
                                if (this.getMRU() + mruGenerated <= this.getMaxMRU()) {
                                    int current = ECUtils.getData(p)
                                        .getPlayerUBMRU();
                                    if (current - mruUsage >= 0) {
                                        ECUtils.getData(p)
                                            .modifyUBMRU((int) (current - mruUsage));
                                        ...
                                        for (int o = 0; o < 10; ++o) {
                                            this.worldObj.spawnParticle(
                                                "reddust",
                                                xCoord + 0.25D + this.worldObj.rand.nextDouble() / 2.2D,
                                                yCoord + 0.25D + ((float) o / 20),
                                                zCoord + 0.25D + this.worldObj.rand.nextDouble() / 2.2D,
                                                1.0D,
                                                0.0D,
                                                1.0D);
                                        }
            ...
            if (t && sndTime <= 0) {
                ...
                this.worldObj.playSound(
                    ...
                    yCoord + 0.5D,
                    zCoord + 0.5D,
                    "essentialcraft:sound.deepnoise",
                    0.01F,
                    2F,
                    false);
            }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MatrixAbsorberSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.GENERATOR_MAX_MRU_GENERIC / 10, "Default balance:1.0",
                    "MRU generated per tick:1", "UBMRU Used per tick:10", "Request Immidiate Data Sync:true" },
                "");
            String dataString = "";
