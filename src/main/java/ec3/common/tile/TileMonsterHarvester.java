    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            ...
            if (destrTick >= mobDestructionTimer) {
                destrTick = 0;
                List<EntityLivingBase> lst = worldObj.getEntitiesWithinAABB(
                ...
                if (!lst.isEmpty() && !this.worldObj.isRemote) {
                    for (int i = 0; i < lst.size(); ++i) {
                        ...
                        if (!(e instanceof EntityPlayer)) {
                            if (getMRU() > mruUsage) {
                                if (e instanceof IBossDisplayData && !allowBossDuplication) return;
                                ...
                                FakePlayer player = new FakePlayer(
                                    (WorldServer) e.worldObj,
                                    ECUtils.EC3FakePlayerProfile);
                                ...
                                if (copy.getHealth() > 0) copy.setDead();
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MonsterDuplicatorSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage Per Mob:100",
                    "Can this device actually generate corruption:false",
                    "The amount of corruption generated each tick(do not set to 0!):10",
                    "Radius to duplicate mobs within:12.0", "Ticks required to duplicate mobs:1440",
                    "Allow duplication of bosses:false", "Remove inventory of a duplicate before killing it:true" },
                "");
            String dataString = "";
