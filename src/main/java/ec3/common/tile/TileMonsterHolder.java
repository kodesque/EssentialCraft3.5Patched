    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            List<EntityLivingBase> lst = getWorldObj().getEntitiesWithinAABB(
                ...
                AxisAlignedBB
                    .getBoundingBox(xCoord - 32, yCoord - 32, zCoord - 32, xCoord + 32, yCoord + 32, zCoord + 32));
            if (!lst.isEmpty()) {
                    ...
                    if (!(e instanceof EntityPlayer)) {
                        if (getMRU() > mruUsage) {
                            ...
                            if (dist.getDistance() < rad && dist.getDistance() >= rad - 3) {
                                Vec3 posVector = Vec3.createVectorHelper(
                                    tilePos.x - mobPosition.x,
                                    tilePos.y - mobPosition.y,
                                    tilePos.z - mobPosition.z);
                                e.setPositionAndRotation(
                                    tilePos.x - posVector.xCoord / 1.1D,
                                    tilePos.y - posVector.yCoord / 1.1D,
                                    tilePos.z - posVector.zCoord / 1.1D,
                                    e.rotationYaw,
                                    e.rotationPitch);
                            }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MonsterHolderSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage Per Mob:1",
