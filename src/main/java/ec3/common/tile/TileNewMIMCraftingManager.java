...
public class TileNewMIMCraftingManager extends TileMRUGeneric {
    ...
    public static class CraftingPattern {
        ...
        public CraftingPattern(ItemStack crafting) {
            if (crafting != null && crafting.getItem() instanceof ItemCraftingFrame && crafting.hasTagCompound()) {
                ...
                if (frame != null && frame.getStackInSlot(9) != null
                    && (frame.getStackInSlot(0) != null || frame.getStackInSlot(1) != null
                        || frame.getStackInSlot(2) != null
                        || frame.getStackInSlot(3) != null
                        || frame.getStackInSlot(4) != null
                        || frame.getStackInSlot(5) != null
                        || frame.getStackInSlot(6) != null
                        || frame.getStackInSlot(7) != null
                        || frame.getStackInSlot(8) != null)) {
                    ItemStack[] is = new ItemStack[] { frame.getStackInSlot(0), frame.getStackInSlot(1),
                        frame.getStackInSlot(2), frame.getStackInSlot(3), frame.getStackInSlot(4),
                        frame.getStackInSlot(5), frame.getStackInSlot(6), frame.getStackInSlot(7),
                        frame.getStackInSlot(8) };
                    result = frame.getStackInSlot(9);
                    crafter = crafting;
                    input = is;
                    isOreDict = !crafting.getTagCompound()
        ...
        public boolean isValidRecipe() {
            return result != null && crafter != null
                && input.length == 9
        ...
        public boolean isResultTheSame(ItemStack is) {
            if (is == null || !isValidRecipe()) return false;
    ...
    public void updateEntity() {
        ...
        if (tickTime == 0) {
            tickTime = 20;
            if (parent != null) if (!parent.isParent(this)) parent = null;
        } else--tickTime;
    }
    ...
    public ItemStack findCraftingFrameByRecipe(ItemStack result) {
        for (int i = 0; i < this.allCrafts.size(); ++i) {
            if (allCrafts.get(i)
                .isResultTheSame(result)) return allCrafts.get(i).crafter;
        }
    ...
    public ItemStack findResultByCraftingFrame(ItemStack frame) {
        for (int i = 0; i < this.allCrafts.size(); ++i) {
            if (ItemStack.areItemStacksEqual(allCrafts.get(i).crafter, frame)
                && ItemStack.areItemStackTagsEqual(allCrafts.get(i).crafter, frame)) {
    ...
    public boolean canCraft(ItemStack[] components, ItemStack crafter) {
        ...
        if (!crafter.hasTagCompound()) return false;

        for (int i = 0; i < components.length; ++i) {
            if (parent.retrieveItemStackFromSystem(
                components[i],
                !crafter.getTagCompound()
                    .getBoolean("ignoreOreDict"),
    ...
    public ArrayList<ItemStack> getAllResults() {
        ...
        for (int i = 0; i < allCrafts.size(); ++i) {
            ...
            if (par.isValidRecipe()) ret.add(par.result);
        }
    ...
    public boolean hasRecipe(ItemStack is) {
        ...
        for (int i = 0; i < this.getSizeInventory(); ++i) {
            if (this.getStackInSlot(i) != null && this.getStackInSlot(i)
                .getItem() instanceof ItemCraftingFrame) {
                InventoryCraftingFrame inv = new InventoryCraftingFrame(this.getStackInSlot(i));
                if (inv != null) {
                    if (inv.getStackInSlot(9) != null && inv.getStackInSlot(9)
                        .isItemEqual(is) && ItemStack.areItemStackTagsEqual(is, inv.getStackInSlot(9))) {
                        return true;
                    }
                }
            }
