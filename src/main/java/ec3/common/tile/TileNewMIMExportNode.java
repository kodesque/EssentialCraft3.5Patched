...
public class TileNewMIMExportNode extends TileMRUGeneric {
    ...
    public ISidedInventory getConnectedInventory() {
        ...
        if (this.worldObj.getTileEntity(xCoord + side.offsetX, yCoord + side.offsetY, zCoord + side.offsetZ) != null) {
            ...
            if (tile instanceof ISidedInventory) return (ISidedInventory) tile;
        }
    ...
    public int[] getAccessibleSlots() {
        return getConnectedInventory().getAccessibleSlotsFromSide(
            getRotation().getOpposite()
                .ordinal());
    }
            ...
            for (int i = 0; i < itemsToExport.size(); ++i) {
                for (int j = 0; j < slots.length; ++j) {
                    if (inv.canInsertItem(
                        ...
                        getRotation().getOpposite()
                            .ordinal())) {
                        if (inv.getStackInSlot(slots[j]) == null || (inv.getStackInSlot(slots[j])
                            .isItemEqual(itemsToExport.get(i))
                            && ItemStack.areItemStackTagsEqual(inv.getStackInSlot(slots[j]), itemsToExport.get(i)))) {
                            if (this.getStackInSlot(0) == null || !(this.getStackInSlot(0)
                                .getItem() instanceof ItemFilter)) {
                                if (inv.getStackInSlot(slots[j]) == null) {
                                    ItemStack copied = itemsToExport.get(i)
                                        .copy();
            ...
            for (int i = 0; i < itemsToExport.size(); ++i) {
                for (int j = 0; j < iinv.getSizeInventory(); ++j) {
                    if (iinv.isItemValidForSlot(j, itemsToExport.get(i))) {
                        if (iinv.getStackInSlot(j) == null || (iinv.getStackInSlot(j)
                            .isItemEqual(itemsToExport.get(i))
                            && ItemStack.areItemStackTagsEqual(iinv.getStackInSlot(j), itemsToExport.get(i)))) {
                            if (this.getStackInSlot(0) == null || !(this.getStackInSlot(0)
                                .getItem() instanceof ItemFilter)) {
                                if (iinv.getStackInSlot(j) == null) {
                                    ...
                                    if (iinv.getStackInSlot(j).stackSize + copied.stackSize
                                        <= iinv.getInventoryStackLimit()
