...
import ec3.dummycore.utils.Pair;
...
public class TileNewMIMInventoryStorage extends TileMRUGeneric {
    ...
    public ArrayList<IInventory> getInventories() {
        ...
        for (Pair<Integer[], IInventory> p : counted) {
            if (this.worldObj.blockExists(p.obj1[0], p.obj1[1], p.obj1[2])
                && this.worldObj.getTileEntity(p.obj1[0], p.obj1[1], p.obj1[2]) instanceof IInventory)
    ...
    /**
     * Decreases the stack sizes in possible inventories, until either the stacksize was satisfied, or there are no more
     * stacks of that type in inventories. Also calls a full rebuild on inventories and stacks. Has a null check for
     * inventories. Returns a stk.stackSize if the desired stack was not found.
     *
     * @param stk the stack to pull. Stacksize unbound.
     * @return 0 if the pull was successful, anything greater than 0, but lower than stk.stackSize if the request was
     *         not fully satisfied.
     */
    public int retrieveStack(ItemStack stk, boolean oreDict, boolean actuallyRetrieve) {
        ...
        for (int i = 0; i < items.size(); ++i) {
            ...
            if (is != null) {
                if ((is.isItemEqual(stk) && ItemStack.areItemStackTagsEqual(stk, is))
                    || (oreDict && ECUtils.oreDictionaryCompare(stk, is))) {
                    index = i;
                    break;
                }
            }
        }
        if (stk.stackSize == 0) stk.stackSize = 1;

        int ret = stk.stackSize;
        if (index != -1) {
            fG: for (int i = 0; i < countedT.size(); ++i) {
                if (countedT.get(i) == null) continue;

                for (int j = 0; j < countedT.get(i)
                    .getSizeInventory(); ++j) {
                    ItemStack s = countedT.get(i)
                        .getStackInSlot(j);
                    if (s != null && s.isItemEqual(stk) && ItemStack.areItemStackTagsEqual(stk, s)
                        || (oreDict && ECUtils.oreDictionaryCompare(stk, s))) {
                        if (ret >= s.stackSize) {
                            ret -= s.stackSize;

                            if (actuallyRetrieve) countedT.get(i)
    ...
    public ArrayList<ItemStack> getItemsByName(String namePart) {
        ...
        for (int i = 0; i < items.size(); ++i) {
            ...
            if (stk.getDisplayName()
                .contains(namePart.toLowerCase())) retLst.add(stk);
        }
    ...
    /**
     * Tries to add a given ItemStack to all linked inventories. Will not try to increase sizes first - sadly
     * optimization is more important here.
     *
    ...
    public boolean insertItemStack(ItemStack is) {
        ...
        for (int i = 0; i < counted.size(); ++i) {
            Integer[] coords = counted.get(i)
                .getFirst();
            IInventory inv = counted.get(i)
                .getSecond();

            if (!this.worldObj.blockExists(coords[0], coords[1], coords[2])) continue;
    ...
    /**
     * Re-calculates all the items there are. I wish there would be a better way to do this. Especially, if I didn't
     * have to do this every tick, since it is pretty resource-intensive. However, if I do not do this every tick then
     * dupes are possible.
     */
    public void rebuildItems() {
        ...
        for (int i = 0; i < countedT.size(); ++i) {
            ...
            for (int j = 0; j < countedT.get(i)
                .getSizeInventory(); ++j) {
                ItemStack stk = countedT.get(i)
                    .getStackInSlot(j);
                if (stk != null && stk.stackSize > 0) {
                    String id = GameRegistry.findUniqueIdentifierFor(stk.getItem())
                        .toString() + "@"
                        + stk.getItemDamage();
                    if (stk.getTagCompound() == null || stk.getTagCompound()
                        .hasNoTags()) {
        ...
        for (int i = 0; i < ids.size(); ++i) {
            ItemStack cID = foundByID.get(ids.get(i))
                .copy();
        ...
        if (items.size() == oldCopy.size()) {
            for (int i = 0; i < oldCopy.size(); ++i) {
                if ((items.get(i) == null && oldCopy.get(i) == null) || (items.get(i)
                    .isItemEqual(oldCopy.get(i)) && ItemStack.areItemStackTagsEqual(items.get(i), oldCopy.get(i))
                    && items.get(i).stackSize == oldCopy.get(i).stackSize)) continue;
                else requireSync = true;
            }
    ...
    public void updateEntity() {
        ...
        if (updateTime <= 0) rebuildInventories();
        else--updateTime;

        if (!this.worldObj.isRemote) rebuildItems();
    }

    /**
     * I wish there would be a better way of doing this, however, vanilla won't sync items in inventories unless they
     * are not opened.
     */
