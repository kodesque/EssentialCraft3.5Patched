...
public class TilePotionSpreader extends TileMRUGeneric {
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            if (this.potionID == -1) for (int i = 1; i < 9; ++i) {
                ...
                List<EntityLivingBase> lst = this.worldObj.getEntitiesWithinAABB(
                    EntityLivingBase.class,
                    AxisAlignedBB
                        .getBoundingBox(xCoord - 8, yCoord - 8, zCoord - 8, xCoord + 9, yCoord + 9, zCoord + 9));
                if (!lst.isEmpty() && !this.worldObj.isRemote) {
                    ...
                    for (int i = 0; i < lst.size(); ++i) {
                        ...
                        if (potionID == Potion.heal.id) {
                            ...
                            shouldUsePotion = (!base.isEntityUndead()
                                && base.getHealth() + healAmount <= base.getMaxHealth())
                                || (base.isEntityUndead() && base.hurtResistantTime == 0 && base.hurtTime == 0);
                        } else if (potionID == Potion.harm.id) {
                            float damageAmount = (float) (6 << effect.getAmplifier());
                            shouldUsePotion = (base.isEntityUndead()
                                && base.getHealth() + damageAmount <= base.getMaxHealth())
                                || (!base.isEntityUndead() && base.hurtResistantTime == 0 && base.hurtTime == 0);
                        } else {
                        ...
                        if (shouldUsePotion && this.getMRU() >= mruUsage) {
                            ...
                            for (int i1 = 0; i1 < 100; ++i1) MiscUtils.spawnParticlesOnServer(
                                "mobSpell",
                                (float) (base.posX + MathUtils.randomFloat(this.worldObj.rand)),
                                (float) (base.posY + 1 + MathUtils.randomFloat(this.worldObj.rand)),
                                (float) (base.posZ + MathUtils.randomFloat(this.worldObj.rand)),
                                f,
                                f1,
                                f2);
                        }
                    ...
                    if (haveUsedPotion) --this.potionUseTime;
                }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "PotionSpreaderSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage Per Mob:250",
