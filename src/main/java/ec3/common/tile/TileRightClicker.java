...
public class TileRightClicker extends TileMRUGeneric {
    ...
    public void finishClick(int slot, boolean setupAll) {
        ...
        for (int i = 0; i < cycle; ++i) {
            ...
            if (fakePlayer.getCurrentEquippedItem() != null) {
                ItemStack setted = fakePlayer.inventory.getCurrentItem()
                    .copy();
                if (setted != null && setted.getItem() instanceof IItemRequiresMRU) {
                    ...
                    if (thM < thMM) {
                        ...
                        if (diff <= current) {
                            ...
                            if (this.getMRU() < 0) this.setMRU(0);
                        } else {
                ...
                if (setted.stackSize <= 0) this.setInventorySlotContents(1 + i, null);
            } else this.setInventorySlotContents(1 + i, null);
    ...
    public void setupFake(int slot, boolean setupAll) {
        ...
        fakePlayer = (FakePlayer) new WeakReference(
            FakePlayerFactory.get((WorldServer) this.worldObj, ECUtils.EC3FakePlayerProfile)).get();
        fakePlayer.inventory.mainInventory = new ItemStack[9];
        fakePlayer.inventory.currentItem = 0;
        if (setupAll) {
            for (int i = 0; i < 9; ++i) {
                fakePlayer.inventory.setInventorySlotContents(
                    fakePlayer.inventory.currentItem + i,
                    this.getStackInSlot(1 + i) == null ? null
                        : this.getStackInSlot(1 + i)
                            .copy());
                if (i + 1 != slot) this.setInventorySlotContents(i + 1, null);
            }
        }

        if (this.getStackInSlot(slot) != null) {
            ItemStack setted = this.getStackInSlot(slot)
                .copy();
            if (setted != null && setted.getItem() instanceof IItemRequiresMRU) {
                IItemRequiresMRU iReq = (IItemRequiresMRU) setted.getItem();
                int current = iReq.getMRU(setted);
            ...
            if (this.rotation == 2) rotation = 0;
            if (this.rotation == 3) rotation = 90;
            if (this.rotation == 4) rotation = 180;
            if (this.rotation == 5) rotation = 270;
            fakePlayer.setPositionAndRotation(
                xCoord + 0.5D + r.offsetX,
                yCoord + 0.5D + r.offsetY,
                zCoord + 0.5D + r.offsetZ,
                rotation,
                this.rotation == 0 ? -90 : this.rotation == 1 ? 90 : 0);
        }
    }
    ...
    public boolean rightClick(boolean sneak) {
        ...
        try {
            PlayerInteractEvent event = ForgeEventFactory
                .onPlayerInteract(fakePlayer, Action.RIGHT_CLICK_AIR, x, y, z, faceDir.ordinal(), worldObj);
            if (event.isCanceled()) return false;

            Block block = worldObj.getBlock(x, y, z);
            List<EntityLivingBase> detectedEntities = worldObj.getEntitiesWithinAABB(
            ...
            if (stack.getItem()
                .itemInteractionForEntity(stack, fakePlayer, (EntityLivingBase) entity)) return true;
            if (entity instanceof EntityAnimal && ((EntityAnimal) entity).interact(fakePlayer)) return true;

            if (stack.getItem()
                .onItemUseFirst(stack, fakePlayer, worldObj, x, y, z, faceDir.ordinal(), dx, dy, dz)) return true;
            if (!worldObj.isAirBlock(x, y, z)
                && block.onBlockActivated(worldObj, x, y, z, fakePlayer, faceDir.ordinal(), dx, dy, dz)) return true;
            ...
            if (stack != null) {
                ...
                if (stack.getItem()
                    .onItemUse(stack, fakePlayer, worldObj, useX, useY, useZ, faceDir.ordinal(), dx, dy, dz))
                    return true;
            }

            ItemStack copy = stack.copy();
            fakePlayer.setCurrentItemOrArmor(
                0,
                stack.getItem()
                    .onItemRightClick(stack, worldObj, fakePlayer));
    ...
    public void updateEntity() {
        ...
        if (wasPowered && !this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) wasPowered = false;

        if (wasPowered) {
            this.worldObj.spawnParticle(
                "reddust",
                xCoord + 0.5D + MathUtils.randomDouble(this.worldObj.rand) / 1.5D,
                yCoord + 0.5D + MathUtils.randomDouble(this.worldObj.rand) / 1.5D,
                zCoord + 0.5D + MathUtils.randomDouble(this.worldObj.rand) / 1.5D,
                1,
                0,
                0);
        }
    ...
    public void onDataPacket(NetworkManager net, S35PacketUpdateTileEntity pkt) {
        if (net.getNetHandler() instanceof INetHandlerPlayClient) if (pkt.func_148853_f() == -10) {
            boolean reRender = pkt.func_148857_g()
                .getInteger("rotation") != this.rotation
                || ItemStack.loadItemStackFromNBT(
                    NBTTagList.class.cast(
                        pkt.func_148857_g()
                            .getTagList("Items", 10))
                        .getCompoundTagAt(10))
                    != this.getStackInSlot(10);
            this.readFromNBT(pkt.func_148857_g());
            if (reRender) this.worldObj
                .markBlockRangeForRenderUpdate(xCoord - 1, yCoord - 1, zCoord - 1, xCoord + 1, yCoord + 1, zCoord + 1);
        }
    }
