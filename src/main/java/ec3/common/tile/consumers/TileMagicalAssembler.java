...
public class TileMagicalAssembler extends TileMRUGeneric {
    ...
    public void readFromNBT(NBTTagCompound i) {
        ...
        if (unformedItems != null) for (int j = 0; j < unformedItems.tagCount(); ++j) {
        ...
        if (this.getStackInSlot(1) != null) {
            ...
            if (this.currentRecipe != -1) this.formRequiredComponents();
        }
    }
    ...
    public void writeToNBT(NBTTagCompound i) {
        ...
        for (int j = 0; j < this.requiredItemsToCraft.size(); ++j) {
            ...
            this.requiredItemsToCraft.get(j)
    ...
    public int getRequiredMRUToCraft() {
        ...
        if (currentSelectedOne instanceof RadiatingChamberRecipe)
            return (int) (((RadiatingChamberRecipe) currentSelectedOne).mruRequired
                * ((RadiatingChamberRecipe) currentSelectedOne).costModifier);
    ...
    public void updateEntity() {
        for (int i = 0; i < mirrors.size(); ++i) {
            ...
            if (tile == null || !(tile instanceof TileMagicalMirror)) mirrors.remove(i);
        }
        super.updateEntity();
        ...
        if (this.getStackInSlot(1) != null) {
            ...
            if (this.currentCraft != null && !this.isWorking
                ...
                for (int i = 0; i < mirrors.size(); ++i) {
                    ...
                    if (tile == null || !(tile instanceof TileMagicalMirror)) mirrors.remove(i);
                    else((TileMagicalMirror) tile).begin(this);
                }
                ...
                || !this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
                    ...
                    for (int i = 0; i < mirrors.size(); ++i) {
                        ...
                        tile = worldObj.getTileEntity((int) coord.x, (int) coord.y, (int) coord.z);
                        if (tile == null || !(tile instanceof TileMagicalMirror)) mirrors.remove(i);
                        else((TileMagicalMirror) tile).end(this);
                    }
                }
            if (this.currentCraft != null && this.isWorking) {
                for (int i = 0; i < mirrors.size(); ++i) {
                    ...
                    tile = worldObj.getTileEntity((int) coord.x, (int) coord.y, (int) coord.z);
                    if (tile == null || !(tile instanceof TileMagicalMirror)) mirrors.remove(i);
                    else((TileMagicalMirror) tile).pulse(this);
                }
            }
            if (this.isWorking && this.currentCraft != null
                && this.worldObj.getWorldTime() % 20 == 0
                && !this.requiredItemsToCraft.isEmpty()
                && (this.getStackInSlot(17) == null || (this.getStackInSlot(17)
                    .isItemEqual(currentCraft)
                    && this.getStackInSlot(17).stackSize + 1 < this.getStackInSlot(17)
                        .getMaxStackSize() + 1))
                && this.getMRU() - getRequiredMRUToCraft() >= 0
                ...
                List<UnformedItemStack> hasItemsCopy = new ArrayList<UnformedItemStack>(
                    this.requiredItemsToCraft.size());
                ...
                if (hasItems.equals(requiredItemsToCraft)) {
                    ...
                    if (this.getStackInSlot(17) == null) this.setInventorySlotContents(17, this.currentCraft.copy());
                    else this.getStackInSlot(17).stackSize += this.currentCraft.stackSize;
                    this.isWorking = false;
        ...
        if (this.currentRecipe != -1) {
            ...
            for (int i = 0; i < rec.getRecipeSize(); ++i) {
                ...
                if (rec instanceof ShapelessOreRecipe) {
                    ...
                    UnformedItemStack uis = new UnformedItemStack(
                        sor.getInput()
                            .get(i));
                    this.requiredItemsToCraft.add(uis);
                }
        ...
        if (is != null) {
            ...
            for (IRecipe scr : rec) {
                if (scr instanceof ShapedRecipes) {
                    ...
                    if (Arrays.asList(sscr.recipeItems)
                        .contains(is)) lstCR.add(sscr);
                }
                if (scr instanceof ShapelessRecipes) {
                    ...
                    for (int i = 0; i < sscr.recipeItems.size(); ++i) {
                        ...
                        if (s != null && s.isItemEqual(is)) lstCR.add(sscr);
                    }
                }
                if (scr instanceof ShapedOreRecipe) {
                    ...
                    for (Object o : sor.getInput()) {
                        ...
                        if (o instanceof ItemStack) {
                            ...
                            if (compared.isItemEqual(is)) lstCR.add(sor);
                        }
                        ...
                        if (o instanceof ItemStack[]) {
                            for (ItemStack s : (ItemStack[]) o) {
                                if (s.isItemEqual(is)) lstCR.add(sor);
                            }
                        }
