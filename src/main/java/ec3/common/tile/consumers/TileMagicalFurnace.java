...
public class TileMagicalFurnace extends TileMRUGeneric {
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) smelt();
    }
    ...
    public boolean isStructureCorrect() {
        ...
        flag = this.worldObj.getBlock(xCoord + 2, yCoord, zCoord + 2) == ECBlocks.voidStone
            ...
            && this.worldObj.getBlock(xCoord + 2, yCoord, zCoord - 2) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord - 2, yCoord, zCoord - 2) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord + 2, yCoord + 1, zCoord + 2) == ECBlocks.heatGenerator
            && this.worldObj.getBlock(xCoord - 2, yCoord + 1, zCoord + 2) == ECBlocks.heatGenerator
            && this.worldObj.getBlock(xCoord + 2, yCoord + 1, zCoord - 2) == ECBlocks.heatGenerator
            && this.worldObj.getBlock(xCoord - 2, yCoord + 1, zCoord - 2) == ECBlocks.heatGenerator;
        return flag;
        ...
        if (this.isStructureCorrect() && this.getMRU() > 0 && smeltingStack != null && smeltingItem != null) {
            ...
            if (this.progressLevel >= smeltingTime) {
                ...
                ItemStack s = FurnaceRecipes.smelting()
                    .getSmeltingResult(mainSmelting);
                s.stackSize += 1;
                ...
                if (!this.worldObj.isRemote && this.worldObj.rand.nextFloat() <= chanceToDoubleOutput) {
                    EntityItem smelted = new EntityItem(
                        this.worldObj,
                        this.xCoord - 1.5,
                        this.yCoord + 2.15,
                        this.zCoord - 1.5,
                ...
                EntityItem smelted = new EntityItem(
                    ...
                    this.zCoord + 2.5,
                    s.copy());
                if (!this.worldObj.isRemote) this.worldObj.spawnEntityInWorld(smelted);
                smelted.motionX = 0;
                smelted.motionY = 0;
                smelted.motionZ = 0;
                if (!this.worldObj.isRemote && this.worldObj.rand.nextFloat() <= chanceToDoubleSlags) {
                    EntityItem slag = new EntityItem(
                        this.worldObj,
                        this.xCoord - 1.5,
                        this.yCoord + 2.15,
                        this.zCoord + 2.5,
                ...
                EntityItem slag = new EntityItem(
                    ...
                    this.zCoord - 1.5,
                    new ItemStack(ECItems.magicalSlag));
                if (!this.worldObj.isRemote) this.worldObj.spawnEntityInWorld(slag);
                smelted.motionX = 0;
                smelted.motionY = 0;
                smelted.motionZ = 0;
                s.stackSize = 0;
        ...
        if (!l.isEmpty()) {
            for (int i = 0; i < l.size(); ++i) {
                ...
                if (FurnaceRecipes.smelting()
                    .getSmeltingResult(item.getEntityItem()) != null) return item;
            }
        }
        return ret;
    }
    ...
    public ItemStack getSmeltingStack() {
        ...
        if (!l.isEmpty()) {
            ...
            for (int i = 0; i < l.size(); ++i) {
                EntityItem item = l.get(i);
                if (FurnaceRecipes.smelting()
                    .getSmeltingResult(item.getEntityItem()) != null) canBeSmelted.add(item.getEntityItem());
            }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MagicalFurnaceSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC * 10, "MRU Usage:25",
                    "Ticks required to smelt 1 item:20", "Chance to double the outcome:0.3",
