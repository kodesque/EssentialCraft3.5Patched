...
public class TileMagicalQuarry extends TileMRUGeneric {
    ...
    public void updateEntity() {
        if (this.syncTick == 10) this.syncTick = 0;
        super.updateEntity();
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) mine();
        if (!this.worldObj.isRemote) collectItems();
    ...
    public boolean hasInventoryUpgrade() {
        ...
        for (int i = 0; i < this.getSizeInventory(); ++i) {
            if (this.getStackInSlot(i) != null && this.getStackInSlot(i)
                .isItemEqual(s)) return true;
        }
        return false;
    }
    ...
    public boolean hasSilkyUpgrade() {
        if (hasSmeltingUpgrade()) return false;
    ...
    public boolean hasFortuneUpgrade() {
        if (hasSilkyUpgrade()) return false;
    ...
    public boolean hasSpaceInInv(IInventory inv) {
        for (int i = 0; i < inv.getSizeInventory(); ++i) {
            if (inv.getStackInSlot(i) == null) return true;
        }
        return false;
    }
    ...
    public boolean canMineBlock(Block b) {
        ...
        if (b == Blocks.obsidian && !this.hasMiningUpgrade()) return false;
        return true;
    }
    ...
    public int determineFortune() {
        ...
        for (int i = 0; i < this.getSizeInventory(); ++i) {
            if (this.getStackInSlot(i) != null && this.getStackInSlot(i)
                .isItemEqual(s)) fortune += this.getStackInSlot(i).stackSize;
        }

        if (fortune <= 128) fortune = 5;
        if (fortune <= 64) fortune = 4;
        if (fortune <= 32) fortune = 3;
        if (fortune <= 16) fortune = 2;
        if (fortune <= 8) fortune = 1;
    ...
    public boolean mineBlock(Block b) {
        if (canMineBlock(b)) {
            ...
            EssentialCraftCore.network.sendToAllAround(
                packet,
                new TargetPoint(
            ...
            if (shouldInstaMine(b)) {
                ...
                if (this.progressLevel >= required) {
                    ...
                    if (this.hasMiningUpgrade()) {
                        quarryFakePlayer.setCurrentItemOrArmor(0, new ItemStack(ECItems.wind_elemental_pick));
                    } else {
                        quarryFakePlayer.setCurrentItemOrArmor(0, new ItemStack(ECItems.weak_elemental_pick));
                    }

                    if (this.hasFortuneUpgrade()) quarryFakePlayer.getCurrentEquippedItem()
                        .addEnchantment(Enchantment.fortune, this.determineFortune());
                    if (this.hasSilkyUpgrade()) quarryFakePlayer.getCurrentEquippedItem()
                        .addEnchantment(Enchantment.silkTouch, 1);
                    ...
                    b.harvestBlock(
                        ...
                        miningY,
                        miningZ,
                        this.worldObj.getBlockMetadata(miningX, miningY, miningZ));

                    this.worldObj.setBlock(miningX, miningY, miningZ, Blocks.air, 0, 3);
                    if (generatesCorruption) ECUtils.increaseCorruptionAt(
                        worldObj,
                        xCoord,
                        yCoord,
                        zCoord,
                        this.worldObj.rand.nextInt(genCorruption));
    ...
    public boolean isMainColomnMined() {
        ...
        while (--r >= 0) {
            ...
            if ((b != null && b.getBlockHardness(worldObj, xCoord, r, zCoord) != -1
                && b != Blocks.air
                && !(b instanceof BlockLiquid && ignoreLiquids)
                && this.canMineBlock(b)) && this.canMineBlock(b)) {
                return false;
            }
        }
        return true;
    ...
    public boolean isRowMined() {
        ...
        for (int x = -rad; x <= rad; ++x) {
            for (int z = -rad; z <= rad; ++z) {
                ...
                if (b != null && b.getBlockHardness(worldObj, xCoord + x, miningY, zCoord + z) != -1
                    && b != Blocks.air
                    && !(b instanceof BlockLiquid && ignoreLiquids)
                    && this.canMineBlock(b)) {
                    return false;
    ...
    public void mine() {
        if (canWork() && !this.worldObj.isRemote) {
            if (isMainColomnMined()) {
                    ...
                    Fort: for (int x = -rad; x <= rad; ++x) {
                        for (int z = -rad; z <= rad; ++z) {
                            if (worldObj.checkChunksExist(
                                ...
                                xCoord + x + 1,
                                miningY + 1,
                                zCoord + z + 1) && worldObj.blockExists(xCoord + x, miningY, zCoord + z)
                                && worldObj.getBlock(xCoord + x, miningY, zCoord + z) != null
                                && worldObj.getBlock(xCoord + x, miningY, zCoord + z)
                                    .getBlockHardness(worldObj, xCoord + x, miningY, zCoord + z) != -1
                                && worldObj.getBlock(xCoord + x, miningY, zCoord + z) != Blocks.air
                                && !(worldObj.getBlock(xCoord + x, miningY, zCoord + z) instanceof BlockLiquid)) {
                ...
                if (worldObj
                    .checkChunksExist(miningX - 1, miningY - 1, miningZ - 1, miningX + 1, miningY + 1, miningZ + 1)
                    && worldObj.blockExists(miningX, miningY, miningZ)) {
                    if (worldObj.getBlock(miningX, miningY, miningZ) != null
                        && worldObj.getBlock(miningX, miningY, miningZ) != Blocks.air
                        && !(worldObj.getBlock(miningX, miningY, miningZ) instanceof BlockLiquid)) {
    ...
    public void collectItems() {
        List<EntityItem> l = this.worldObj.getEntitiesWithinAABB(
            EntityItem.class,
            AxisAlignedBB.getBoundingBox(miningX, miningY, miningZ, miningX + 1, miningY + 1, miningZ + 1)
                .expand(4D, 2D, 4D));
        if (!l.isEmpty()) {
            for (int i = 0; i < l.size(); ++i) {
                ...
                if (this.hasSmeltingUpgrade() && s != null) {
                    ItemStack forged = FurnaceRecipes.smelting()
                        .getSmeltingResult(s);
                    if (forged != null) {
    ...
    public boolean voidItemStack(ItemStack s) {
        return s == null
            || voidList.contains(s.getItem() instanceof ItemBlock ? Block.getBlockFromItem(s.getItem()) : s.getItem());
    }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MagicalQuarrySettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage:8",
                    "Should not mine liquids:true", "Can this device actually generate corruption:true",
                    "The amount of corruption generated each tick(do not set to 0!):2",
                    "Efficency added to the device per upgrade:0.5", "Block hardness modifier:9.0" },
                "");
            String dataString = "";
    ...
    public static GameProfile quarryFakePlayerProfile = new GameProfile(
        UUID.fromString("5cd89d0b-e9ba-0000-89f4-badbb05963dd"),
        "[EC3]Quarry");
}
