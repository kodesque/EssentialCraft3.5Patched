...
public class TileMagicalTeleporter extends TileMRUGeneric {
    ...
    public void updateEntity() {
        ...
        super.updateEntity();
        this.spawnParticles();
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) this.tryTeleport();
    ...
    public boolean isStructureCorrect() {
            ...
            for (int z = -2; z <= 2; ++z) {
                if (((x == 2 || x == -2) && z == 0) || ((z == 2 || z == -2) && x == 0)) {
                    ItemStack s = new ItemStack(
                        this.worldObj.getBlock(xCoord + x, yCoord, zCoord + z),
                        this.worldObj.getBlockMetadata(xCoord + x, yCoord, zCoord + z),
                        1);
                    ItemStack c = new ItemStack(ECBlocks.magicPlating, 0, 1);
                    ...
                    if (x == 0 && z == 0) {
                        ...
                        flag = this.worldObj.getBlock(xCoord + x, yCoord, zCoord + z) == ECBlocks.voidStone;
                        if (!flag) {
                            return false;
                        }
                    }
        ...
        flag = this.worldObj.getBlock(xCoord + 1, yCoord + 1, zCoord + 2) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord + 1, yCoord + 2, zCoord + 2) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord + 2, yCoord + 1, zCoord + 1) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord + 2, yCoord + 2, zCoord + 1) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord - 2, yCoord + 1, zCoord + 1) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord - 2, yCoord + 2, zCoord + 1) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord - 1, yCoord + 1, zCoord + 2) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord - 1, yCoord + 2, zCoord + 2) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord + 2, yCoord + 1, zCoord - 1) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord + 2, yCoord + 2, zCoord - 1) == ECBlocks.voidStone
            && this.worldObj.getBlock(xCoord + 1, yCoord + 1, zCoord - 2) == ECBlocks.voidStone
    ...
    public void tryTeleport() {
        if (this.hasRequiredItemToTeleport() && this.hasPlayer()
            && this.getMRU() >= this.getTPCost()
            && this.isStructureCorrect()) {
            EntityPlayer player = this.getPlayer();
            ++this.progressLevel;
            this.worldObj
                .playSoundAtEntity(player, "minecart.base", 0.1F, 0.8F + ((float) this.progressLevel / teleportTime));
                ...
                if (!this.worldObj.isRemote) {
                    ...
                    for (int i = 0; i < 20; ++i) {
                        this.worldObj.playSoundAtEntity(
                            ...
                            1.0F,
                            0.5F + MathUtils.randomFloat(this.worldObj.rand));
                    }
                    ...
                    if (currentPlayerDim != newDim && !player.worldObj.isRemote && allowDimensionalTeleportation) {
                        ...
                        p_72356_1_.playerNetServerHandler.sendPacket(
                            new S07PacketRespawn(
                                p_72356_1_.dimension,
                                p_72356_1_.worldObj.difficultySetting,
                                p_72356_1_.worldObj.getWorldInfo()
                                    .getTerrainType(),
                        ...
                        p_72356_1_.playerNetServerHandler.setPlayerLocation(
                            p_72356_1_.posX,
                            p_72356_1_.posY,
                            p_72356_1_.posZ,
                            p_72356_1_.rotationYaw,
                            p_72356_1_.rotationPitch);
                        p_72356_1_.theItemInWorldManager.setWorld(worldserver1);
                        mcServer.getConfigurationManager()
                            .updateTimeAndWeatherForPlayer(p_72356_1_, worldserver1);
                        mcServer.getConfigurationManager()
                            .syncPlayerInventory(p_72356_1_);
                        Iterator<?> iterator = p_72356_1_.getActivePotionEffects()
                            .iterator();

                        while (iterator.hasNext()) {
                            PotionEffect potioneffect = (PotionEffect) iterator.next();
                            p_72356_1_.playerNetServerHandler
                                .sendPacket(new S1DPacketEntityEffect(p_72356_1_.getEntityId(), potioneffect));
                        }
                        FMLCommonHandler.instance()
                            .firePlayerChangedDimensionEvent(p_72356_1_, j, newDim);
                    }
    ...
    public void transferEntityToWorld(Entity p_82448_1_, int p_82448_2_, WorldServer p_82448_3_,
        ...
        if (p_82448_2_ != 1) {
            ...
            if (p_82448_1_.isEntityAlive()) {
                p_82448_1_
                    .setLocationAndAngles(d0, p_82448_1_.posY, d1, p_82448_1_.rotationYaw, p_82448_1_.rotationPitch);
    ...
    public int getTPCost() {
        ...
        if (this.worldObj.provider.dimensionId != dim) ret = (int) cfgMaxMRU;
        return ret;
    }

    @SuppressWarnings("unchecked")
    public EntityPlayer getPlayer() {
        List<EntityPlayer> l = this.worldObj.getEntitiesWithinAABB(
            EntityPlayer.class,
    ...
    public int getDimensionToTP() {
        ...
        return MiscUtils.getStackTag(s)
            .getInteger("dim");
    }
    ...
    public boolean hasRequiredItemToTeleport() {
        return this.getStackInSlot(1) != null && this.getStackInSlot(1)
        ...
        if (this.hasPlayer()) {
            ...
            for (int i = 0; i < (this.progressLevel / 5); ++i) {
                this.worldObj.spawnParticle(
                    "reddust",
                    p.posX + MathUtils.randomFloat(this.worldObj.rand) / 2,
                    p.posY + MathUtils.randomFloat(this.worldObj.rand) * 2 - 1,
                    p.posZ + MathUtils.randomFloat(this.worldObj.rand) / 2,
                    0,
                    0,
                    1);
                this.worldObj.spawnParticle(
                    "reddust",
                    this.xCoord + 0.5 + MathUtils.randomFloat(this.worldObj.rand) * 2,
                    this.yCoord + 3,
                    this.zCoord + 0.5 + MathUtils.randomFloat(this.worldObj.rand) * 2,
                    0,
                    0,
                    1);
                this.worldObj.spawnParticle(
                    "reddust",
                    this.xCoord + 0.5 + 2,
                    this.yCoord + 2 + MathUtils.randomFloat(this.worldObj.rand),
                    this.zCoord + 0.5 + MathUtils.randomFloat(this.worldObj.rand) * 2,
                    0,
                    0,
                    1);
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MagicalTeleporterSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC * 10, "MRU Usage per block:500",
