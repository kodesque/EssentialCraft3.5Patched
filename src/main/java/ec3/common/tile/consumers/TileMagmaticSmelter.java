    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            if (!worldObj.isRemote) {
                if (FluidContainerRegistry.getFluidForFilledItem(getStackInSlot(1)) != null
                    ...
                    } else if (lavaTank.getFluidAmount() != 8000 && lavaTank.getFluid()
                        .isFluidEqual(FluidContainerRegistry.getFluidForFilledItem(getStackInSlot(1)))) {
                            lavaTank.fill(FluidContainerRegistry.getFluidForFilledItem(getStackInSlot(1)), true);
                            setInventorySlotContents(2, consumeItem(getStackInSlot(1)));
                            this.decrStackSize(1, 1);
                        }
            ...
            if (ore != null && !this.worldObj.isRemote) {
                ...
                if (oreIds.length > 0) oreName = OreDictionary.getOreName(oreIds[0]);
                int metadata = -1;
                ...
                if (metadata != -1) {
                    if (this.getStackInSlot(4) == null) {
                        if (this.getMRU() - mruUsage >= 0 && this.lavaTank != null
                            && this.lavaTank.getFluid() != null
                            && this.lavaTank.getFluid()
                                .getFluid() == FluidRegistry.LAVA
                            && this.lavaTank.getFluidAmount() > 0) {
                            this.setMRU(this.getMRU() - mruUsage);
                            if (this.worldObj.rand.nextFloat() <= 0.1F) this.drain(1, true);
                            this.worldObj.spawnParticle(
                                "flame",
                                xCoord + 0.5D + MathUtils.randomDouble(this.worldObj.rand) / 2.2D,
                                yCoord,
                                zCoord + 0.5D + MathUtils.randomDouble(this.worldObj.rand) / 2.2D,
                                0,
                                -0.1D,
                                0);
                            ++this.progressLevel;
                            ...
                            if (this.progressLevel >= oreSmeltingTime) {
                                ...
                                this.setInventorySlotContents(
                                    4,
                                    new ItemStack(ECItems.magicalAlloy, suggestedStackSize, metadata));
                                this.progressLevel = 0;
                                if (this.getStackInSlot(7) == null) {
                                    this.setInventorySlotContents(7, new ItemStack(ECItems.magicalSlag, 1, 0));
                                } else {
                                    if (this.getStackInSlot(7)
                                        .getItem() == ECItems.magicalSlag && this.getStackInSlot(7).stackSize < 64) {
                    ...
                    } else if (this.getStackInSlot(4)
                        .getItem() == ECItems.magicalAlloy
                        && this.getStackInSlot(4)
                            .getItemDamage() == metadata
                        && this.getStackInSlot(4).stackSize + 2 <= this.getStackInSlot(4)
                            .getMaxStackSize()
                        && this.getStackInSlot(4).stackSize + 4 <= this.getInventoryStackLimit()) {
                            if (this.getMRU() - mruUsage >= 0 && this.lavaTank != null
                                && this.lavaTank.getFluid() != null
                                && this.lavaTank.getFluid()
                                    .getFluid() == FluidRegistry.LAVA
                                ...
                                if (this.progressLevel >= oreSmeltingTime) {
                                    ...
                                    if (is.stackSize > is.getMaxStackSize()) is.stackSize = is.getMaxStackSize();
                                    ...
                                    if (this.getStackInSlot(7) == null) {
                                        ...
                                        if (this.getStackInSlot(7)
                                            .getItem() == ECItems.magicalSlag
                                            && this.getStackInSlot(7).stackSize < 64) {
                                            ItemStack slagIS = this.getStackInSlot(7);
                                            slagIS.stackSize += 1;
                                            this.setInventorySlotContents(7, slagIS);
                                        }
            ...
            } else this.progressLevel = 0;

            ItemStack alloy = this.getStackInSlot(5);
            if (alloy != null && this.getStackInSlot(5)
                .getItem() == ECItems.magicalAlloy) {
                ...
                if (outputName.isEmpty()) suggestedIngotName = "ingot" + oreName.substring(3);
                else suggestedIngotName = outputName;
                List<ItemStack> oreLst = OreDictionary.getOres(suggestedIngotName);

                if (oreLst != null && !oreLst.isEmpty() && !this.worldObj.isRemote) {
                    ItemStack ingotStk = oreLst.get(0)
                        .copy();
                    ...
                    } else if (this.getStackInSlot(6)
                        .isItemEqual(ingotStk)
                        && this.getStackInSlot(6).stackSize + 2 <= this.getStackInSlot(6)
                            .getMaxStackSize()
                        && this.getStackInSlot(6).stackSize + 2 <= this.getInventoryStackLimit()) {
                            if (this.getMRU() - smeltMRUUsage >= 0) {
                                this.setMRU(this.getMRU() - smeltMRUUsage);
                                this.worldObj.spawnParticle(
                                    "flame",
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "MagmaticSmelterySettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage when smelting Ores:50",
                    "Can this device actually generate corruption:false",
                    "The amount of corruption generated each tick(do not set to 0!):3",
                    "MRU Usage when smelting Alloys:30", "Ticks required to smelt an ore into Alloy:400",
                    "Ticks required to smelt an alloy:40", "A chance to quadriple the outcome:0.33" },
                "");
            String dataString = "";
