...
public class TileCrystalFormer extends TileMRUGeneric {
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) this.doWork();
        this.spawnParticles();
    }
        ...
        if (canWork()) {
            if (this.getMRU() > mruUsage) {
                if (!this.worldObj.isRemote) {
                    if (this.setMRU(this.getMRU() - mruUsage)) ++this.progressLevel;
    ...
    public void createItem() {
        ...
        ItemStack crystal = new ItemStack(ECBlocks.elementalCrystal, 1, 0);
    ...
    public boolean canWork() {
        ...
        if (s[0] == null) {
            if (s[1] != null && s[2] != null && s[3] != null && s[4] != null && s[5] != null && s[6] != null) {
                if (s[1].getItem() == Items.water_bucket && s[2].getItem() == Items.water_bucket
    ...
    public boolean isGlassBlock(ItemStack is) {
        ...
        if (is.getItem() == Item.getItemFromBlock(Blocks.glass)
            || is.getItem() == Item.getItemFromBlock(Blocks.stained_glass)) return true;
        ...
        if (OreDictionary.getOreIDs(is) != null && OreDictionary.getOreIDs(is).length > 0) {
            for (int i = 0; i < OreDictionary.getOreIDs(is).length; ++i) {
                ...
                if (name.equals("blockGlass")) return true;
            }
        }
        ...
        if (this.canWork() && this.getMRU() > 0) {
            for (int o = 0; o < 10; ++o) {
                this.worldObj.spawnParticle(
                    "reddust",
                    xCoord + 0.1D + this.worldObj.rand.nextDouble() / 1.1D,
                    yCoord + ((float) o / 10),
                    zCoord + 0.1D + this.worldObj.rand.nextDouble() / 1.1D,
                    -1.0D,
                    1.0D,
                    1.0D);
            }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "CrystalFormerSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.DEVICE_MAX_MRU_GENERIC, "MRU Usage:100",
                    "Ticks required to create a crystal:1000", "Can this device actually generate corruption:true",
