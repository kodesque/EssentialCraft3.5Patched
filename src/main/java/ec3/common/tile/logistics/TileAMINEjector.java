...
public class TileAMINEjector extends TileMINEjector {
    ...
    public boolean ejectItem(IInventory inv, int slotNum) {
        ...
        if (ejected != null) {
            if (this.getStackInSlot(slotNum + 64) == null) {
                ItemStack inserted = ejected.copy();
                inserted.stackSize = 1;
                this.setInventorySlotContents(slotNum + 64, inserted);
                inv.decrStackSize(slotNum, 1);
                return true;
            } else {
                if (this.getStackInSlot(slotNum + 64).stackSize + 1 <= this.getStackInSlot(slotNum + 64)
                    .getMaxStackSize()) {
    ...
    public int findItemToEject(IInventory inv, int prevSlotnum) {
        if (inv != null) for (int i = prevSlotnum; i < inv.getSizeInventory() && i < 64; ++i) {
            ItemStack filter = this.getStackInSlot(i);
            ItemStack stk = inv.getStackInSlot(i);
            if (stk == null || filter == null) continue;
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            ...
            if (inv != null) for (int i = 0; i < inv.getSizeInventory(); ++i)
                if (this.findItemToEject(inv, i) != -1) this.ejectItem(inv, this.findItemToEject(inv, i));
        }
    }
