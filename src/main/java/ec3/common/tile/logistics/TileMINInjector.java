...
public class TileMINInjector extends TileMRUGeneric {
    ...
    public boolean injectItem(IInventory inv, int slotNum) {
        ...
        if (this.getStackInSlot(1) != null) {
            if (injected != null) {
                ...
                ItemStack inserted = this.getStackInSlot(1)
                    .copy();
    ...
    public int[] getAccessibleSlotsFromSide(ForgeDirection side) {
        if (this.worldObj.getTileEntity(xCoord + side.offsetX, yCoord + side.offsetY, zCoord + side.offsetZ) != null) {
            ...
            if (tile instanceof ISidedInventory) {
                return ((ISidedInventory) tile).getAccessibleSlotsFromSide(
                    side.getOpposite()
                        .ordinal());
    ...
    public int findItemToInject(IInventory inv) {
        if (this.getAccessibleSlotsFromSide(this.getRotation()) != null
            && this.getAccessibleSlotsFromSide(this.getRotation()).length > 0) {
            ...
            for (int i = 0; i < slots.length; ++i) {
                ...
                if (filter == null) {
                    ...
                    if (in == null || (in.isItemEqual(stk) && ItemStack.areItemStackTagsEqual(stk, in)
                        && in.stackSize + 1 <= in.getMaxStackSize())) return slots[i];
                } else {
                    if (ECUtils.canFilterAcceptItem(new InventoryMagicFilter(filter), stk, filter)
                        && (in == null || (in.stackSize + 1 <= in.getMaxStackSize() && in.isItemEqual(stk)
                            && ItemStack.areItemStackTagsEqual(in, stk)))) {
                        return slots[i];
                    }
                }
            }
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord))
            if (this.getAccessibleSlotsFromSide(this.getRotation()) != null) {
                IInventory inv = (IInventory) this.worldObj.getTileEntity(
                    xCoord + this.getRotation().offsetX,
                    yCoord + this.getRotation().offsetY,
                    zCoord + this.getRotation().offsetZ);
                if (this.findItemToInject(inv) != -1) this.injectItem(inv, this.findItemToInject(inv));
            }
    }
