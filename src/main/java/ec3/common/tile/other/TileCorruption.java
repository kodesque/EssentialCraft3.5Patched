...
public class TileCorruption extends TileEntity {
    ...
    public void onDataPacket(NetworkManager net, S35PacketUpdateTileEntity pkt) {
        if (net.getNetHandler() instanceof INetHandlerPlayClient) if (pkt.func_148853_f() == -11) {
    ...
    public void updateEntity() {
        try {
            super.updateEntity();
            Potion potionToAdd = ECPotions.mruCorruptionPotion;
            Block blk = this.worldObj.getBlock(xCoord, yCoord, zCoord);
            if (blk == ECBlocks.lightCorruption[0]) {
                potionToAdd = ECPotions.chaosInfluence;
            }
            if (blk == ECBlocks.lightCorruption[1]) {
                potionToAdd = ECPotions.frozenMind;
            }
            ...
            if (this.worldObj.rand.nextFloat() <= 0.0001F) {
                int biomeId = 0;
                if (blk == ECBlocks.lightCorruption[0]) {
                    biomeId = ECBiomes.chaosCorruption.biomeID;
                }
                if (blk == ECBlocks.lightCorruption[1]) {
                    biomeId = ECBiomes.frozenCorruption.biomeID;
                }
                if (blk == ECBlocks.lightCorruption[2]) {
                    biomeId = ECBiomes.shadowCorruption.biomeID;
                }
                if (blk == ECBlocks.lightCorruption[3]) {
                    biomeId = ECBiomes.magicCorruption.biomeID;
                }
                if (!this.worldObj.isRemote) changeBiomeAtPos(biomeId);
            }
            int metadata = this.worldObj.getBlockMetadata(xCoord, yCoord, zCoord);
            if (metadata >= 7 && canDestroyBlocks) {
                for (int i = 0; i < 6; ++i) {
                    ForgeDirection dir = ForgeDirection.VALID_DIRECTIONS[i];
                    if (this.worldObj.getTileEntity(xCoord + dir.offsetX, yCoord + dir.offsetY, zCoord + dir.offsetZ)
                        == null
                        && this.worldObj.getBlock(xCoord + dir.offsetX, yCoord + dir.offsetY, zCoord + dir.offsetZ)
                            .isBlockSolid(
                                worldObj,
                                xCoord + dir.offsetX,
                                yCoord + dir.offsetY,
                                ...
                                dir.getOpposite()
                                    .ordinal())) {
                        this.worldObj
                            .setBlock(xCoord + dir.offsetX, yCoord + dir.offsetY, zCoord + dir.offsetZ, blk, 0, 3);
                    }
                }
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "CorruptionSettings",
                "tileentities",
                new String[] { "Change Biome:true", "Destroy Blocks if grown:true" },
