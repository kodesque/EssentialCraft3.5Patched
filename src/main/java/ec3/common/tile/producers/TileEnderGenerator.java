...
public class TileEnderGenerator extends TileMRUGeneric {
    ...
    public void updateEntity() {
        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            AxisAlignedBB endermenTPRadius = AxisAlignedBB.getBoundingBox(
                xCoord - endermenCatchRadius,
                yCoord - endermenCatchRadius,
                zCoord - endermenCatchRadius,
                xCoord + endermenCatchRadius,
                yCoord + endermenCatchRadius,
                zCoord + endermenCatchRadius);
            ...
            if (Loader.isModLoaded("HardcoreEnderExpansion")) {
                try {
                    ...
                    l.addAll(
                        (List<EntityEnderman>) this.worldObj.getEntitiesWithinAABB(
                            Class.forName("chylex.hee.entity.mob.EntityMobAngryEnderman"),
                            endermenTPRadius));
                    l.addAll(
                        (List<EntityEnderman>) this.worldObj.getEntitiesWithinAABB(
                            Class.forName("chylex.hee.entity.mob.EntityMobBabyEnderman"),
                            endermenTPRadius));
                    l.addAll(
                        (List<EntityEnderman>) this.worldObj.getEntitiesWithinAABB(
                            Class.forName("chylex.hee.entity.mob.EntityMobEndermage"),
                            endermenTPRadius));
                    l.addAll(
                        (List<EntityEnderman>) this.worldObj.getEntitiesWithinAABB(
                            Class.forName("chylex.hee.entity.mob.EntityMobEnderman"),
                            endermenTPRadius));
                    l.addAll(
                        (List<EntityEnderman>) this.worldObj.getEntitiesWithinAABB(
                            Class.forName("chylex.hee.entity.mob.EntityMobParalyzedEnderman"),
                            endermenTPRadius));
            ...
            if (!l.isEmpty()) {
                for (int i = 0; i < l.size(); ++i) {
                    l.get(i)
                        .setPositionAndRotation(xCoord + 0.5D, yCoord + 1D, zCoord + 0.5D, 0, 0);
                }
            }
            AxisAlignedBB endermanAttackRad = AxisAlignedBB
            ...
            if (!l1.isEmpty()) {
                if (!this.worldObj.isRemote) {
                    for (int i = 0; i < l1.size(); ++i) {
                        if (l1.get(i)
                            .attackEntityFrom(DamageSource.magic, 1)) {
