        ...
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            if (!this.worldObj.isRemote) {
                if (this.burnedFlower == null && this.getMRU() < this.getMaxMRU()) {
                    ...
                    if (name.toLowerCase()
                        .contains("sapling") || b instanceof BlockSapling) {
                        burnedFlower = new Coord3D(xCoord + offsetX, yCoord, zCoord + offsetZ);
                    ...
                    if (this.burnedFlower != null) {
                        ...
                        if (burnTime <= 0) {
                            if (createDeadBush) this.worldObj.setBlock(
                                (int) burnedFlower.x,
                                (int) burnedFlower.y,
                                (int) burnedFlower.z,
                                Blocks.deadbush,
            ...
            if (this.burnedFlower != null && burnTime > 0) {
                ...
                --burnTime;
                if (burnTime <= 0) {
                    for (int t = 0; t < 200; ++t) EssentialCraftCore.proxy.SmokeFX(
                        burnedFlower.x + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.3F,
                        burnedFlower.y + 0.1F + this.worldObj.rand.nextFloat() / 2,
                        burnedFlower.z + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.3F,
                        0,
    ...
    public static void setupConfig(Configuration cfg) {
        try {
            ...
            String[] cfgArrayString = cfg.getStringList(
                "NatureFurnaceSettings",
                "tileentities",
                new String[] { "Max MRU:" + ApiCore.GENERATOR_MAX_MRU_GENERIC, "Default balance(-1 is random):-1.0",
                    "MRU generated per tick:10", "Time required to burn a flower:600",
                    "Time required to burn a sapling:900", "Time required to burn grass:150",
