...
public class TileUltraFlowerBurner extends TileMRUGeneric {
    ...
    public void updateEntity() {
        ...
        if (this.getBalance() == -1) {
            this.setBalance(this.worldObj.rand.nextFloat() * 2);
        }
        if (!this.worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
            if (!this.worldObj.isRemote) {
                if (this.worldObj.getWorldTime() % 80 == 0) {
                    List<EntityItem> sapplings = this.worldObj.getEntitiesWithinAABB(
                        EntityItem.class,
                                ...
                                if (sStk != null) {
                                    ...
                                    if (name.contains("treeSapling")) {
                                        ...
                                        if (b.isAir(worldObj, pBIDX, pBIDY, pBIDZ)) {
                                            ...
                                            if (sBlk != null) {
                                                this.worldObj
                                                    .setBlock(pBIDX, pBIDY, pBIDZ, sBlk, sStk.getItemDamage(), 3);
                                                sappling.setDead();
                                            }
                                        }
                                    }
                ...
                if (this.burnedFlower == null && this.getMRU() < this.getMaxMRU()) {
                    ...
                    y: for (int y = 32; y >= 0; --y) {
                        ...
                        if (name.toLowerCase()
                            .contains("flower") || b instanceof BlockFlower) {
                            burnedFlower = new Coord3D(xCoord + offsetX, yCoord + y, zCoord + offsetZ);
                            burnTime = 30 * 20;
                            mruProduced = 60;
                        }
                        if (name.toLowerCase()
                            .contains("leaves") || b.isLeaves(worldObj, xCoord + offsetX, yCoord, zCoord + offsetZ)) {
                            burnedFlower = new Coord3D(xCoord + offsetX, yCoord + y, zCoord + offsetZ);
                            burnTime = 15 * 20;
                            mruProduced = 70;
                        }
                        if (name.toLowerCase()
                            .contains("tallgrass") || b instanceof BlockTallGrass
                            || b instanceof BlockDoublePlant) {
                            burnedFlower = new Coord3D(xCoord + offsetX, yCoord + y, zCoord + offsetZ);
                            burnTime = 5 * 20;
                            mruProduced = 20;
                        }
                        if (name.toLowerCase()
                            .contains("logWood") || b instanceof BlockLog) {
                            burnedFlower = new Coord3D(xCoord + offsetX, yCoord + y, zCoord + offsetZ);
                            burnTime = 120 * 20;
                            mruProduced = 100;
                        }
                        if (mruProduced > 0) break y;
                    }
                } else {
                    if (this.burnedFlower != null) {
                        ...
                        if (this.getMRU() > this.getMaxMRU()) this.setMRU(this.getMaxMRU());
                        if (burnTime <= 0) {
                            if (this.worldObj.getBlock((int) burnedFlower.x, (int) burnedFlower.y, (int) burnedFlower.z)
                                .isLeaves(worldObj, (int) burnedFlower.x, (int) burnedFlower.y, (int) burnedFlower.z)) {
                                Item droppedSapling = this.worldObj
                                    .getBlock((int) burnedFlower.x, (int) burnedFlower.y, (int) burnedFlower.z)
                                    .getItemDropped(
                                ...
                                if (droppedSapling != null) {
                                    if (this.worldObj.rand.nextFloat() < 0.05F) {
                                        ItemStack saplingStk = new ItemStack(
                                            droppedSapling,
                                            1,
                                            this.worldObj
                                                .getBlock(
                                                ...
                                                .damageDropped(
                                                    this.worldObj.getBlockMetadata(
                                                        ...
                                                        (int) burnedFlower.y,
                                                        (int) burnedFlower.z)));
                                        EntityItem sapling = new EntityItem(
                                            this.worldObj,
                                            (int) burnedFlower.x + 0.5D,
                                            (int) burnedFlower.y + 0.5D,
                                            (int) burnedFlower.z + 0.5D,
                                            saplingStk);
                            ...
                            this.worldObj
                                .setBlockToAir((int) burnedFlower.x, (int) burnedFlower.y, (int) burnedFlower.z);
                            burnedFlower = null;
                            mruProduced = 0;
                        }
                    }
            ...
            if (this.burnedFlower != null && burnTime > 0) {
                EssentialCraftCore.proxy.FlameFX(
                    burnedFlower.x + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.3F,
                    burnedFlower.y + 0.1F + this.worldObj.rand.nextFloat() / 2,
                    burnedFlower.z + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.3F,
                    0,
                    0,
                    0,
                    1,
                ...
                EssentialCraftCore.proxy.FlameFX(
                    ...
                    burnedFlower.y + 0.1F + this.worldObj.rand.nextFloat() / 2,
                    burnedFlower.z + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.3F,
                    ((xCoord - 0.5D) - burnedFlower.x + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.3F) / 20,
                    (yCoord - burnedFlower.y + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.3F) / 20,
                    ((zCoord - 0.5D) - burnedFlower.z + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.3F) / 20,
                    1,
                    1,
                    1,
                    1);
                --burnTime;
                if (burnTime <= 0) {
                    for (int t = 0; t < 600; ++t) EssentialCraftCore.proxy.SmokeFX(
                        burnedFlower.x + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.3F,
                        burnedFlower.y + 0.1F + this.worldObj.rand.nextFloat() / 2,
                        burnedFlower.z + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.3F,
                        0,
        ...
        EssentialCraftCore.proxy.FlameFX(
            ...
            1);
        EssentialCraftCore.proxy.FlameFX(
            xCoord + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.2F,
            yCoord + 0.2F,
            zCoord + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.2F,
            0,
            0.01F,
            0,
            1D,
            ...
            1,
            1);
        for (int i = 0; i < 10; ++i) EssentialCraftCore.proxy.SmokeFX(
            xCoord + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.1F,
            yCoord + 0.6F,
            zCoord + 0.5F + MathUtils.randomFloat(this.worldObj.rand) * 0.1F,
            0,
            0,
            0,
            1);
