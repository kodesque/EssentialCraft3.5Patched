    ...
    public void updateEntity() {
                ...
                if (!this.worldObj.isRemote) {
                    ...
                    for (int i = 0; i < 4; ++i) {
                        if (b[i] == Blocks.air) {
                            ...
                            mruFactor *= (((IHotBlock) b[i])
                                .getHeatModifier(getWorldObj(), xCoord + ox[i], yCoord, zCoord + oz[i]));
                        } else {
                            mruFactor *= 0.5F;
                        }
            ...
            if (!this.worldObj.isRemote) {
                if (this.getStackInSlot(0) != null) {
                    if (this.currentBurnTime <= 0 && this.getMRU() < this.getMaxMRU()) {
                        this.currentMaxBurnTime = this.currentBurnTime = TileEntityFurnace
                            .getItemBurnTime(this.getStackInSlot(0));

                        if (this.currentBurnTime > 0) {
                            if (this.getStackInSlot(0) != null) {

                                if (this.getStackInSlot(1) == null
                                    || this.getStackInSlot(1).stackSize < this.getInventoryStackLimit()) {
                                    ...
                                    if (this.getStackInSlot(1) == null) {
                                        ItemStack stk = new ItemStack(ECItems.magicalSlag, 1, 0);
                                        this.setInventorySlotContents(1, stk);
                                    }
                                }
                ...
                if (this.heat > 0) {
                    ...
                    for (int i = 0; i < 4; ++i) {
                        if (i == 0) this.worldObj
                            .spawnParticle("flame", xCoord + 0.05D, yCoord + 1.2F, zCoord + 0.05D, 0, 0.01f, 0);
                        if (i == 1) this.worldObj
                            .spawnParticle("flame", xCoord + 0.95D, yCoord + 1.2F, zCoord + 0.05D, 0, 0.01f, 0);
                        if (i == 2) this.worldObj
                            .spawnParticle("flame", xCoord + 0.05D, yCoord + 1.2F, zCoord + 0.95D, 0, 0.01f, 0);
                        if (i == 3) this.worldObj
                            .spawnParticle("flame", xCoord + 0.95D, yCoord + 1.2F, zCoord + 0.95D, 0, 0.01f, 0);
                    }
                }
            }
        }
        for (int i = 2; i < 6; ++i) {
            ...
            EssentialCraftCore.proxy.FlameFX(
                xCoord + 0.725F + rotXAdv / 2.2F,
                yCoord + 0.4F,
                zCoord + 0.725F + rotZAdv / 2.2F,
                0,
                0F,
                0,
                0.8D,
