                    ...
                    for (int dy = -1; dy <= 1; ++dy) {
                        for (int dz = z - direction.offsetX; dz <= z + direction.offsetX; ++dz) {
                            ...
                            if (b != Blocks.chest && b != Blocks.air
                                ...
                                && !(b instanceof BlockLeaves)
                                && (b != BlocksCore.concrete)
                                && (b != BlocksCore.root)) w.setBlock(dx, y + dy, dz, Blocks.air, 0, 3);
                        }
                    }
                }
            }
                    ...
                    for (int dy = -2; dy <= 2; ++dy) {
                        for (int dz = z + direction.offsetX * 2; dz <= z - direction.offsetX * 2; ++dz) {
                            ...
                            if (b != Blocks.chest && b != Blocks.air
                                ...
                                && !(b instanceof BlockLeaves)
                                && (b != BlocksCore.concrete)
                                && (b != BlocksCore.root)) w.setBlock(dx, y + dy, dz, generateFrom, 0, 3);
                        }
                    }
                }
                for (int dx = x - direction.offsetZ; dx <= x + direction.offsetZ; ++dx) {
    ...
    public void generateBrokenPath(World w, int x, int y, int z) {
        for (int i = -2; i <= 2; ++i) {
            for (int j = -2; j <= 2; ++j) {
                for (int k = -2; k <= 2; ++k) {
                    int tryInt = j + 3;
                    if (w.rand.nextInt(tryInt) == 0) w.setBlock(x + i, y + j, z + k, BlocksCore.concrete);
                }
            }
        }
    }
                ...
                for (int k = -3; k <= 3; ++k) {
                    if (w.rand.nextInt(3) == 0) {
                        ...
                        if (b != Blocks.chest && b != Blocks.air
                            ...
                            && !(b instanceof BlockLeaves)
                            && (b != BlocksCore.concrete)
                            && (b != BlocksCore.root)) w.setBlock(x + i, y + j, z + k, Blocks.leaves);
                    }
                }
            }
        }
