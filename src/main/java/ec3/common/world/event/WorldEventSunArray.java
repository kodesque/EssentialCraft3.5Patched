...
public class WorldEventSunArray implements IWorldEvent {
    ...
    public void onEventBeginning(World w) {
        ECUtils.sendChatMessageToAllPlayersInDim(
            Config.dimensionID,
            EnumChatFormatting.RED + "This is going to be a hot day...");
    }
    ...
    public void playerTick(EntityPlayer p, int leftoverTime) {
        ...
        if (b != null) {
            for (int i = 0; i < b.getSizeInventory(); ++i) {
                ...
                if (is != null && is.getItem() != null
                    ...
                    && is.getItemDamage() == 19) ignoreSun = true;
            }
        }
        if (!p.capabilities.isCreativeMode && p.dimension == Config.dimensionID
            && p.worldObj.canBlockSeeTheSky(
                MathHelper.floor_double(p.posX),
                MathHelper.floor_double(p.posY + 2),
        ...
        if (p.dimension == Config.dimensionID) {
            EntityFallingBlock sand = new EntityFallingBlock(
                p.worldObj,
                p.posX + MathUtils.randomDouble(p.worldObj.rand) * 128,
                255,
                p.posZ + MathUtils.randomDouble(p.worldObj.rand) * 128,
                Blocks.fire);
            sand.field_145812_b = 3;
            sand.field_145813_c = false;
            if (!p.worldObj.isRemote) p.worldObj.spawnEntityInWorld(sand);
        }
    ...
    public void onEventEnd(World w) {
        ECUtils.sendChatMessageToAllPlayersInDim(
            Config.dimensionID,
            EnumChatFormatting.GREEN + "The suns are back to normal!");
    }
