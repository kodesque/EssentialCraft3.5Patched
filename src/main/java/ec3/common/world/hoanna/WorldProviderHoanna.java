...
public class WorldProviderHoanna extends WorldProvider {
    ...
    protected void registerWorldChunkManager() {
        this.worldChunkMgr = new WorldChunkManagerHoanna(this.worldObj.getSeed(), this.terrainType);
    ...
    public void generateLightBrightnessTable() {
        ...
        for (int i = 0; i <= 15; ++i) {
            float f1;
            if (!ECUtils.isEventActive("ec3.event.darkness")) f1 = 1.0F - (float) i / 15.0F;
            else f1 = 1.9F - (float) i / 15.0F;
    ...
    public IChunkProvider createChunkGenerator() {
        return new ChunkProviderHoanna(this.worldObj, this.worldObj.getSeed(), true);
    }
