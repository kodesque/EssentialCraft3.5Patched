...
public class WorldGenElementalDrops extends WorldGenerator {
    ...
    public boolean generate(World w, Random rnd, int x, int y, int z) {
        ...
        new WorldGenMinable(ECBlocks.oreDrops, meta, 16, minableReplacable).generate(w, rnd, x, y, z);

        new WorldGenMinable(ECBlocks.oreDrops, meta + 1, 8, minableReplacable).generate(
            w,
            rnd,
            x + rnd.nextInt(6) - rnd.nextInt(6),
            y + rnd.nextInt(6) - rnd.nextInt(6),
            z + rnd.nextInt(6) - rnd.nextInt(6));
        new WorldGenMinable(ECBlocks.oreDrops, meta + 2, 8, minableReplacable).generate(
            w,
            rnd,
            x + rnd.nextInt(6) - rnd.nextInt(6),
            y + rnd.nextInt(6) - rnd.nextInt(6),
            z + rnd.nextInt(6) - rnd.nextInt(6));
        new WorldGenMinable(ECBlocks.oreDrops, meta + 3, 8, minableReplacable).generate(
            w,
            rnd,
            x + rnd.nextInt(6) - rnd.nextInt(6),
            y + rnd.nextInt(6) - rnd.nextInt(6),
            z + rnd.nextInt(6) - rnd.nextInt(6));
        new WorldGenMinable(ECBlocks.oreDrops, meta + 4, 8, minableReplacable).generate(
            w,
            rnd,
            x + rnd.nextInt(6) - rnd.nextInt(6),
            y + rnd.nextInt(6) - rnd.nextInt(6),
    ...
    public static void handleGeneration(Random random, int chunkX, int chunkZ, World world,
        ...
        for (int i = 0; i < Config.oreGenAttempts; ++i) {
            ...
            new WorldGenElementalDrops(world, type, x, y, z).selectValidAABBGen()
