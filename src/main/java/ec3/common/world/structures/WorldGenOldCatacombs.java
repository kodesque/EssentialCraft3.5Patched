...
public class WorldGenOldCatacombs extends WorldGenerator {
    ...
    public static final WeightedRandomChestContent[] generatedItems = new WeightedRandomChestContent[] {
        new WeightedRandomChestContent(ECItems.titanite, 0, 8, 32, 20),
        new WeightedRandomChestContent(ECItems.twinkling_titanite, 0, 2, 16, 10),
        new WeightedRandomChestContent(ECItems.genericItem, 5, 1, 16, 15),
        new WeightedRandomChestContent(ECItems.genericItem, 6, 1, 16, 15),
        new WeightedRandomChestContent(ECItems.genericItem, 7, 1, 16, 15),
        new WeightedRandomChestContent(ECItems.genericItem, 8, 1, 16, 15),
        new WeightedRandomChestContent(ECItems.genericItem, 9, 1, 16, 15),
        new WeightedRandomChestContent(ECItems.genericItem, 10, 1, 16, 15),
        new WeightedRandomChestContent(ECItems.genericItem, 11, 1, 16, 15),
        new WeightedRandomChestContent(ECItems.genericItem, 20, 1, 12, 10),
        new WeightedRandomChestContent(ECItems.genericItem, 3, 1, 16, 15),
        ...
        new WeightedRandomChestContent(ECItems.magicalSlag, 0, 1, 16, 70),
        new WeightedRandomChestContent(ECItems.ember, 0, 1, 16, 10),
        new WeightedRandomChestContent(ECItems.ember, 1, 1, 16, 10),
        new WeightedRandomChestContent(ECItems.ember, 2, 1, 16, 10),
        new WeightedRandomChestContent(ECItems.ember, 3, 1, 16, 10),
        new WeightedRandomChestContent(ECItems.ember, 4, 1, 16, 10),
        new WeightedRandomChestContent(ECItems.ember, 5, 1, 16, 10),
        new WeightedRandomChestContent(ECItems.ember, 7, 1, 16, 10),
        new WeightedRandomChestContent(ECItems.ember, 6, 1, 16, 10),
        new WeightedRandomChestContent(ECItems.bauble, 0, 1, 1, 15) };
    ...
    public boolean generate(World w, Random r, int x, int y, int z) {
        ...
        if (this.cameFrom != ForgeDirection.SOUTH) genTry[0] = ForgeDirection.NORTH;
        if (this.cameFrom != ForgeDirection.NORTH) genTry[1] = ForgeDirection.SOUTH;
        if (this.cameFrom != ForgeDirection.EAST) genTry[2] = ForgeDirection.WEST;
        if (this.cameFrom != ForgeDirection.WEST) genTry[3] = ForgeDirection.EAST;
                ...
                if (genTry[i] != null)
                    if (generateCorridor(w, x, y, z, lengthGenned, genTry[i], r.nextFloat() <= greenRoomChance)) {
                        ...
                        catacombs.generate(
                            w,
                            r,
                            x + ForgeDirection.values()[i + 2].offsetX * 10,
                            y,
                            z + ForgeDirection.values()[i + 2].offsetZ * 10);
                    }
            }
        } else {
            if (w.rand.nextFloat() < 0.03F) generateWayUp(w, x, y, z);
            if (w.getBlock(x, y, z) != Blocks.chest) {
                w.setBlock(x, y, z, Blocks.chest, 0, 3);
                w.setBlock(x, y - 1, z, ECBlocks.voidStone);
                TileEntityChest chest = (TileEntityChest) w.getTileEntity(x, y, z);
                if (chest != null) {
                    WeightedRandomChestContent
                        .generateChestContents(w.rand, generatedItems, chest, w.rand.nextInt(12) + 6);
    ...
    public boolean generateCorridor(World w, int x, int y, int z, int length, ForgeDirection direction, boolean green) {
        ...
        boolean gen = true;
        while (gen) {
            if (w.rand.nextFloat() < this.destroyedRoomChance) generateBrokenPath(w, x, y, z);
            w.getBlock(x, y, z);
                ...
                for (int dx = x - direction.offsetZ * 2; dx <= x + direction.offsetZ * 2; ++dx) {
                    for (int dy = -2; dy <= 2; ++dy) {
                        for (int dz = z - direction.offsetX * 2; dz <= z + direction.offsetX * 2; ++dz) {
                            ...
                            if (b != Blocks.chest && b != Blocks.air
                    ...
                    for (int dy = -1; dy <= 1; ++dy) {
                        for (int dz = z - direction.offsetX; dz <= z + direction.offsetX; ++dz) {
                            ...
                            if (b != Blocks.chest && b != Blocks.air
                                ...
                                && !(b instanceof BlockLeaves)
                                && (b != ECBlocks.concrete)
                                && (b != ECBlocks.root)) w.setBlock(dx, y + dy, dz, Blocks.air, 0, 3);
                        }
                    }
                }
            }
                    ...
                    for (int dy = -2; dy <= 2; ++dy) {
                        for (int dz = z + direction.offsetX * 2; dz <= z - direction.offsetX * 2; ++dz) {
                            ...
                            if (b != Blocks.chest && b != Blocks.air
                                ...
                                && !(b instanceof BlockLeaves)
                                && (b != ECBlocks.concrete)
                                && (b != ECBlocks.root)) w.setBlock(dx, y + dy, dz, generateFrom, 0, 3);
                        }
                    }
                }
                for (int dx = x - direction.offsetZ; dx <= x + direction.offsetZ; ++dx) {
                    ...
                    for (int dy = -2; dy <= 2; ++dy) {
                        for (int dz = z - direction.offsetX * 2; dz <= z + direction.offsetX * 2; ++dz) {
                            ...
                            if (b != Blocks.chest && b != Blocks.air
                                ...
                                && (b != ECBlocks.concrete)
                                && (b != ECBlocks.root))
                                if (!w.isAirBlock(dx, y + dy, dz)) w.setBlock(dx, y + dy, dz, generateFrom, 0, 3);
                        }
                    }
                }
                for (int dx = x + direction.offsetZ; dx <= x - direction.offsetZ; ++dx) {
            ...
            x += direction.offsetX;
            z += direction.offsetZ;
            if (w.rand.nextFloat() < this.greenRoomChance) generateGrownPath(w, x, y, z);
        }
        return false;
    }

    public Block getBlockToGenFrom() {
        return this.blockToGenerateOf == null ? ECBlocks.fortifiedStone : blockToGenerateOf;
    }
    ...
    public void generateBrokenPath(World w, int x, int y, int z) {
        for (int i = -2; i <= 2; ++i) {
            for (int j = -2; j <= 2; ++j) {
                for (int k = -2; k <= 2; ++k) {
                    int tryInt = j + 3;
                    if (w.rand.nextInt(tryInt) == 0) w.setBlock(x + i, y + j, z + k, ECBlocks.concrete);
                }
            }
        }
    }
    ...
    public void generateGrownPath(World w, int x, int y, int z) {
        Vec3 rootVec = Vec3
            .createVectorHelper(MathUtils.randomDouble(w.rand) * 3, -6, MathUtils.randomDouble(w.rand) * 3);
        for (int vi = 0; vi <= 6; ++vi) {
            w.setBlock(
                (int) (x + rootVec.xCoord / vi),
                (int) (y - 2 - rootVec.yCoord / vi),
                (int) (z + rootVec.zCoord / vi),
                ECBlocks.root);
        }
        for (int i = -3; i <= 3; ++i) {
            for (int j = -3; j <= 3; ++j) {
                for (int k = -3; k <= 3; ++k) {
                    if (w.rand.nextInt(3) == 0) {
                        ...
                        if (b != Blocks.chest && b != Blocks.air
                            ...
                            && !(b instanceof BlockLeaves)
                            && (b != ECBlocks.concrete)
                            && (b != ECBlocks.root)) w.setBlock(x + i, y + j, z + k, Blocks.leaves);
                    }
                }
            }
        }
    ...
    public boolean generateWayUp(World w, int x, int y, int z) {
        ...
        while (maxY < 256) {
            ...
            boolean isAir = true;
            for (int i = 0; i < 10; ++i) {
                if (!w.isAirBlock(x, maxY + i, z)) isAir = false;
            }
            if (isAir) break;
        }
