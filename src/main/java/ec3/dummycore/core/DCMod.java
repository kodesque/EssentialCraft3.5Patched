...
public class DCMod {
    ...
    public DCMod(Class<?> mod, String uName) {
        ...
        if (Loader.instance()
            .hasReachedState(LoaderState.INITIALIZATION)) {
            Notifier.notifyWarn(
                String.format(
                    "Mod %s is being registered in %s state! This can cause all kinds of unexpected behaviour!",
                    ufName + "[modid:" + modid + ",classpath:" + mod + "]",
                    Loader.instance()
                        .getLoaderState()
                        .toString()));
        }
