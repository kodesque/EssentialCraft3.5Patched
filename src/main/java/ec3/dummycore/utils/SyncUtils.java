...
public class SyncUtils {
    ...
    public static void makeSync_LotsSmallPackets() {
        for(int i = 0; i < needsSyncing_Global.size(); ++i) {
            String syncID = (String)needsSyncing_Global.get(i);
            ...
            if (indexOfStrip != -1) {
                ...
                if (DummyCore.Utils.MiscUtils.registeredServerWorldData.containsKey(modid + "|" + dataName)) {
                    String dataString = "||mod:DummyCore.InfoSync||" + modid + ":" + dataName + "||ddata:" + (String) DummyCore.Utils.MiscUtils.registeredServerWorldData.get(modid + "|" + dataName);
                    ...
                    DummyCore.Utils.Notifier.notifyCustomMod(modid, "The sync packet for data " + modid + "|" + dataName + " could not be generated - the requested server data does not exist!");
                }
            }
        }

        for(int i = 0; i < needsSyncing_Player.size(); ++i) {
            String syncID = (String)needsSyncing_Player.get(i);
            int indexOfStrip = syncID.indexOf(":");
            ...
            if (indexOfPlayer != -1 && indexOfStrip != -1) {
                ...
                String dataName = syncID.substring(indexOfStrip + 1);
                if (DummyCore.Utils.MiscUtils.registeredServerData.containsKey(playerName + "_" + modid + "|" + dataName)) {
                    String dataString = "||mod:DummyCore.PlayerInfoSync||playerName:" + playerName + "||" + modid + ":" + dataName + "||ddata:" + (String) MiscUtils.registeredServerData.get(playerName + "_" + modid + "|" + dataName);
