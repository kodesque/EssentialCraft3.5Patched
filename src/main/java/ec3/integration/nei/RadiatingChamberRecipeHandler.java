...
public class RadiatingChamberRecipeHandler extends TemplateRecipeHandler {
    ...
    public void loadUsageRecipes(String inputId, Object... ingredients) {
        if (ingredients.length > 0) {
            ...
            if (inputId.equalsIgnoreCase(getOverlayIdentifier())) loadCraftingRecipes(inputId, ingredients);
        }
    }

    public void loadTransferRects() {
        super.transferRects
            .add(new RecipeTransferRect(new Rectangle(18, 20, 18, 18), this.getOverlayIdentifier(), new Object[0]));
    }
    ...
    public void drawExtras(int recipe) {
        ...
        Minecraft.getMinecraft().fontRenderer
            .drawStringWithShadow((int) (rec.mruRequired * rec.mod) + " MRU", posX + 2, posY + 5, 0xffffff);

        Minecraft.getMinecraft().fontRenderer.drawStringWithShadow(
            "" + rec.mruRequired / 20 / 60
                + "Min "
                + (rec.mruRequired / 20 - ((rec.mruRequired / 20 / 60) * 60))
                + "Sec",
            posX + 2 + 60,
            posY + 5,
            0xffffff);
        ...
        Minecraft.getMinecraft().fontRenderer
            .drawStringWithShadow("Upper Balance: " + upperBalance, posX + 38, 5, 0xffffff);

        Minecraft.getMinecraft().fontRenderer
            .drawStringWithShadow("MRU/Tick: " + (int) rec.mod, posX + 38, 23, 0xffffff);

        Minecraft.getMinecraft().fontRenderer
            .drawStringWithShadow("Lower Balance: " + lowerBalance, posX + 38, 40, 0xffffff);
