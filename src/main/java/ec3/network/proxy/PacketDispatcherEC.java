...
public class PacketDispatcherEC implements IMessageHandler<PacketNBT, IMessage> {
    ...
    public IMessage onMessage(PacketNBT message, MessageContext ctx) {
        switch (message.packetID) {
            case 0: {
                // Generic data SYNC.
                ECUtils.readOrCreatePlayerData(
                    message.theTag.hasKey("syncplayer") ? MinecraftServer.getServer()
                        .getConfigurationManager()
                        .func_152612_a(message.theTag.getString("syncplayer"))
                        : EssentialCraftCore.proxy.getClientPlayer(),
                    message.theTag);
                break;
            }
            case 1: {
                // Server-side to client Sync request
                EntityPlayer target = MinecraftServer.getServer()
                    .getConfigurationManager()
                    .func_152612_a(message.theTag.getString("syncplayer"));
                EntityPlayer clientPlayer = MinecraftServer.getServer()
                    .getConfigurationManager()
                    .func_152612_a(message.theTag.getString("sender"));
                if (target != null) {
                    NBTTagCompound theTag = new NBTTagCompound();
                    theTag.setString("syncplayer", target.getCommandSenderName());
                    ECUtils.getData(target)
                        .writeToNBTTagCompound(theTag);
            ...
            case 3: {
                TileEntity tile = EssentialCraftCore.proxy.getClientPlayer().worldObj.getTileEntity(
                    message.theTag.getInteger("x"),
                    message.theTag.getInteger("y"),
                    message.theTag.getInteger("z"));
                if (tile != null && tile instanceof TileNewMIMInventoryStorage) {
                    TileNewMIMInventoryStorage.class.cast(tile).items.clear();
                    for (int i = 0; i < message.theTag.getTagList("items", 10)
                        .tagCount(); ++i) {
                        ItemStack s = ItemStack.loadItemStackFromNBT(
                            message.theTag.getTagList("items", 10)
                                .getCompoundTagAt(i));
                        s.stackSize = message.theTag.getTagList("items", 10)
                            .getCompoundTagAt(i)
                            .getInteger("stackSize");
            ...
            case 5: {
                ...
                EntityPlayerMP requester = MinecraftServer.getServer()
                    .getConfigurationManager()
                    .func_152612_a(message.theTag.getString("requester"));

                if (!message.theTag.getBoolean("craft")) retrieved.stackSize = message.theTag.getInteger("size");

                int size = message.theTag.getInteger("size");
                TileEntity tile = requester.worldObj.getTileEntity(
                    message.theTag.getInteger("px"),
                    message.theTag.getInteger("py"),
                    message.theTag.getInteger("pz"));
                if (tile != null && tile instanceof TileNewMIM) {
                    ...
                    if (!message.theTag.getBoolean("craft")) {
                        ...
                        TileEntity t = requester.worldObj.getTileEntity(
                            message.theTag.getInteger("x"),
                            message.theTag.getInteger("y"),
                            message.theTag.getInteger("z"));
            ...
            case 7: {
                ...
                if (tag != null && !tag.hasNoTags()) {
                    ...
                    EntityPlayer requester = MinecraftServer.getServer()
                        .getConfigurationManager()
