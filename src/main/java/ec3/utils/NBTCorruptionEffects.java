...
public class NBTCorruptionEffects implements ICorruptionEffect {
    ...
    public ResourceLocation[] allLocs = new ResourceLocation[] { loc("body_hands"), loc("body_hands"),
        loc("body_hands"), loc("body_legs"), loc("body_legs"), loc("mind_white"), loc("mind_black"), loc("mind_purple"),
        loc("mind_red"), loc("mind_blue"), loc("mind_teal"), loc("matrix_damage"), loc("matrix_end"),
        loc("matrix_explosion"), loc("matrix_lava"), loc("matrix_lightning"), loc("matrix_matrix"), loc("matrix_moon"),
        loc("matrix_mru"), loc("matrix_nether"), loc("matrix_overworld"), loc("matrix_rain"), loc("matrix_sun"),
        loc("matrix_water"), loc("matrix_wind") };
    ...
    public NBTCorruptionEffects() {}

    public NBTCorruptionEffects setType(EnumCorruptionEffect e) {
        type = e;
        return this;
    }

    public NBTCorruptionEffects setCost(int i) {
        cost = i;
        return this;
    }

    public NBTCorruptionEffects setMeta(int i) {
        meta = i;
        return this;
    }

    public NBTCorruptionEffects done() {
    ...
    public void onPlayerTick(EntityPlayer player) {
        if (!player.worldObj.isRemote) {
            switch (this.meta) {
                ...
                case 5: {
                    if (player.worldObj.rand.nextFloat() <= 0.001F && !player.isPotionActive(ECPotions.paradox)) {
                        message(player, "ec3.effect.desc_" + meta);
                        player.addPotionEffect(new PotionEffect(ECPotions.paradox.id, 2000, 1, true));
                    }
                    break;
                }
                case 6: {
                ...
                case 8: {
                    if (player.worldObj.rand.nextFloat() <= 0.00005F) {
                        ...
                        for (int i = 0; i < player.inventory.getSizeInventory(); ++i) {
                            if (player.inventory.getStackInSlot(i) != null) playerItems.add(
                                player.inventory.getStackInSlot(i)
                                    .copy());
                            else playerItems.add(null);
                        }
                    ...
                    if (player.worldObj.rand.nextFloat() <= 0.001F) {
                        ...
                        if (player.inventory.getCurrentItem() != null) {
                            player.inventory.getCurrentItem()
                                .useItemRightClick(player.worldObj, player);
                        }
                    }
                    break;
                }
                ...
                case 17: {
                    if (player.worldObj.rand.nextFloat() <= 0.01F && !player.worldObj.isRaining()
                        && !player.worldObj.isDaytime()
                        && player.worldObj.isDaytime()
                        && player.worldObj.canBlockSeeTheSky(
                            MathHelper.floor_double(player.posX),
                            MathHelper.floor_double(player.posY + player.eyeHeight),
                            MathHelper.floor_double(player.posZ))) {
                        int moonPhase = (int) (player.worldObj.getWorldTime() / 24000L % 8L + 8L) % 8;
                        int damage = 0;
                        if (moonPhase == 0) damage = 5;
                        if (moonPhase == 1 || moonPhase == 7) damage = 3;
                        if (moonPhase == 2 || moonPhase == 6) damage = 2;
                        if (moonPhase == 3 || moonPhase == 5) damage = 1;
                ...
                case 18: {
                    if (player.worldObj.rand.nextFloat() <= 0.001F
                        && player.isPotionActive(ECPotions.mruCorruptionPotion.id)) {
                ...
                case 21: {
                    if (player.worldObj.rand.nextFloat() <= 0.1F && player.worldObj.isRaining()
                        && player.worldObj.isDaytime()
                        && player.worldObj.canBlockSeeTheSky(
                            MathHelper.floor_double(player.posX),
                            MathHelper.floor_double(player.posY + player.eyeHeight),
                ...
                case 22: {
                    if (player.worldObj.rand.nextFloat() <= 0.003F && player.worldObj.isDaytime()
    ...
    public static void message(EntityPlayer p, String message) {
        // p.addChatComponentMessage(new ChatComponentText(StatCollector.translateToLocal(message)).setChatStyle(new
    ...
    public ICorruptionEffect copy() {
        ...
        NBTCorruptionEffects effect = new NBTCorruptionEffects();
    ...
    public boolean effectEquals(ICorruptionEffect effect) {
        return effect.getClass() == this.getClass()
            ? (NBTCorruptionEffects.class.cast(effect).cost == this.cost
                && NBTCorruptionEffects.class.cast(effect).meta == this.meta
                && NBTCorruptionEffects.class.cast(effect).type == this.type)
            : false;
    }
