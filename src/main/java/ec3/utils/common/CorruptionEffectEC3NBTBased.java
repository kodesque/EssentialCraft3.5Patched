...
public class CorruptionEffectEC3NBTBased implements ICorruptionEffect {
    ...
    public void onPlayerTick(EntityPlayer player) {
        if (!player.worldObj.isRemote) {
            switch (this.meta) {
                ...
                case 18: {
                    if (player.worldObj.rand.nextFloat() <= 0.001F
                        && player.isPotionActive(PotionRegistry.mruCorruptionPotion.id)) {
    ...
    public boolean effectEquals(ICorruptionEffect effect) {
        return effect.getClass() == this.getClass()
            ? (CorruptionEffectEC3NBTBased.class.cast(effect).cost == this.cost
                && CorruptionEffectEC3NBTBased.class.cast(effect).meta == this.meta
                && CorruptionEffectEC3NBTBased.class.cast(effect).type == this.type)
            : false;
    }
