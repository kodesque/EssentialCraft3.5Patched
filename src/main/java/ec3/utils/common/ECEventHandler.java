...
import ec3.dummycore.events.DummyEvent_OnClientGUIButtonPress;
...
public class ECEventHandler {
                ...
                if (event.source.getSourceOfDamage() instanceof EntityPlayer) {
                    ...
                    if (radiation) {
                        if (event.entityLiving instanceof EntityPlayer) {
                            RadiationManager.increasePlayerRadiation(
    ...
    @SubscribeEvent
    public void guiButtonPressed(DummyEvent_OnClientGUIButtonPress event) {
        ...
        if (event.client_ParentClassPath.equalsIgnoreCase("ec3.client.gui.GuiMagicalAssembler")
            && event.buttonID == 0) {
    ...
    public void getBiomeFolliageColor(BiomeEvent.GetFoliageColor event) {
        ...
        if (player != null) {
            int dimID = player.dimension;
            if (dimID == Config.dimensionID) {
                if (!(event.biome instanceof BiomeGenCorruption_Chaos)
                    && !(event.biome instanceof BiomeGenCorruption_Frozen)
                    && !(event.biome instanceof BiomeGenCorruption_Magic)) {
