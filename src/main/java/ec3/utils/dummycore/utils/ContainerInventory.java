    ...
    public ItemStack transferStackInSlot(EntityPlayer entityPlayer, int slotIndex) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotIndex < inv.getSizeInventory()) {
                if (!this.mergeItemStack(itemStack, inv.getSizeInventory(), inventorySlots.size(), false)) return null;
            } else if (!this.mergeItemStack(itemStack, 0, inv.getSizeInventory(), false)) return null;
