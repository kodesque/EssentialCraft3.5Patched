package ec3.utils.dummycore.utils.FX;
...
public class ArcFX {
     ...
     */
    public ArcFX(Random rand, Coord3D begin, Coord3D stop, float curve, float... color) {
        rnd = rand;
        start = begin;
        end = stop;
    ...
    private void curve(float factor) {
        ...
        generateLightningBetween2Points(
            lightningVecsStart[3],
            new Coord3D(end.x * (factor * 10), end.y, end.z * (factor * 10)),
            8,
            4,
            factor * 3);
        int genIndex = 0;
        for (int i = 0; i < 12; ++i) {
            ...
            generateLightningBetween2Points(
                ...
                new Coord3D(
                    lightningVecsStart[i].x + MathUtils.randomFloat(rnd) * (genDistance / 4),
                    lightningVecsStart[i].y + MathUtils.randomFloat(rnd) * (genDistance / 4),
                    lightningVecsStart[i].z + MathUtils.randomFloat(rnd) * (genDistance / 4)),
                12 + i,
                1 + rndSteps,
                factor / 2);
        }
    ...
    private void generateLightningBetween2Points(Coord3D from, Coord3D to, int beginVecIndex, int steps, float curve) {
        ...
        for (int i = 0; i < steps; ++i) {
            if (i == 0) {
                ...
                lightningVecsEnd[beginVecIndex + i] = new Coord3D(
                    lightningVecsStart[beginVecIndex + i].x + distX / steps + MathUtils.randomFloat(rnd) * curve,
                    lightningVecsStart[beginVecIndex + i].y + distY / steps + MathUtils.randomFloat(rnd) * curve,
                    lightningVecsStart[beginVecIndex + i].z + distZ / steps + MathUtils.randomFloat(rnd) * curve);
            } else {
    ...
    public void render(double x, double y, double z, float partialTicks) {
        ...
        if (renderTicksExisted >= this.lightningVecsStart.length) renderTicksExisted = this.lightningVecsStart.length;
        GL11.glPushMatrix();
        GL11.glEnable(2929);
        GL11.glEnable(3042);
    ...
    public void renderBeam(Coord3D begin, Coord3D stop, float type) {
        if (type != 1F) GL11.glLineWidth(3);
        else GL11.glLineWidth(1);
        GL11.glPushMatrix();
        ...
        if (type != 1) GL11.glColor4f(1, 1, 1, 0.2F);
        else GL11.glColor4f(this.getLightningColor[0], this.getLightningColor[1], this.getLightningColor[2], 0.8F);
