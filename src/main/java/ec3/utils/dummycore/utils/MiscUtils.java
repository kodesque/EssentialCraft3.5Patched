...
public class MiscUtils {
    ...
    public static final List<BlockPos> unbreakableBlocks = new ArrayList<BlockPos>();
    ...
    public static void syncTileEntity(ITEHasGameData t, Side s) {
        ...
        DummyPacket simplePacket = new DummyPacket(dataString);
        if (s == Side.CLIENT) {
    ...
    public static void syncTileEntity(NBTTagCompound tileTag, int packetID) {
        DummyPacketTile simplePacket = new DummyPacketTile(tileTag);
        CoreInitializer.network.sendToAll(simplePacket);
    }
            ...
            for (int i = 0; i < playerLst.size(); ++i) {
                ...
                if (player instanceof EntityPlayerMP) {
                    if (pkt instanceof S35PacketUpdateTileEntity) {
                        ...
                        if (player.dimension == dimId) ((EntityPlayerMP) player).getServerForPlayer()
                            .func_73046_m()
     ...
     */
    public static void playSoundOnServerToAllNearby(double x, double y, double z, String sound, float volume,
        float pitch, double radius, int dim) {
        ...
        DummyPacket pkt = new DummyPacket(
            "||mod:DummyCore.Sound" + aaa + "" + aab + "" + aac + "" + aad + "" + aae + "" + aaf);
        DummyPacketHandler.sendToAllAround(pkt, new TargetPoint(dim, x, y, z, radius));
    }

    /**
     * Adds a potion effect to the player.<BR>
     * If the effect exists - increases the duration.<BR>
     * If the duration is over specified amount adds +1 level.
     *
    ...
    public static void calculateAndAddPE(EntityLivingBase mob, Potion potion, int index, int index2) {
        ...
        if (hasEffect) {
            int currentDuration = mob.getActivePotionEffect(potion)
                .getDuration();
    ...
    public static void renderItemStack_Full(ItemStack stk, double posX, double posY, double posZ, double screenPosX,
        ...
        DrawUtils.renderItemStack_Full(
            stk,
            posX,
            posY,
            posZ,
    ...
    public static Entity cloneEntity(Entity e) {
        ...
        try {
            retEntity = e.getClass()
                .getConstructor(World.class)
                .newInstance(e.worldObj);
    ...
    public static float applyPotionDamageCalculations(EntityLivingBase base, DamageSource dam, float amount) {
        if (dam.isDamageAbsolute()) {
            ...
            if (base.isPotionActive(Potion.resistance) && dam != DamageSource.outOfWorld) {
                i = (base.getActivePotionEffect(Potion.resistance)
                    .getAmplifier() + 1) * 5;
                j = 25 - i;
                f1 = amount * (float) j;
                amount = f1 / 25.0F;
     ...
     * Sometimes considered as a dirty hacking of the java code. I agree. There is nothing more dirty, than just
     * removing the FINAL modifier of the variable. It's like Java can't even do anything, no matter the protection
     * given.
     * This should not be done. However, in vanilla MC it is pretty much the only way to do so, so I can't help it.
     * The only thing, that would be worse is using ASM to remotely change the compiled final variable. That is the most
     * disgusting thing you can do with Java, I believe.
     *
     ...
     */
    public static void setPrivateFinalValue(Class<?> classToAccess, Object instance, Object value,
        String fieldNames[]) {
    ...
    public static int extendPotionArray(int byAmount) {
        ...
        setPrivateFinalValue(
            Potion.class,
            null,
            potionTypes,
            ObfuscationReflectionHelper
                .remapFieldNames(Potion.class.getName(), new String[] { "potionTypes", "field_76425_a", "a" }));
    ...
    public static void setBlockUnbreakable(World w, int x, int y, int z, boolean remove) {
        if (!isBlockUnbreakable(w, x, y, z) && !remove) {
            ...
            for (int i = 0; i < unbreakableBlocks.size(); ++i) {
                BlockPos pos = unbreakableBlocks.get(i);
    ...
    @SideOnly(Side.CLIENT)
    public static void handleButtonPress(int buttonID, Class<? extends Gui> parentClass,
        Class<? extends GuiButton> buttonClass, EntityPlayer presser, int bX, int bY, int bZ, String additionalData) {
    ...
    public static void openGui(World w, int x, int y, int z, EntityPlayer player, int guiID) {
        player.openGui(CoreInitializer.instance, guiID, w, x, y, z);
    }

    public static void setShaders(int shaderID) {
        if (shaderID >= defaultShaders.length) shaderID = defaultShaders.length - 1;
        if (shaderID < 0) setShaders(null);
        else CoreInitializer.proxy.initShaders(defaultShaders[shaderID]);
    }

    public static void setShaders(ResourceLocation shaders) {
        CoreInitializer.proxy.initShaders(shaders);
    }
