...
public class UnformedItemStack {
    ...
    public UnformedItemStack(Object obj) {
        if (obj instanceof ItemStack) possibleStacks.add((ItemStack) obj);
        if (obj instanceof String) possibleStacks.addAll(OreDictionary.getOres((String) obj));
        if (obj instanceof List<?>) possibleStacks.addAll((List<ItemStack>) obj);
        if (obj instanceof ItemStack[]) possibleStacks.addAll(Arrays.asList((ItemStack[]) obj));
        if (obj instanceof Block) possibleStacks.add(new ItemStack((Block) obj, 1, OreDictionary.WILDCARD_VALUE));
        if (obj instanceof Item) possibleStacks.add(new ItemStack((Item) obj, 1, OreDictionary.WILDCARD_VALUE));
        sort();
    }
    ...
    public boolean itemStackMatches(ItemStack is) {
        ...
        for (ItemStack s : possibleStacks) {
            if (is.isItemEqual(s) || (is.getItem()
                .equals(s.getItem()) && s.getItemDamage() == OreDictionary.WILDCARD_VALUE)) return true;
        }
        return false;
    }
